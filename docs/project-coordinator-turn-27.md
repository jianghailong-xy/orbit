...
2. **§7.2 TU6 的 `REQUEST_APPROVAL` + USER blocker 没有实现。** MANUAL 策略下实际发生的是 §1.7 那三件事：
   不开 turn、不花键、请求保持未消费，外加一行 warn。它**不静默**（既没有代人执行，也没有把请求丢掉），但也
   还没有把"等你批"变成控制面上一条看得见的行 —— 那条行属于 §9.2 的审批面，不在本任务的 AC 里。
3. **`ACCEPTANCE` 与 `REPLAN` 仍未求值**（26 的边界，本轮没有动）。因此没有任何项目开始收到 `REPLAN` turn。
4. **没有新增迁移、没有改 Prisma schema、没有改任何一条已冻结契约条款**（`docs/` 只加了 §18 的单元 27 一行 +
   本文）。§33 作为 v1.17 的修订日志按仓库惯例不回写 —— 它说的是"v1.17 没做"，那句话对 v1.17 仍然为真。
5. **没有改写任何历史失败 Session**，也没有用任何形式的"完成"覆盖真实运行结果。
6. **没有合并进 `main`。** 收工时 `origin/main` 是 `00841cfb`（开工时 `9b814afd`，期间有别的会话并入过东西），
   里面既没有单元 26 的 `project-turn-reason.ts` 也没有本单元 —— 没有并发重复劳动。
