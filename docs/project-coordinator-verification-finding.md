...
- `src/apiserver/prisma/migrations/0135_task_verification_finding/` —— 数据库这一半。
...
3. **原子性**：迁移 0135 的 `task_verification_verdict_atomic` 触发器 + `TasksService.updateTask` 的拒绝，
...
聚合只能提供前者；写 DONE 会铸出 0135 拒绝的形状，而且是由唯一一个背后没有人的写入者铸出来的。反向
...
`world.protocol.findings` 只在 0135 已应用的数据库上被打上；未打上时读作「这份快照早于 finding 驱动的
