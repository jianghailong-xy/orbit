...
- `TranscriptReducer` 对 `state.items` **没有任何上限**(grep `prefix(|dropFirst|maxItems` 全无命中),
  ...
  6.8 MB 快照**(§6),那是单个 console 的量级,不是 12 个的总和。
...
| 收紧常驻 console 数 / items 上限 | 12 console × 528 items = +13.6 MB(Linux 代理值);items 无上限,长会话单个 4,490 items;**模拟器实测:打开 6 个超长会话 +40~47 MB 就到平台,离开不回落** —— 但**"依次打开会话"验收不出 items 上限**,要用"盯着流式跑"或"往上翻页",见 §3.3.1 | §3.3、**§3.3.1**、§6 |
...
| 减少 transcript 快照写放大 | 长会话单次 save 6.8 MB / 编码 150 ms(x86)/ 42 ms(arm64);流式时 ≈ 102 MB/分钟 | §6 |
| 给 seen 设上界 | **只占快照 0.4–0.6%(41 KB / 6.8 MB)—— 优先级应下调** | §6 |
| 避免 rows 每次 body 全量重建 | 流式 turn 里占 **90.4%(x86)/ 95.6%(arm64)**;4,490 items 单次 13.63 ms(x86)/ 4.19 ms(arm64) | §7 |
