...
> 本节的绝对值来自 **2026-08-24 的 Mac(Apple Silicon,Xcode 26 工具链)**,不是 §1.4 那台 x86,
> 也不是 §6 表里 08-22 那台 Mac mini(Swift/JSONEncoder 版本不同,uncapped 字节 6,897,763 vs 6,806,406)。
> **改前/改后两行来自同一次运行,可以直接比;不要拿它去和上面两张表的绝对值相减。**
...
| 减少 transcript 快照写放大 | ~~长会话单次 save 6.8 MB~~ → **items 上限之后单次 1.53 MB / ≈ 23 MB/分钟**(§6.1)。剩下的是**写入节奏和增量**的问题,不再是单次体积 | §6、**§6.1** |
