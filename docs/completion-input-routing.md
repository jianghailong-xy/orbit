...
| Parked-attempt human blocker/comment | Session lifecycle and absence of L0/L1/L2 paths | N11 HUMAN_SIGNOFF request creation | `HUMAN_SIGNOFF_REQUESTED + requestId + criterionRevision/evidenceDigest` | `HUMAN_INBOX` (N12 request/inbox delivery), never an agent Session |
| No first-class replacement delivery | Whole-project rescan happened later | N11 request supersession by new evidence | `HUMAN_SIGNOFF_REQUEST_SUPERSEDED + oldRequestId + replacementRequestId/replacementDigest` | `HUMAN_INBOX` |
| General Task settlement after signoff | All Project Task statuses | `TasksService.signoff` decision commit | `HUMAN_SIGNOFF_DECIDED + requestId + signoffId/evidenceDigest/decision` | `DERIVED_COMPLETION_EVALUATOR` |
...
`AWAITING_INPUT` neither refuses nor delays evidence/request routing. HUMAN_SIGNOFF consumers are
