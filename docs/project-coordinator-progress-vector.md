...
| `prisma/migrations/0140_task_progress_vector_identity/` | 四个新列、九个计数的 CHECK/单调触发器、回填 |
...
触发器一条语句——迁移补一个当时还不存在的 key，不是改写这条判断的结论，而不补的代价是每个 pre-0140 的
任务在恢复时都被报成账本与列不一致。

列默认值也要一起改：0138 的 `DEFAULT` 是七个 key 的字面量，不改的话 0140 之后新建的每个任务都会在第一次
