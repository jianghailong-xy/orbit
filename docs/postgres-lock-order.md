...
| 60 | `task_dependency` / `task_comment` / `task_completion_evidence` / `task_completion_evidence_idempotency` / `conversation_turn` / `run_event` / `tool_call` / `attachment` / `project_event` / `project_action` / `project_acceptance_criterion_definition` / `project_acceptance_criteria_confirmation` / `project_acceptance_conclusion` / `failure_continuation_attempt_receipt` / `failure_continuation_obligation` / `failure_continuation_wakeup_outbox` / `failure_continuation_route_decision` / `failure_successor_handoff` / `failure_successor_current_binding` / `failure_successor_dependency_rebind` | 只插入/更新/删除 | 子行：到这一步它们的外键父行都已经在手，不会引入新的等待边。完成证据先锁 `task`(50)，再追加 evidence 与幂等键子行；来源 Session/Attempt 是不取锁、无外键的审计快照。Acceptance definition 的整组替换先锁 `project`(40)，再移动/删除/写入这些子行；标准集确认与 conclusion 同样都在 project 锁后追加，conclusion 的 BEFORE INSERT 验证触发器也使数据库直写维持 40 → 60。Typed failure receipt/obligation/outbox 由 0210 的两个 AFTER trigger 在已经持有 project(40)、task(50)、attempt/continuation 子行后追加；0211 路由只锁一个已领取 outbox，再用 project/fingerprint advisory key 串行追加 decision，不回取父行写锁。0212 handoff 在 owner graph mutex、Session、project 与失败 source Task 都已锁定后，追加不可变接管证据、推进唯一 current binding，并物理移动每条下游边；边触发器最后推进 revision(70)。投递器后续每次只 CAS 一行 outbox，Session effect 不在该事务里。 |
...
| `POST /runner/sessions/:id/turn-complete` | `session`(30) → typed executable 时 `project`(40) → `task`(50) → conversation/attempt/receipt/continuation/obligation/outbox 等子行(60) → **一次** `session` 写 | merge 状态清理已折进 park 那一次写；只有 park 一行都没匹配上（UPDATE 匹配 0 行不改任何元组，也就不动 `xmin`）或提前 ack 返回时，才单独发一条 —— 两条路径上都仍然只有一次真正的写。失败 continuation 的低延迟 `kick()` 在提交后发生，触发事务内没有 Session 创建或其它外部 effect。 |
| `POST /runner/sessions/:id/inbox`（dequeue） | `session`(30) → conversation_turn(60) | 不写 `session` 行 |
| failure successor 自动 Session | task-run receipt → `project`(40；当前 no-owner/capable successor 由 immutable handoff 授权) → `task`(50) → Session INSERT | 0212 的专用授权只匹配 current binding；旧 generation、owner-required 或 capability 不可用的 route 都不能绕过最终门。 |
...
| `task_executable_attempt_failure_continuation_receipt` / `task_executable_continuation_failure_wakeup`（0210） | 已持有 `project`(40)、`task`(50) 后，从 attempt/continuation 子行追加 receipt → obligation → outbox(60)；只做无锁父事实读取和子行 INSERT | **新增，保留** | 两个 trigger 把 typed termination、不可改写 receipt、canonical DIAGNOSIS obligation 和 wakeup outbox 留在同一提交边界。外键父行已由 `turnComplete` 持有；兼容 sweep 用 `FOR UPDATE SKIP LOCKED` 只锁 continuation/outbox 子行，不回头取得 Task/Project 写锁。投递、Session 创建与 ACK 全部在提交后，并由 generation/token fence 隔开。 |
| `failure_continuation_route_claim`（0211） | 锁一个已领取的 wakeup outbox(60)，读取 immutable receipt/current binding/capability snapshot，再取 project+fingerprint advisory key 并追加 route decision(60) | **新增，保留** | 同一调用不再取得 Project/Task 写锁；不同 outbox 的相同 fingerprint 在 advisory key 上同序，保证 generation 与三代 evidence streak 不分叉。route decision 是 append-only，重复 lease/delivery 直接重放首个 decision。 |
| ~~`outcome_binding_transition_append_only`、`outcome_obligation_reduction_append_only`、`outcome_obligation_successor_append_only`、`outcome_obsolete_obligation_append_only`、`outcome_proof_obsolescence_append_only`、`outcome_proof_successor_append_only`（0196）~~ | — | **已删除（0222）** | 义务代数整体移除；六个 append-only 边界随各自的表一起消失，等待图中不再有它们。 |
| ~~`outcome_matching_fact_invalidates_reduction`、`outcome_authority_revocation_invalidates_reduction`、`outcome_binding_transition_record`（0196）~~ | — | **已删除（0222）** | 三个 producer 触发器随 canonical fact / binding / authority 表一起删除，它们那条 stream → 义务 的链在等待图中不复存在。 |
