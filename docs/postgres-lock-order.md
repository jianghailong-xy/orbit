...
| 60 | `task_dependency` / `task_comment` / `task_completion_evidence` / `task_completion_evidence_idempotency` / `conversation_turn` / `run_event` / `tool_call` / `attachment` / `project_event` / `project_action` / `project_acceptance_criterion_definition` | 只插入/更新/删除 | 子行：到这一步它们的外键父行都已经在手，不会引入新的等待边。完成证据先锁 `task`(50)，再追加 evidence 与幂等键子行；来源 Session/Attempt 是不取锁、无外键的审计快照。Acceptance definition 的整组替换先锁 `project`(40)，再移动/删除/写入这些子行。0179 的 conclusion 子行原本也在这一秩，0229 随项目验收判定一起删除。0210–0212 的失败 receipt/obligation/outbox/route/handoff 子行原本也在这一秩，0226 随失败路由一起删除，它们在等待图中不复存在。 |
...
| `POST /runner/sessions/:id/turn-complete` | `session`(30) → typed executable 时 `project`(40) → `task`(50) → conversation/attempt/continuation 等子行(60) → **一次** `session` 写 | merge 状态清理已折进 park 那一次写；只有 park 一行都没匹配上（UPDATE 匹配 0 行不改任何元组，也就不动 `xmin`）或提前 ack 返回时，才单独发一条 —— 两条路径上都仍然只有一次真正的写。0226 删除失败路由后，这个事务不再追加 receipt/obligation/outbox，提交后也不再有 `kick()`。 |
...
| ~~`task_executable_attempt_start_guard`（0200，BEFORE INSERT ON task_executable_attempt）、`task_executable_plan_bind`（0200，BEFORE INSERT/UPDATE ON task）、`project_acceptance_run_closure_guard`（0215）~~ | — | **已删除（0227_executable_acceptance_runtime_removal）** | EXECUTABLE 验收运行时整体移除。start guard 曾在每次 attempt INSERT 上取 `task` FOR UPDATE(50) 再取 admission(60) 并回写 `task.execution_attempt_count`；plan bind 在每次 task 声明写入上重算两个 digest；0215 的闭合守卫挂在 `project_acceptance_run` 上。三者与它们的表一起消失，等待图里不再有 attempt→task 这条边。EXECUTABLE 仍按退出码判定（0177 的两列 + 0181 的 request/result + `task-completion-criterion.ts`），那条路径不取额外行锁。 |
| ~~`task_executable_attempt_failure_continuation_receipt` / `task_executable_continuation_failure_wakeup`（0210）、`failure_successor_task_binding_immutable`（0212）、`failure_continuation_route_claim`（0211）~~ | — | **已删除（0226_failure_continuation_successor_removal）** | 失败路由与 successor 接管整体移除。两个 0210 的 AFTER trigger 曾在 `task_executable_*` 的每次 typed termination 上追加 receipt → obligation → outbox(60)，0212 的 `task` 触发器在每次 status/supersession 更新上点火，0211 的路由 reducer 串行追加 decision——三者连同它们的表一起消失，等待图里不再有这些边。普通 supersede 仍由 0128/0130 的既有守卫把关，形状不变。 |
...
状态、完成策略或任务 verdict 的变化不再触发项目验收重开，因此这里原先记录的
`task → project` 验收等待边已经不存在。0229 之后连被调用的那个函数本身也不存在了。
