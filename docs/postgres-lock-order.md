...
| failure successor 自动 Session | task-run receipt → `project`(40，普通任务 ratification；当前 no-owner/capable successor 由 immutable handoff 授权) → `task`(50) → Session INSERT | 0212 的专用授权只匹配 current binding；旧 generation、owner-required 或 capability 不可用的 route 都不能绕过最终门。 |
