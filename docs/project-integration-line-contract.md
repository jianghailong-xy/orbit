...
**G5（不加唤醒事件、不加 blocker kind）**：新机制放在新表里。`COORDINATOR_WAKE_EVENTS` 与 `project_blocker_kind_chk` 两个闭集不增加成员：前者每加一个事件要改 8–10 处，而且正是本项目要替换的旧通道；后者被 `project-source-contract.spec.ts`（SR50/SR51）按设计拒绝任何新增。
