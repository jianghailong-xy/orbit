# Project · Agent · Workspace 领域契约 v1.1
...
> **代码基线**：`a4adabf9 feat(project): make a project's DONE stand on evidence the server can check`，
...
> （错误码闭合、快照两段式不重叠、§13→§14 映射完整、§7.5 的 id 都被 §10 覆盖）。改本文件必须让它继续绿。
...
|---|---|---|---|
| 1 | 以 `b5fe1b2e` 为基线、要求"迁移 0111 创建 `project_member`"，而该表已由 0111 创建且 `agent_id` 指向 `workspace` | 基线更新到 `a4adabf9`/0127；`project_member` 是**既有表**，v1 只做**保序重指向**（同一行、同一 id、同一 role），迁移编号 `0128` | §3.2 · §11.2 |
...
- **T4（保序重指向）**：迁移必须满足四条可断言的性质：① 行数不变；② 每一行的 `id` / `project_id` / `role` / `added_at` 逐字节不变；③ 新 `agent_id` 与旧 `agent_id` 之间存在双射（`agent.legacy_workspace_id`）；④ 每个 Project 的 COORDINATOR 归属对象不变（旧 workspace 的镜像 Agent 就是新的 COORDINATOR）。
...
**REFUSE 发生在第 6 步之前，因此此刻没有 Session。** 审计必须落在不需要 Session 的行上，规则见 §7.4 AU-F —— 这条约束适用于**派发路径上**的每一个 §12 错误码，不只是 `PROVIDER_UNAVAILABLE`。（写入路径的拒绝码 —— `TASK_PROVIDER_PIN_REFUSED`、`PERMISSION_WIDENING_REFUSED`、`TEAM_COORDINATOR_EXISTS`、`AGENT_IN_USE`、`WORKSPACE_IN_USE`、`TASK_CONTRACT_NEEDS_PROJECT` —— 直接返回给写入方，不落 blocker：它们拒绝的是一次编辑，不是一次运行。）
...
| 优先级 | 条件 | 结果 |
|---|---|---|
| 1 | `task.workspaceId` 非空 | 该 Workspace。**两道校验都要过**：(a) 它必须在候选集内 —— 不在则 REFUSE `WORKSPACE_PIN_NOT_A_CANDIDATE`，pin 不是绕过候选集这道安全边界的口子；(b) 它必须满足需求集 —— 不满足则 REFUSE `RUNTIME_REQUIREMENT_UNMET`。两种情况都**绝不改派**。`resolution.where.source = "task-pin"` |
| 2 | Default Workspace ∈ 可行集 | Default Workspace，`source = "project-default"` |
| 3 | 可行集非空 | 按 `project_workspace.position ASC, id ASC` 取第一个（确定性 tie-break），`source = "project-candidate"` |
...
- **AU-F2**：`project_blocker` 的其余必填列（`owner` / `recovery` / `severity` / `required_action` / `next_check_at` /
  `dedupe_key` / `lifecycle_generation` / `condition_version`）按 PCC（`docs/project-coordinator-contract.md`）§11.2 中**同一个 kind 那一行**取值。
  两份契约在这件事上只有一张表，本文件不复制它，避免同一个 kind 在两处有两套默认值。
...
- **L5**：迁移之后新建的 Project Task 由服务显式写成 V1，因此**创建时就必须给 `assigneeAgentId`**（否则第一次派发 `WHO_UNRESOLVED`）。
  05B/05C 的创建表单必须把它做成必填项。这是**刻意的显式失败**，不是回归 —— 它只作用于迁移之后新建的任务。
...
编号按合并时刻的下一个可用值；本文冻结时 `feat/project` 的最新迁移是 `0127_project_acceptance_run`，故为 **0128**。
...
| 4 | **保序重指向 `project_member.agent_id`**（§3.2 T4）：同一事务内 ① `UPDATE project_member m SET agent_id = a.id FROM agent a WHERE a.legacy_workspace_id = m.agent_id`；② `ALTER TABLE project_member DROP CONSTRAINT project_member_agent_id_fkey`；③ 重新 `ADD CONSTRAINT … FOREIGN KEY (agent_id) REFERENCES agent(id) ON DELETE RESTRICT`。**不 DROP TABLE、不 DELETE、不换 `id`**；`project_member_project_id_agent_id_key`、`project_member_coordinator_idx`、`project_member_agent_id_idx` 三个索引原地存活 | 重跑时 ① 匹配不到行（`agent_id` 已是 agent id）→ 0 行更新；②③ 用 `IF EXISTS` / 固定约束名 |
...
|---|---|---|---|
| `WHO_UNRESOLVED` | 409 | V1 Task 没有 `assigneeAgentId` | taskId |
| `WHO_NOT_IN_TEAM` | 403 | 指派 / 派发时 Agent 不在 Team | agentId, projectId |
| `WHO_DISABLED` | 409 | Agent 被 disable 或软删 | agentId |
| `PROVIDER_UNAVAILABLE` | 409 | Provider 校验失败且 fallback 链为空或耗尽 | 按序尝试过的 provider 列表 |
| `RUNTIME_REQUIREMENT_UNMET` | 409 | 可行集为空，或 pin 的 Workspace 在候选集内但不满足需求 | 需求集 + 每个候选缺哪些能力 |
| `NO_PROJECT_WORKSPACE` | 409 | Project **一个候选 Workspace 都没有**（候选集为空） | projectId |
| `WORKSPACE_PIN_NOT_A_CANDIDATE` | 409 | `task.workspaceId` pin 的 Workspace **不在**本 Project 的候选集内 | taskId, workspaceId, projectId |
| `TASK_PROVIDER_PIN_REFUSED` | 400 | 往 V1 Task 上写 `provider` / `model`，或晋升时写入后的行仍带 pin | taskId, 字段名 |
| `TASK_CONTRACT_NEEDS_PROJECT` | 400 | 把无 Project 的 Task 置为 `execution_contract = 'V1'`，或给它写 `assigneeAgentId` | taskId |
...
|---|---|---|---|
| 1 | Project 可配 Coordinator + 多成员 Agent，公开 ID 均 Base62 | §3.2, §10 | `+` `02B.1` · `+` `03B.1` · `-` `02B.3` · `-` `04A.12` · `M` `02A.8` |
| 2 | Agent 可配 Provider/Model/提示词/能力/权限；Provider 不再由普通 Task 直接配置 | §3.1, §3.4 K1, §7.2 P1 | `+` `02A.1` · `+` `03A.1` · `+` `06A.4` · `-` `02C.3` · `-` `04A.6` · `-` `04A.8` · `-` `03C.14` |
| 3 | Task 只从 Project Team 选 assigneeAgent；Coordinator 建树，执行 Agent 在权限内建子任务 | §8.1, §8.2, §8.4 | `+` `03C.1` · `+` `03C.2` · `+` `03C.11` · `+` `03C.13` · `-` `03C.3` · `-` `03C.4` · `-` `03C.5` · `-` `03C.6` · `-` `03C.7` · `-` `03C.10` · `-` `03C.12` |
...
| 6 | Web/API/CLI 对 Team、Agent 配置、指派与调度结果有完整可独立运行的测试，并覆盖迁移 | §13 全表 | `+` `05A.2` · `+` `05B.1` · `+` `05C.1` · `+` `05D.1` · `-` `00.7` · `-` `05A.3` · `M` `06B.1` |
| 7 | UI 能管理团队，并在任务树里清晰展示 Assignee Agent 及其 Provider | §11.4, §13.13–13.15 | `+` `05B.1` · `+` `05B.2` · `+` `05C.1` · `+` `05D.2` · `-` `05B.3` · `-` `05C.3` · `-` `05D.5` |
| 8 | 既有行为兼容，且**可验证**普通开发 Task 不会再被错调度到 Mac Runner | §11, §7.3 | `+` `06A.1` · `+` `06A.2` · `+` `06A.6` · `+` `04A.11` · `+` `06B.5` · `-` `06A.5` · `M` `02C.8` · `M` `06B.1` · `M` `06B.3` |
...
>
> 第 2 条的证明分两半：`02C.3` 证明**写不进去**（数据库 CHECK），`04A.8` 证明**读不出来**（V1 的 resolution 里
> 永远不会出现 `task-pin`）。v1 的映射只拒绝 Member 修改 pin，而 Owner/Coordinator 仍可直接配置，
> 证明的恰好是这条验收标准的反面。
...
- **O7**：PCC（`docs/project-coordinator-contract.md`）§7.4 EC1 的决策输入投影目前声明它读 `tasks[].provider`，
  那是按 v1 的 WITH 链写的。v1.1 之后 V1 Task 上没有 pin，而控制环需要多读一列 `tasks[].executionContract`
  才能知道一条 Task 走哪条链。**PCC 是另一份带独立评审史与自检测试的冻结契约，本次不改它**；
  这条差异在 02–04 实施 `resolveExecutionContext` 之前必须由一个单独的任务在 PCC 上关闭。
