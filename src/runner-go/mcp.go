...
func toolDescriptors(includePermissionPrompt, includeOrchestration bool) []map[string]interface{} {
	...
	// runAt is the one-time scheduled start the server's once-a-minute sweep keeps
	// (dispatchDueScheduledTasks). Stated once for task_create, every task_create_batch item and
	// task_update, so what a due schedule still has to meet cannot drift between the three doors.
	runAtDescription := "A one-time scheduled start: an ISO 8601 date-time (e.g. \"2026-09-21T12:34:00Z\") at or after which the server starts this task by itself. " +
		"It is not a deadline — dueDate is a date nothing dispatches on, runAt is a trigger. " +
		"Once it is due, the server's scan, which runs once a minute, starts the task for you the way task_start would, whether or not autoRunWhenReady is set: that flag is the separate trigger for prerequisites finishing, and it too holds off until a future runAt arrives. " +
		"The run it starts clears runAt back to null, so a schedule fires once. " +
		"A due schedule still starts only a task that is OPEN, not held (dispatchHold false), with an assignee bound to a runner, every prerequisite satisfied and no session already occupying it; until then it is kept rather than spent, and every later scan tries again. " +
		"A time already past is started by the next scan. " +
		"task_start ignores runAt and starts the task immediately, and that run clears runAt as well."
	runAtProp := map[string]interface{}{"type": "string", "description": runAtDescription}
	updateRunAtProp := map[string]interface{}{
		"type":        []string{"string", "null"},
	...
	taskCreateProps := func() map[string]interface{} {
		return map[string]interface{}{
			...
			"completionCriterion": map[string]interface{}{
				...
				"description": "The task's one normal completion criterion. EXECUTABLE uses acceptanceCommand plus acceptanceExpectedExitCode; VERIFICATION uses an independent task's verdict with completionPolicy VERIFICATION_PASSED; EVIDENCE_JUDGMENT uses one decision on the task's own submitted evidence; OWNER_CONFIRMED (completionPolicy MANUAL, no acceptanceCommand, no verifiesTaskId) is settled only by the account owner confirming it in the Orbit app — no agent session can confirm it, a coordinator included, so a run of it finishes the work, says what it did, and ends its turn. They are peer choices, not a fallback chain. Runner task creation always requires this explicit field; related command, policy, and verifier fields do not replace it. Two facts about the VERIFICATION shape decide whether that row lives. completionPolicy VERIFICATION_PASSED makes it a pure gate, and a pure gate is NOT dispatchable: a manual execute is refused with 409 and auto-dispatch passes over it, so a row that has work of its own declares MANUAL. And the server never files the verifier for you — the check has to arrive in the SAME call, as `verification` on a single create or `verifiesRef` on a batch item. Without it the row waits on a check nobody is going to file.",
			},
	...
	tools := []map[string]interface{}{
		...
		},
		{
			...
			"description": fmt.Sprintf("Create up to %d tasks in one atomic call (attributed to this agent) — the batch form of task_create, and the right tool whenever a plan produces more than one task. Nothing is written unless every item is valid. Every item declares completionCriterion explicitly; EVIDENCE_JUDGMENT is available but never inferred from omission, and a verifier relation (including verifiesRef), executable pair, or completion policy does not replace that declaration. An item declaring VERIFICATION with completionPolicy VERIFICATION_PASSED is a pure gate: it is NOT dispatchable — a manual execute is refused with 409 and auto-dispatch passes over it — and the server never files its verifier for you, so `verifiesRef` from a later item of this same call is what pairs it; unpaired, that row can never complete. Items are created in order, and a later item can depend on an earlier one WITHOUT knowing its id: give the earlier item a `ref` and list that ref in the later item's `dependsOnRefs` (e.g. [{ref:\"s0\",…},{ref:\"s1\",dependsOnRefs:[\"s0\"]}]). `dependsOnTaskIds` still takes ids of tasks that already exist; the two combine. Each item takes the same fields as task_create, `projectId` and `acceptanceCriteria` included, so a plan for one project files every task it produces under that project in the same call, each carrying what would settle it. Ask, per item, what would settle it — of the person you are working with, in the conversation, before writing the batch — and put their answer in that item's `acceptanceCriteria`: an item's criteria are theirs to state, and the card this call raises is a yes/no on the plan rather than the conversation where they are agreed. Where a single command decides it — a test suite, a build, a lint — give the item `acceptanceCommand` with `acceptanceExpectedExitCode` too, and the item settles itself from the exit code instead of waiting on a person to read its evidence. An item filed with neither leaves its session no legal way to finish, and it ends at ATTEMPT_ENDED_WITHOUT_JUDGMENT_PATH — so a plan that skips this question is a plan that comes back to a human one task at a time. Items nest the same way they order: `parentRef` names an EARLIER item's ref as this item's PARENT, so a plan lands as a tree — its steps written as parts of the piece of work being planned, in the same call that creates it. `parentTaskId` stays for hanging items under a task that ALREADY exists (same project as the item); one item cannot carry both. The two ref fields answer different questions: `dependsOnRefs` is when an item may run, parentRef is what it is a part of. Returns the created tasks in input order, each echoing its `ref`. Tasks are only recorded — call task_start for one that should run now, or give an item runAt to have the server start it at a set time. This BLOCKS until a human approves the batch: they see what would be created and, above all, how many of it starts running within the minute. Send the batch you actually mean — a rejected one costs the whole call, not one item.", maxTaskBatchCreate),
			"inputSchema": obj(map[string]interface{}{
		...
		tools = append(tools,
			map[string]interface{}{
				...
				"inputSchema": obj(map[string]interface{}{
					...
					"wait": map[string]interface{}{"type": "boolean", "description": "Wait for the new session's first turn to settle and return its full state inline: up to about 10 minutes, less when this session was itself spawned by a wait. The wait is recorded on Orbit's server as a watch before it starts, so when that time runs out or the connection drops, the call returns the session as last seen with the watch under \"watch\", and the watch wakes this session when the turn settles; end your turn then instead of polling. If the server does not record the watch, the result says no server-held watch backs the wait, and a refused watch returns at once. Default false: returns immediately."},
				}, "prompt"),
			},
			map[string]interface{}{
				"name":        "session_list",
