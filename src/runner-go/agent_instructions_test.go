...
func TestAppendClaudeAgentInstructionArgsAddsOnlyAbsolutePhase1Rules(t *testing.T) {
	...
	wantAllowed := append([]string{"Read", "mcp__orbit__*"}, orbitCLIAllowedTools(exe)...)
	want := []string{
		"-p",
...
func TestAppendClaudeAgentInstructionArgsFailsClosedWithoutSafeExecutable(t *testing.T) {
	...
	if got := appendClaudeAgentInstructionArgs([]string{"-p"}, agent, ""); !reflect.DeepEqual(got, want) {
	...
	if got := appendClaudeAgentInstructionArgs([]string{"-p"}, AgentExecConfig{}, ""); !reflect.DeepEqual(got, []string{"-p"}) {
...
func TestOrbitCLIAllowedToolsRejectsPolicyMetacharacters(t *testing.T) {
	...
	if got := orbitCLIAllowedTools(""); len(got) != 0 {
	...
	spaced := orbitCLIAllowedTools("/opt/orbit runner/bin/orbit")
