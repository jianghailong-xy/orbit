...
func TestCreateBatchDryRunAsksForAPlanAndNotAWrite(t *testing.T) {
	...
	var out bytes.Buffer
	if err := cmdTaskCLI(
		[]string{"create-batch", "--tasks", `[{"title":"a","projectId":"proj-1","completionCriterion":"EVIDENCE_JUDGMENT"}]`, "--dry-run", "--json"},
...
func TestCreateBatchWithoutDryRunSendsNoSuchFlag(t *testing.T) {
	...
	var out bytes.Buffer
	if err := cmdTaskCLI(
		[]string{"create-batch", "--tasks", `[{"title":"a","completionCriterion":"EVIDENCE_JUDGMENT"}]`, "--json"}, strings.NewReader(""), &out,
	); err != nil {
		t.Fatal(err)
	}
	var body map[string]interface{}
