...
--handoff-reason declares that this create crosses OUT of the project this session coordinates and
into the one --project-id names. Three things about it. It is meaningless on its own: a crossing has
to name where it is going, so passing it without --project-id is refused here, before any request.
It carries NO authority — it does not grant the crossing, it makes it askable, and the server files
the question against the crossing itself and refuses this write until somebody answers. And that
somebody is the ACCOUNT OWNER (or their standing policy when both projects are on AUTO): no agent
and no coordinator of either project can answer it, so read the row back with 'orbit project
crossings PROJECT_ID' and send this create again once it says APPROVED. Undeclared, the same write
is refused PROJECT_SCOPE_MISMATCH and retrying it changes nothing — which is the point, since "I did
not realise it was another project" and "I am asking to move work between two goals" must not arrive
as the same request. The account owner never needs it: §4 R1 exempts them from this contract.
...
--run-at schedules a one-time start: an ISO 8601 date-time (e.g. 2026-09-21T12:34:00Z) at or after
which the server starts the task by itself. It is not a deadline — --due-date is a date nothing
dispatches on. The server scans for due schedules once a minute and starts each task the way
'orbit task start' would, whether or not --auto-run-when-ready is set, and the run it starts clears
the schedule, so it fires once. A due task still starts only while it is OPEN, not held
(dispatchHold), with an assignee bound to a runner, every prerequisite satisfied and no session
already occupying it; until then the schedule is kept and every later scan tries again. A time
already past is started by the next scan. 'orbit task start' ignores the schedule and starts the
task now.
`,
...
"runAt" is an item's one-time scheduled start, an ISO 8601 date-time, with the rules --run-at has
on 'orbit task create': the server's once-a-minute scan starts that task once the time has come,
and the run it starts clears it.
...
item WOULD land (project id, title, status), "findings": every check
...
One read, four answers:

  owning       the project this work COUNTS TOWARDS — its title, its Base62 id and its
               status. The only authoritative attribution there is
...
Every absent fact is null beside a reason — "nothing is holding this up" and "this build cannot
tell you" are different answers, and a missing field says neither.

The acceptance lane this read used to carry — which of the project's stated criteria cite the task,
and what each was judged to be — went with migration 0229. The criteria are still stated; nothing
judges them.
...
Options:
  ...
  --criterion-key KEY         Declare, or correct, which of the PROJECT's stated acceptance
                              criteria this task serves (a key from project_get). Re-sending
                              the same key re-records that criterion's CURRENT revision, which
                              is how work declared against wording that has since moved is
  ...
  --auto-run-when-ready[=BOOL]
  --priority N | --clear-priority
                              Where this task stands in its list's queue: when the list has more
                              ready tasks than free slots, higher is dispatched first. 0 is the
                              default, and --clear-priority returns the task to it
  --completion-policy MANUAL|ALL_CHILDREN_DONE|VERIFICATION_PASSED
...
--run-at sets or moves the task's one-time scheduled start and --clear-run-at cancels it; passing
neither leaves the schedule as it is. The schedule means what it means on 'orbit task create': not
a deadline like --due-date, but the instant after which the server's once-a-minute scan starts the
task by itself, whether or not --auto-run-when-ready is set (a time already past, on the next scan),
and only while the task is OPEN, not held, with an assignee bound to a runner, its prerequisites
satisfied and no session occupying it. The run it starts clears the schedule.

--priority orders this task within its list's queue. When the list has more ready tasks than free
slots (its max concurrency), the server's automatic dispatch — the once-a-minute scan, and the
start that follows a prerequisite finishing — gives the next slot to the highest priority. Equal
priorities keep the order they already had, so a task nobody raises is dispatched as it always
was, and a negative value goes after everything left at 0. It starts nothing by itself and gets
past no gate: a paused list, unfinished prerequisites, a future --run-at and the list's cap all
still apply, and 'orbit task start' ignores it. --clear-priority returns the task to 0.
...
func cmdTaskCLI(args []string, in io.Reader, out io.Writer) error {
	...
	case "reopen":
		return cliTaskReopen(args[1:], out)
	case "delete":
...
func cliTaskCreate(args []string, in io.Reader, out io.Writer) error {
	...
	// nothing in front of them. The server's rule that it must name a destination is repeated
	// below, once the whole body is built.
	if flagWasSet(fs, "handoff-reason") {
		if strings.TrimSpace(*handoffReason) == "" {
			return fmt.Errorf("--handoff-reason cannot be blank: it is what whoever answers the crossing reads")
		}
		body["handoff"] = map[string]interface{}{"reason": *handoffReason}
	}
	...
	// A trigger, not a deadline like --due-date: the server's once-a-minute sweep starts the task once
	// this instant has come, and the run it starts clears it.
	if flagWasSet(fs, "run-at") {
		if strings.TrimSpace(*runAt) == "" {
			return fmt.Errorf("--run-at cannot be empty")
		}
		body["runAt"] = *runAt
	}
	...
	if err != nil {
		// Verbatim, plus how to ask when the refusal is the one whose own requiredAction offers a
		// handoff as the way through.
		return fmt.Errorf("create task: %w%s", err, crossProjectCrossingGuidance(err))
	}
	return writeCLIRawJSON(out, raw, *jsonOut)
}
...
func cliTaskCreateBatch(args []string, in io.Reader, out io.Writer) error {
	...
	if err != nil {
		return fmt.Errorf("%s: %w%s", verb, err, crossProjectCrossingGuidance(err))
	}
...
func parseTaskBatchItems(raw string) ([]map[string]interface{}, error) {
	...
	for i, item := range items {
		...
		if err := requireHandoffNamesItsDestination(item); err != nil {
			return nil, fmt.Errorf("tasks[%d]: %w", i, err)
		}
	}
	return items, nil
...
func cliTaskUpdate(args []string, in io.Reader, out io.Writer) error {
	...
	autoRun := fs.Bool("auto-run-when-ready", false, "auto-run after dependencies complete")
	priority := fs.Int("priority", 0, "where this task stands in its list's queue: higher is dispatched first")
	clearPriority := fs.Bool("clear-priority", false, "return this task's priority to the default, 0")
	completionPolicy := fs.String("completion-policy", "", "how this task's completion is decided (MANUAL|ALL_CHILDREN_DONE|VERIFICATION_PASSED)")
	...
	if *clearRunAt && flagWasSet(fs, "run-at") {
		return fmt.Errorf("--clear-run-at and --run-at cannot be used together")
	}
	if *clearPriority && flagWasSet(fs, "priority") {
		return fmt.Errorf("--clear-priority and --priority cannot be used together")
	}
	...
	if *clearCriterionKey && flagWasSet(fs, "criterion-key") {
		return fmt.Errorf("--clear-criterion-key and --criterion-key cannot be used together")
	}
	...
	// Three-state like the id flags around it, and the only one of them whose value half crosses a
	// line somebody else guards: taking the task OUT of a project is always the caller's to do, but
	// filing it INTO another one is a cross-project crossing the account owner answers. Both
	// spellings are sent as given and the server's refusal is passed back verbatim — a client that
	// pre-judged which moves are allowed would be a second, weaker copy of that boundary.
	if *noProject {
		body["projectId"] = nil
	} else if flagWasSet(fs, "project") {
		if strings.TrimSpace(*projectID) == "" {
	...
	// this client's to pre-empt — and refused below if it named no destination.
	if flagWasSet(fs, "handoff-reason") {
		if strings.TrimSpace(*handoffReason) == "" {
			return fmt.Errorf("--handoff-reason cannot be blank: it is what whoever answers the crossing reads")
		}
	...
	// Three-state like --due-date above: --clear-run-at cancels the schedule, --run-at sets or moves
	// it, and neither leaves it as it is.
	if *clearRunAt {
		body["runAt"] = nil
	} else if flagWasSet(fs, "run-at") {
		if strings.TrimSpace(*runAt) == "" {
	...
	// Three-state like the id flags above, and re-sending the SAME key is not a no-op: the server
	// re-records the criterion's current revision, which is how work declared against wording that
	// has since moved is brought back up to date.
	if *clearCriterionKey {
		body["criterionKey"] = nil
	} else if flagWasSet(fs, "criterion-key") {
		if strings.TrimSpace(*criterionKey) == "" {
			return fmt.Errorf("--criterion-key cannot be empty; use --clear-criterion-key")
		}
		body["criterionKey"] = *criterionKey
	}
	if flagWasSet(fs, "completion-criterion") {
	...
	// Three-state like --run-at above: --clear-priority sends null, which the server stores as the
	// default 0, --priority a number, and neither leaves it as it is. A number past the column's range
	// is refused here, where the flag can be named, rather than coming back as the server's 400.
	if *clearPriority {
		body["priority"] = nil
	} else if flagWasSet(fs, "priority") {
		if *priority < taskPriorityMin || *priority > taskPriorityMax {
			return fmt.Errorf("--priority must be between %d and %d", taskPriorityMin, taskPriorityMax)
		}
	...
	if err != nil {
		// The server's refusal, exactly as it came back, plus the one thing it cannot say about
		// itself: where to read the crossing it names, or — one refusal earlier — how to declare one.
		return fmt.Errorf("update task: %w%s", err, crossProjectCrossingGuidance(err))
	}
	return writeCLIRawJSON(out, raw, *jsonOut)
}
