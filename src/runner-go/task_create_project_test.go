...
func TestTaskCLICreateSendsProjectID(t *testing.T) {
	...
	err := cmdTaskCLI([]string{"create", "--title", "Ship it", "--project-id", "proj-1", "--json"},
...
func TestTaskCLICreateOmitsProjectIDWhenNotAsked(t *testing.T) {
	...
	var out bytes.Buffer
	if err := cmdTaskCLI([]string{"create", "--title", "Ship it", "--json"}, strings.NewReader(""), &out); err != nil {
		t.Fatal(err)
	}
...
func TestMCPTaskCreateBatchSendsEveryItemsProjectID(t *testing.T) {
	...
	res := mcp.callTool("task_create_batch", map[string]interface{}{
		"tasks": []interface{}{
			map[string]interface{}{"title": "first", "projectId": "proj-1", "ref": "s0"},
			map[string]interface{}{"title": "second", "projectId": "proj-1", "dependsOnRefs": []interface{}{"s0"}},
		},
	})
	if res["isError"] == true {
