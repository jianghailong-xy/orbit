...
// to exercise every local path. The claim names are the ones ServiceTokenAuthorizer signs: a
// fixture that kept the pre-rename `agentId` is how `orbit capabilities` stopped reporting the
// pin while every test here stayed green.
func fakeServiceToken(t *testing.T, scopes []string, workspaceID string, expiresIn time.Duration) string {
	t.Helper()
...
func TestTokenCLIMintValidatesScopesTTLAndAgentPin(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "create without workspace pin",
			args:      []string{"mint", "--scope", "session:create"},
			wantError: "--workspace-id is required with the session:create scope",
		},
		{
			name:      "pin under both names",
			args:      []string{"mint", "--scope", "session:get", "--workspace-id", "w-1", "--agent-id", "w-2"},
			wantError: "--workspace-id and --agent-id are the same flag",
		},
		{
			name:      "ttl too long",
...
func TestTokenCLIMintSendsScopesAndPrintsTokenOnce(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"id":"token-1","token":"minted.jwt.value","scopes":["session:get","session:create"],"workspaceId":"workspace-1","expiresAt":"2026-08-05T00:00:00.000Z"}`))
	}))
	defer srv.Close()
	configureCLITestRunner(t, srv.URL)

	// --agent-id is what 0.1.171's help and every script written against it say.
	for _, pinFlag := range []string{"--workspace-id", "--agent-id"} {
		var out bytes.Buffer
		args := []string{"mint", "--scope", "session:get,session:create", pinFlag, "workspace-1", "--ttl", "2h", "--label", "feishu bridge"}
		if err := cmdTokenCLI(args, &out); err != nil {
			t.Fatalf("%s: %v", pinFlag, err)
		}
		...
		// The pin travels under the name the control plane reads, whichever flag named it.
		if body["workspaceId"] != "workspace-1" || body["label"] != "feishu bridge" {
			t.Errorf("%s: body = %#v", pinFlag, body)
		}
		if _, stale := body["agentId"]; stale {
			t.Errorf("%s: body still carries the pre-rename agentId: %#v", pinFlag, body)
		}
		if body["ttlSeconds"] != float64(2*60*60) {
			t.Errorf("ttlSeconds = %#v, want 7200", body["ttlSeconds"])
		}
		printed := out.String()
		for _, want := range []string{"workspace: workspace-1", "minted.jwt.value", "Shown once", envServiceToken, "orbit token revoke token-1"} {
...
func TestServiceTokenClaimsAreReadLocallyForDiscoveryOnly(t *testing.T) {
	valid := fakeServiceToken(t, []string{"session:get", "session:create", "session:end"}, "workspace-1", time.Hour)
	...
	if claims.WorkspaceID != "workspace-1" {
		t.Errorf("workspaceId = %q", claims.WorkspaceID)
	}
