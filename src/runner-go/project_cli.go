...
an edit that drops a criterion, or whose direction cannot be read at all, is NOT applied — it is
held as a proposal for the account owner to decide, and the response says so in
acceptanceCriteriaHold, beside the criteria that are still the ones in force. When it does land,
the set you send becomes the standard this project states. Preserve ids returned by
