...
exports.TasksService = exports.ACTIVE_TASK_STRIP_MAX = exports.TASK_LABEL_SUMMARY_MAX = exports.BATCH_EXECUTE_DISPATCH_CONCURRENCY = exports.MAX_TASK_PAGE_SIZE = exports.DEFAULT_TASK_PAGE_SIZE = exports.MAX_CONSECUTIVE_FOREMEN = exports.FOREMAN_RETRY_BACKOFF_MS = exports.MAX_VERIFICATIONS_PER_TASK = exports.SCHEDULED_DISPATCH_MAX_PER_SWEEP = exports.TASK_LIST_SELECT = exports.TASK_COMMENT_MENTION_DELIVERY_VERSION = exports.QUOTA_BLIND_RETRY_BACKOFF_MS = exports.MAX_AUTO_RUN_FAILURES = exports.AUTO_RUN_RETRY_BACKOFF_MS = void 0;
exports.buildTaskExecutionPrompt = buildTaskExecutionPrompt;
exports.normalizeTaskLabels = normalizeTaskLabels;
exports.parseLabelsQuery = parseLabelsQuery;
exports.autoDispatchStillValid = autoDispatchStillValid;
exports.takeBudget = takeBudget;
exports.diskBelowFloor = diskBelowFloor;
exports.encodeTaskPageCursor = encodeTaskPageCursor;
exports.decodeTaskPageCursor = decodeTaskPageCursor;
...
const workspace_provider_1 = require("../workspaces/workspace-provider");
const task_aggregation_1 = require("../projects/task-aggregation");
const task_aggregation_writer_1 = require("../projects/task-aggregation-writer");
const prisma_service_1 = require("../prisma/prisma.service");
const realtime_service_1 = require("../realtime/realtime.service");
const sessions_service_1 = require("../sessions/sessions.service");
const engine_signin_preflight_1 = require("../sessions/engine-signin-preflight");
const session_state_1 = require("../sessions/session-state");
const dto_1 = require("./dto");
const task_supersession_1 = require("./task-supersession");
const task_lock_order_1 = require("./task-lock-order");
const project_coordinator_session_1 = require("../projects/project-coordinator-session");
const task_retry_policy_1 = require("./task-retry-policy");
var task_retry_policy_2 = require("./task-retry-policy");
Object.defineProperty(exports, "AUTO_RUN_RETRY_BACKOFF_MS", { enumerable: true, get: function () { return task_retry_policy_2.AUTO_RUN_RETRY_BACKOFF_MS; } });
Object.defineProperty(exports, "MAX_AUTO_RUN_FAILURES", { enumerable: true, get: function () { return task_retry_policy_2.MAX_AUTO_RUN_FAILURES; } });
Object.defineProperty(exports, "QUOTA_BLIND_RETRY_BACKOFF_MS", { enumerable: true, get: function () { return task_retry_policy_2.QUOTA_BLIND_RETRY_BACKOFF_MS; } });
const reclaim_stalled_task_1 = require("./reclaim-stalled-task");
const task_dependencies_1 = require("./task-dependencies");
const verification_epoch_read_1 = require("./verification-epoch-read");
const task_dag_1 = require("./task-dag");
...
const SINGLE_RUN_DEDUP = [client_1.RunStatus.PENDING, client_1.RunStatus.RUNNING];
/**
 ...
 */
exports.TASK_COMMENT_MENTION_DELIVERY_VERSION = 1;
/**
...
class MentionUndeliverable extends Error {
    ...
    availability;
    requiredAction;
    constructor(code, message, availability = false, 
...
function isTaskWorkFence(error) {
    return (0, task_supersession_1.taskFenceConflictMessage)(error) != null;
}
...
const RUNNABLE_TASK_SQL = client_1.Prisma.sql `
  ...
  AND ${client_1.Prisma.raw((0, task_dependencies_1.dependenciesSatisfiedSql)('t'))}
  AND ${client_1.Prisma.raw((0, task_supersession_1.taskNotObsoleteSql)('t'))}`;
/**
 ...
 */
const AUTO_RUN_READY_SQL = client_1.Prisma.sql `
  ...
  AND NOT EXISTS (
    ...
    WHERE s.task_id = t.id
      AND s.status IN (${client_1.Prisma.join(reclaim_stalled_task_1.TASK_OCCUPYING.map((status) => client_1.Prisma.sql `${status}::run_status`), ', ')})
  )`;
/**
 ...
 */
const SCHEDULED_DUE_SQL = client_1.Prisma.sql `
 ...
 */
function taskScopeSql(scope) {
    const clauses = [client_1.Prisma.sql `t.owner_id = ${scope.ownerId}::uuid`];
    if (scope.listId === null)
        clauses.push(client_1.Prisma.sql `t.list_id IS NULL`);
    else if (typeof scope.listId === 'string') {
        clauses.push(client_1.Prisma.sql `t.list_id = ${scope.listId}::uuid`);
    }
    if (typeof scope.projectId === 'string') {
        clauses.push(client_1.Prisma.sql `t.project_id = ${scope.projectId}::uuid`);
    }
    if (typeof scope.assigneeId === 'string') {
        clauses.push(client_1.Prisma.sql `t.assignee_id = ${scope.assigneeId}::uuid`);
    }
    ...
    if (labels && typeof labels === 'object' && 'hasEvery' in labels) {
        const wanted = labels.hasEvery;
        if (wanted.length)
            clauses.push(client_1.Prisma.sql `t.labels @> ARRAY[${client_1.Prisma.join(wanted)}]::text[]`);
    }
...
exports.SCHEDULED_DISPATCH_MAX_PER_SWEEP = 200;
/**
 ...
 */
function autoDispatchStillValid(current, observed, now = Date.now()) {
    if ((current?.getTime() ?? null) !== (observed?.getTime() ?? null))
        return false;
    if (current !== null && current.getTime() > now)
        return false;
    return true;
}
...
exports.MAX_VERIFICATIONS_PER_TASK = 2;
/**
 ...
 */
function takeBudget(budget, runnerId, listId) {
    const runnerLeft = budget.runner.get(runnerId);
    const listLeft = listId ? budget.list.get(listId) : undefined;
    if (runnerLeft !== undefined && runnerLeft <= 0)
        return false;
    if (listLeft !== undefined && listLeft <= 0)
        return false;
    if (runnerLeft !== undefined)
        budget.runner.set(runnerId, runnerLeft - 1);
    if (listLeft !== undefined && listId)
        budget.list.set(listId, listLeft - 1);
    return true;
}
exports.FOREMAN_RETRY_BACKOFF_MS = [30 * 60_000, 2 * 60 * 60_000, 8 * 60 * 60_000];
exports.MAX_CONSECUTIVE_FOREMEN = exports.FOREMAN_RETRY_BACKOFF_MS.length + 1;
/**
 ...
 */
function diskBelowFloor(freeBytes, minFreeDiskMb) {
    if (freeBytes == null)
        return false;
    if (minFreeDiskMb == null || !Number.isFinite(minFreeDiskMb) || minFreeDiskMb <= 0)
        return false;
...
exports.DEFAULT_TASK_PAGE_SIZE = 100;
exports.MAX_TASK_PAGE_SIZE = 200;
 ...
 * the better answer.
 */
exports.ACTIVE_TASK_STRIP_MAX = 50;
...
function encodeTaskPageCursor(task) {
    return Buffer.from(JSON.stringify({ createdAt: task.createdAt.toISOString(), id: task.id })).toString('base64url');
}
function decodeTaskPageCursor(cursor) {
    try {
        const parsed = JSON.parse(Buffer.from(cursor, 'base64url').toString('utf8'));
        const createdAt = new Date(parsed.createdAt);
        if (!UUID_RE.test(parsed.id) || Number.isNaN(createdAt.getTime()))
            throw new Error('invalid');
        return { createdAt, id: parsed.id };
    }
    catch {
        throw new common_1.BadRequestException('invalid task cursor');
    }
}
function dependencyGraphOwnerScope(ownerId) {
    return (0, crypto_1.createHash)('sha256').update(`dependency-graph-owner:${ownerId}`).digest('base64url');
}
function encodeDependencyGraphBranchCursor(ownerId, focusTaskId, anchorTaskId, direction) {
    const cursor = {
        version: 1,
...
function decodeDependencyGraphBranchCursor(cursor) {
    try {
        const parsed = JSON.parse(Buffer.from(cursor, 'base64url').toString('utf8'));
        if (parsed.version !== 1 ||
        ...
        throw new common_1.BadRequestException('invalid dependency graph expansion cursor');
    }
}
function dependencyGraphLimit(value, fallback, name, maximum) {
    if (value === undefined)
        return fallback;
    const parsed = Number(value);
    if (!Number.isInteger(parsed) || parsed < 1 || parsed > maximum) {
        throw new common_1.BadRequestException(`${name} must be an integer from 1 to ${maximum}`);
    }
    return parsed;
}
function dependencyGraphExpansionLimit(value) {
    if (value === undefined)
        return DEFAULT_DEPENDENCY_GRAPH_EXPANSION_BATCH_SIZE;
    const parsed = Number(value);
    if (!Number.isInteger(parsed) || parsed < 1 || parsed > MAX_DEPENDENCY_GRAPH_EXPANSION_BATCH_SIZE) {
        throw new common_1.BadRequestException(`limit must be an integer from 1 to ${MAX_DEPENDENCY_GRAPH_EXPANSION_BATCH_SIZE}`);
    }
    return parsed;
}
function dependencyGraphBoolean(value, name) {
    if (value === undefined || value === false || value === 'false')
        return false;
    if (value === true || value === 'true')
        return true;
    throw new common_1.BadRequestException(`${name} must be true or false`);
}
let TasksService = TasksService_1 = class TasksService {
    ...
    onModuleInit() {
        ...
        this.reconcileTimer = setInterval(() => {
            ...
            this.reconcileReadyTasks()
                .catch((e) => this.logger.error(`reconcile sweep failed: ${e instanceof Error ? e.message : e}`))
                ...
                .catch((e) => this.logger.error(`scheduled sweep failed: ${e instanceof Error ? e.message : e}`))
                .then(() => this.dispatchStalledListForemen())
                .catch((e) => this.logger.error(`foreman sweep failed: ${e instanceof Error ? e.message : e}`))
                ...
                .catch((e) => this.logger.error(`mention delivery sweep failed: ${e instanceof Error ? e.message : e}`));
     ...
     */
    async assertOwnedWorkspace(ownerId, workspaceId) {
        if (!workspaceId)
            return;
     ...
     */
    async assertUsableProvider(ownerId, provider) {
        if (!provider)
            return;
        if (Object.values(shared_1.AgentProvider).includes(provider))
            return;
        ...
        if (!configured)
            throw new common_1.BadRequestException('provider not available');
    }
    /** A task may only be filed under a list the same user owns (cf. assertOwnedWorkspace). */
    async assertOwnedList(ownerId, listId) {
        if (!listId)
            return;
        ...
        if (!list)
            throw new common_1.ForbiddenException('task list not found');
    }
    /** A task may only be filed under a project the same user owns (cf. assertOwnedList). */
    async assertOwnedProject(ownerId, projectId) {
        if (!projectId)
            return;
     ...
     */
    async translateProjectFk(e, ownerId, projectIds) {
        const raced = e instanceof client_1.Prisma.PrismaClientKnownRequestError &&
            (e.code === 'P2003' || e.code === 'P2025');
        if (!raced)
            return e;
        const named = [...new Set(projectIds.filter((id) => !!id))];
        if (named.length === 0)
            return e;
        const live = await this.prisma.project.count({ where: { id: { in: named }, ownerId } });
        if (live === named.length)
            return e;
        ...
        return new common_1.ForbiddenException('project not found');
    }
    /**
     ...
     */
    async assertParentEligible(db, ownerId, parentTaskId, projectId) {
        ...
        if ((parent.projectId ?? null) !== projectId) {
            throw new common_1.BadRequestException('A subtask must be in the same project as its parent task — file both under the same ' +
     ...
     */
    async assertVerificationEligible(db, ownerId, verifierTaskId, verifiesTaskId, projectId) {
        if (verifierTaskId && verifiesTaskId === verifierTaskId) {
            throw new common_1.BadRequestException('A task cannot verify itself');
        }
        ...
        if (!subject)
            throw new common_1.NotFoundException('verified task not found');
        if (subject.verifiesTaskId) {
            throw new common_1.BadRequestException('That task is itself a verification — a check of a check has nothing left to verify');
        }
        if ((subject.projectId ?? null) !== projectId) {
            throw new common_1.BadRequestException('A verification must be in the same project as the task it verifies — file both under ' +
     ...
     */
    async recomputeAggregates(ownerId, seedTaskIds) {
        const seeds = [...new Set(seedTaskIds.filter((id) => !!id))];
        if (!seeds.length)
            return [];
        const scope = await (0, task_aggregation_writer_1.collectAggregationScope)(this.prisma, ownerId, seeds);
        if (scope.truncated) {
            this.logger.warn(`aggregation skipped for ${seeds.length} task(s): scope exceeded ` +
                `${task_aggregation_writer_1.AGGREGATION_SCOPE_MAX_TASKS} tasks`);
            return [];
        }
        const plan = (0, task_aggregation_1.planTaskAggregation)(scope.facts);
        if (plan.cycleTaskIds.length) {
            ...
            // had nothing to do.
            this.logger.warn(`aggregation skipped: ${plan.cycleTaskIds.length} task(s) lie on a parentTaskId cycle`);
            return [];
        }
        if (!plan.aggregations.length)
            return [];
        const moved = await (0, task_aggregation_writer_1.applyTaskAggregations)(this.prisma, ownerId, plan.aggregations, new Date());
        for (const taskId of moved) {
            this.realtime.publishForUser(ownerId, shared_1.RunEventType.TASK_CHANGED, taskId);
        }
        return moved;
    }
    /**
     ...
     */
    async assertNoParentCycle(db, taskId, parentTaskId) {
        const seen = new Set();
        let cursor = parentTaskId;
        for (let depth = 0; depth < MAX_TASK_PARENT_DEPTH && cursor; depth += 1) {
            if (cursor === taskId) {
                throw new common_1.BadRequestException('That parent task is already one of this task’s subtasks');
            }
            if (seen.has(cursor))
                return;
            seen.add(cursor);
            const above = await db.task.findUnique({
                where: { id: cursor },
        ...
        if (cursor) {
            throw new common_1.BadRequestException(`Subtask chains are limited to ${MAX_TASK_PARENT_DEPTH} levels`);
        }
    }
    /**
     ...
     */
    async assertHierarchyConsistent(db, ownerId, taskId, current, dto) {
        ...
        const parentTaskId = dto.parentTaskId === undefined ? current.parentTaskId : (dto.parentTaskId ?? null);
        const verifiesTaskId = dto.verifiesTaskId === undefined ? current.verifiesTaskId : (dto.verifiesTaskId ?? null);
        if (parentTaskId) {
            if (parentTaskId === taskId) {
                throw new common_1.BadRequestException('A task cannot be its own parent');
            }
        ...
        if (projectId !== current.projectId) {
            ...
            if (subtasks > 0) {
                throw new common_1.BadRequestException(`This task has ${subtasks} subtask(s) that would be left in a different project — ` +
            ...
            if (checks > 0) {
                throw new common_1.BadRequestException(`This task has ${checks} verification(s) that would be left in a different project — ` +
                    'move or detach them before moving it');
            }
        }
    }
    /** Assert every id is a task this user owns (dependency endpoints both sides). */
    async assertOwnedTasks(ownerId, ids) {
     ...
     */
    async lockDependencyGraph(tx, ownerId) {
        await (0, lock_order_1.lockOwnerTaskGraph)(tx, ownerId);
    }
    /**
     ...
     */
    async preLockCreatorSessions(tx, sessionIds, taskIds = []) {
        const fromTasks = await (0, lock_order_1.creatorSessionsOf)(tx, taskIds);
        await (0, lock_order_1.lockCreatorSessions)(tx, [...sessionIds, ...fromTasks]);
    }
    /**
     ...
     */
    async dependencyStatesFor(taskIds) {
        return new Map([...await this.dependencyFactsFor(taskIds)]
            .map(([taskId, facts]) => [taskId, (0, task_dependencies_1.computeDependencyState)(facts)]));
    }
    /**
     ...
     */
    async dependencyFactsFor(taskIds) {
        ...
        const epochs = await (0, verification_epoch_read_1.loadVerificationEpochGates)(this.prisma, [...byTask.values()].flat().map((entry) => entry.id));
        ...
        const dependents = epochs.size === 0 ? new Map()
        ...
        return new Map([...byTask].map(([taskId, entries]) => [taskId, entries.map((entry) => ({
                status: entry.status,
                verificationGate: (0, task_dependencies_1.dependencyEpochGate)(entry.id, epochs, dependents.get(taskId)),
            }))]));
    }
    /**
     ...
     */
    async dependencyStatesForGraph(ownerId, taskIds) {
        if (taskIds.length === 0)
            return new Map();
        const scoped = {
            taskId: { in: taskIds },
        ...
        const [totals, completed, failed] = await Promise.all([
            ...
            this.prisma.taskDependency.groupBy({
                by: ['taskId'],
                where: { ...scoped, dependsOnTask: { ownerId, status: shared_1.TaskStatus.DONE } },
                _count: { _all: true },
            }),
            this.prisma.taskDependency.groupBy({
                by: ['taskId'],
                where: {
                    ...
                    dependsOnTask: {
                        ownerId,
                        status: { in: [shared_1.TaskStatus.FAILED, shared_1.TaskStatus.CANCELLED] },
                    },
                },
                _count: { _all: true },
            }),
        ...
        for (const row of totals) {
            ...
            states.set(row.taskId, failedCount > 0
                ? 'BLOCKED_FAILED'
     ...
     */
    async verificationBlockedCounts(ownerId, taskIds) {
        if (taskIds.length === 0)
            return new Map();
        ...
        if (edges.length === 0)
            return new Map();
        const epochs = await (0, verification_epoch_read_1.loadVerificationEpochGates)(this.prisma, edges.map((edge) => edge.dependsOnTaskId));
        const counts = new Map();
        for (const edge of edges) {
            const gate = (0, task_dependencies_1.dependencyEpochGate)(edge.dependsOnTaskId, epochs, edge.task?.verifiesTaskId ?? null);
            if (gate == null)
                continue;
     ...
     */
    async dependencyConnectionCounts(ownerId, taskIds) {
        if (taskIds.length === 0)
            return new Map();
        const ownerScope = {
            task: { ownerId },
        ...
        const counts = new Map(taskIds.map((id) => [id, { prerequisiteCount: 0, dependentCount: 0 }]));
        for (const row of prerequisites)
            counts.get(row.taskId).prerequisiteCount = row._count._all;
    ...
    dependencyGraphCollapsedGroups(ownerId, focusTaskId, graphDirection, nodeIds, edges, connectionCounts) {
        const known = new Set(nodeIds);
        const loadedPrerequisites = new Map();
        const loadedDependents = new Map();
        for (const edge of edges) {
            if (!known.has(edge.sourceTaskId) || !known.has(edge.targetTaskId))
                continue;
            const prerequisites = loadedPrerequisites.get(edge.targetTaskId) ?? new Set();
            prerequisites.add(edge.sourceTaskId);
            loadedPrerequisites.set(edge.targetTaskId, prerequisites);
            const dependents = loadedDependents.get(edge.sourceTaskId) ?? new Set();
            dependents.add(edge.targetTaskId);
            loadedDependents.set(edge.sourceTaskId, dependents);
        }
        const directions = graphDirection === 'both' ? ['prerequisites', 'dependents'] : ['prerequisites'];
        ...
        for (const anchorTaskId of nodeIds) {
            ...
            for (const direction of directions) {
                const total = direction === 'prerequisites' ? counts.prerequisiteCount : counts.dependentCount;
                const loaded = direction === 'prerequisites'
     ...
     */
    async resolveAgentCreator(ownerId, workspaceId) {
        if (!workspaceId)
            return undefined;
        const workspace = await this.prisma.workspace.findFirst({
        ...
        if (!workspace)
            throw new common_1.ForbiddenException('workspace not found');
        return { type: client_1.CreatorType.AGENT, id: workspace.id };
    }
    /**
     ...
     */
    async assertSupersessionAlreadyRecorded(db, ownerId, predecessorId, successorId) {
        ...
        if (!predecessor)
            throw new common_1.NotFoundException('the task being superseded was not found');
        if (predecessor.supersededByTaskId === successorId
            && predecessor.terminalReason === 'SUPERSEDED') {
            return;
        }
        throw new common_1.ConflictException(`task ${(0, shared_1.uuidToBase62)(successorId)} already exists but does not supersede ` +
            `${(0, shared_1.uuidToBase62)(predecessorId)}` +
            (predecessor.supersededByTaskId
                ? ` — that attempt was superseded by ${(0, shared_1.uuidToBase62)(predecessor.supersededByTaskId)}`
     ...
     */
    async lockTaskForSupersessionWrite(tx, taskId) {
        try {
            ...
            if (!(0, task_supersession_1.isLockNotAvailable)(error))
                throw error;
            throw new common_1.ConflictException(`task ${(0, shared_1.uuidToBase62)(taskId)} is being written by a run starting on it right now, so its ` +
    ...
    async assertNotRunningBeforeRetiring(tx, ownerId, taskId) {
        const live = await tx.$queryRaw(client_1.Prisma.sql `
    ...
    `);
        if (live[0]) {
            ...
            throw new common_1.ConflictException(`task ${(0, shared_1.uuidToBase62)(taskId)} still has a live session (${(0, shared_1.uuidToBase62)(live[0].id)}, ` +
    ...
    async linkSupersededBy(tx, ownerId, predecessorId, successor, now) {
        ...
        const [predecessor] = await tx.$queryRaw(client_1.Prisma.sql `
    ...
    `);
        ...
        const refusal = (0, task_supersession_1.supersessionRefusal)({
        ...
        const already = (0, task_supersession_1.taskRetirement)(predecessor);
        if (already) {
            throw new common_1.ConflictException(already === 'SUPERSEDED' && predecessor.supersededByTaskId
                ? `task ${(0, shared_1.uuidToBase62)(predecessorId)} has already been superseded by ` +
                    `${(0, shared_1.uuidToBase62)(predecessor.supersededByTaskId)}`
                : `task ${(0, shared_1.uuidToBase62)(predecessorId)} is already ${already} — unlink it first if this ` +
      ...
      UPDATE "task"
       ...
       WHERE "id" = ${predecessorId}::uuid AND "owner_id" = ${ownerId}::uuid
        ...
        if (linked !== 1) {
            throw new common_1.ConflictException(`task ${(0, shared_1.uuidToBase62)(predecessorId)} was superseded by another attempt while this one was ` +
                'being created');
        }
    }
    /**
     ...
     */
    transientWriteRetry(operation, policy = {}) {
        return (0, transaction_retry_1.loggedRetry)(this.logger, operation, policy);
    }
    async create(ownerId, dto, creator, creatorSessionId) {
        if (!dto.title)
        ...
        const idempotencyKey = sessionId && turnId ? this.taskIdempotencyKey(sessionId, turnId, dto.title, dto.description) : undefined;
        if (idempotencyKey) {
            ...
            if (existing) {
                ...
                if (dto.supersedesTaskId) {
                    await this.assertSupersessionAlreadyRecorded(this.prisma, ownerId, dto.supersedesTaskId, existing.id);
                }
                return existing;
            }
        }
        ...
        const data = this.taskCreateData(ownerId, dto, creator, sessionId, idempotencyKey, await this.pausedListIds(ownerId, [dto.listId]));
        ...
        let task;
        try {
            task = await (0, transaction_retry_1.withTransactionRetry)(this.prisma, async (tx) => {
                ...
                await (0, lock_order_1.lockTaskLists)(tx, [dto.listId]);
              ...
              SELECT 1 FROM "project" WHERE "id" = ${dto.projectId}::uuid FOR NO KEY UPDATE`;
                ...
                if (dto.verifiesTaskId) {
                    await this.assertVerificationEligible(tx, ownerId, null, dto.verifiesTaskId, dto.projectId ?? null);
                }
                ...
                if (dto.supersedesTaskId) {
                    await this.linkSupersededBy(tx, ownerId, dto.supersedesTaskId, { id: created.id, projectId: created.projectId }, new Date());
                }
                return created;
            ...
            const fenced = (0, task_supersession_1.taskFenceConflictMessage)(e);
            if (fenced)
                throw new common_1.ConflictException(fenced);
        ...
        if (sessionId)
            this.realtime.publishTaskChanged(sessionId, task.id);
        ...
        if (task.parentTaskId || task.verifiesTaskId || dto.supersedesTaskId) {
            await this.recomputeAggregates(ownerId, [task.parentTaskId, task.verifiesTaskId, dto.supersedesTaskId].filter((id) => !!id)).catch((e) => this.logger.warn(`aggregation after create of ${task.id} failed: ${e?.message ?? e}`));
        }
        return task;
    }
    /**
     ...
     */
    async pausedListIds(ownerId, listIds) {
        const ids = [...new Set(listIds.filter((v) => !!v))];
        if (ids.length === 0)
            return new Set();
    ...
    taskCreateData(ownerId, dto, creator, sessionId, idempotencyKey, heldListIds) {
        return {
            ...
            creatorType: creator?.type ?? client_1.CreatorType.USER,
     ...
     */
    async assertBatchValid(ownerId, dto) {
        const items = dto.tasks ?? [];
        if (!items.length)
            throw new common_1.BadRequestException('tasks is required');
        if (items.length > dto_1.TASK_BATCH_CREATE_MAX)
            throw new common_1.BadRequestException(`at most ${dto_1.TASK_BATCH_CREATE_MAX} tasks per batch`);
        const positionByRef = new Map();
        items.forEach((item, index) => {
            if (!item.title)
                throw new common_1.BadRequestException(`tasks[${index}]: title is required`);
            if (item.ref === undefined)
                return;
            if (positionByRef.has(item.ref))
                throw new common_1.BadRequestException(`tasks[${index}]: duplicate ref "${item.ref}"`);
        ...
        items.forEach((item, index) => {
            for (const ref of item.dependsOnRefs ?? []) {
                ...
                if (position === undefined || position >= index)
                    throw new common_1.BadRequestException(`tasks[${index}]: dependsOnRefs "${ref}" must name an earlier task in this batch`);
            }
        });
        ...
        items.forEach((item, index) => {
            ...
            if (item.parentTaskId)
                throw new common_1.BadRequestException(`tasks[${index}]: parentRef and parentTaskId name two different parents — pass one`);
            const position = positionByRef.get(item.parentRef);
            if (position === undefined || position >= index)
                throw new common_1.BadRequestException(`tasks[${index}]: parentRef "${item.parentRef}" must name an earlier task in this batch`);
            ...
            if ((items[position].projectId ?? null) !== (item.projectId ?? null))
                throw new common_1.BadRequestException(`tasks[${index}]: a subtask must be in the same project as its parent task — file both ` +
        ...
        items.forEach((item, index) => {
            if (item.verifiesRef === undefined)
                return;
            if (item.verifiesTaskId)
                throw new common_1.BadRequestException(`tasks[${index}]: verifiesRef and verifiesTaskId name two different subjects — pass one`);
            const position = positionByRef.get(item.verifiesRef);
            if (position === undefined || position >= index)
                throw new common_1.BadRequestException(`tasks[${index}]: verifiesRef "${item.verifiesRef}" must name an earlier task in this batch`);
            const subject = items[position];
            if (subject.verifiesTaskId || subject.verifiesRef)
                throw new common_1.BadRequestException(`tasks[${index}]: that task is itself a verification — a check of a check has nothing ` +
                    'left to verify');
            if ((subject.projectId ?? null) !== (item.projectId ?? null))
                throw new common_1.BadRequestException(`tasks[${index}]: a verification must be in the same project as the task it verifies — ` +
        ...
        if (existingPrerequisites.length)
            await this.assertOwnedTasks(ownerId, existingPrerequisites);
        return items;
    }
    /**
     ...
     */
    async assertBatchHierarchy(db, ownerId, items) {
        for (const item of items) {
            if (item.parentTaskId) {
                await this.assertParentEligible(db, ownerId, item.parentTaskId, item.projectId ?? null);
            }
            if (item.verifiesTaskId) {
                await this.assertVerificationEligible(db, ownerId, null, item.verifiesTaskId, item.projectId ?? null);
            }
        }
    }
    /**
     ...
     */
    async previewCreateMany(ownerId, dto) {
        ...
        const assigneeIds = [...new Set(items.map((i) => i.assigneeId).filter((v) => !!v))];
        const externalIds = [...new Set(items.flatMap((i) => i.dependsOnTaskIds ?? []))];
        const listIds = [...new Set(items.map((i) => i.listId).filter((v) => !!v))];
        ...
        for (const item of items) {
            ...
            const waitsOnExisting = (item.dependsOnTaskIds ?? []).some((id) => statusOf.get(id) !== shared_1.TaskStatus.DONE);
            // What both sweeps require of a task before they will dispatch it at all.
            const runnable = !!item.assigneeId &&
            ...
            const hasPrerequisites = (item.dependsOnRefs?.length ?? 0) + (item.dependsOnTaskIds?.length ?? 0) > 0;
            if (!hasPrerequisites) {
                needsManualStart += 1;
                continue;
            }
            ...
            if (item.autoRunWhenReady !== false && runnable)
                startingNow += 1;
            else
                notDispatchable += 1;
        }
        return {
            ...
            tasks: items.slice(0, dto_1.DAG_PREVIEW_TITLES).map((item) => ({
                title: item.title,
            ...
            titlesTruncated: Math.max(0, items.length - dto_1.DAG_PREVIEW_TITLES),
        };
    }
    /**
     ...
     */
    async createMany(ownerId, dto, creator, creatorSessionId) {
        const items = await this.assertBatchValid(ownerId, dto);
        const sessionId = await this.resolveOwnedSession(ownerId, creatorSessionId);
        ...
        const created = await (0, transaction_retry_1.withTransactionRetry)(this.prisma, async (tx) => {
            ...
            await (0, lock_order_1.lockTaskLists)(tx, items.map((item) => item.listId));
            await this.preLockCreatorSessions(tx, [sessionId], items.map((item) => item.supersedesTaskId));
            ...
            if (hasSupersessions) {
                const projectIds = [...new Set(items
                        ...
                        .filter((projectId) => !!projectId))].sort();
            ...
            if (hasParents)
                await this.assertBatchHierarchy(tx, ownerId, items);
            ...
            for (const item of items) {
                ...
                const idempotencyKey = sessionId && turnId ? this.taskIdempotencyKey(sessionId, turnId, item.title, item.description) : undefined;
                ...
                // items.
                const row = item.parentRef === undefined && item.verifiesRef === undefined
                    ? item
                    : {
                        ...item,
                ...
                const task = existing ??
                    (await tx.task.create({
                        data: this.taskCreateData(ownerId, row, creator, sessionId, idempotencyKey, heldListIds),
                    }));
                if (item.ref !== undefined)
                    idByRef.set(item.ref, task.id);
                ...
                if (item.supersedesTaskId) {
                    if (existing) {
                        ...
                        await this.assertSupersessionAlreadyRecorded(tx, ownerId, item.supersedesTaskId, task.id);
                    }
                    else {
                        await this.linkSupersededBy(tx, ownerId, item.supersedesTaskId, { id: task.id, projectId: task.projectId }, new Date());
                    }
                }
            ...
            if (hasSupersessions && this.isDuplicateKey(e)) {
                ...
                for (const item of items) {
                    ...
                    if (!winner) {
                        ...
                        // can name.
                        throw new common_1.ConflictException('this batch lost a concurrent create and the winning tasks cannot all be identified, ' +
                    ...
                    if (item.supersedesTaskId) {
                        ...
                        await this.assertSupersessionAlreadyRecorded(this.prisma, ownerId, item.supersedesTaskId, winner.id);
                    }
        ...
        const touched = [
            ...created.flatMap((task) => [task.parentTaskId, task.verifiesTaskId].filter((id) => !!id)),
            // Same reason as create: each predecessor this batch retired has a parent and a subject whose
            // answers moved.
            ...items.map((item) => item.supersedesTaskId).filter((id) => !!id),
        ];
        if (touched.length) {
            await this.recomputeAggregates(ownerId, touched).catch((e) => this.logger.warn(`aggregation after batch create failed: ${e?.message ?? e}`));
        }
        return created;
    }
    /** Return the session id only if it exists under this owner; otherwise undefined. */
    async resolveOwnedSession(ownerId, sessionId) {
        if (!sessionId)
            return undefined;
     ...
     */
    taskIdempotencyKey(sessionId, turnId, title, description) {
    ...
    async list(ownerId) {
        ...
        return withRun.map((t) => {
            ...
            return { ...t, dependencyState, blocked: !(0, task_dependencies_1.canRun)(dependencyState) };
        });
    }
    /**
     ...
     */
    async listPage(ownerId, query = {}) {
        const limit = query.limit === undefined ? exports.DEFAULT_TASK_PAGE_SIZE : Number(query.limit);
        if (!Number.isInteger(limit) || limit < 1 || limit > exports.MAX_TASK_PAGE_SIZE) {
            throw new common_1.BadRequestException(`limit must be an integer from 1 to ${exports.MAX_TASK_PAGE_SIZE}`);
        }
        const countsMode = query.counts?.trim().toLowerCase();
        if (countsMode !== undefined && countsMode !== 'none' && countsMode !== 'total') {
            throw new common_1.BadRequestException("counts must be 'none' or 'total' when set");
        }
        ...
        let statuses;
        if (status === 'ONGOING')
            statuses = [shared_1.TaskStatus.OPEN, shared_1.TaskStatus.IN_PROGRESS];
        else if (status && !runnableOnly && !runningOnly) {
            if (!Object.values(shared_1.TaskStatus).includes(status)) {
                throw new common_1.BadRequestException('invalid task status');
            }
            statuses = [status];
        }
        const scopedWhere = { ownerId };
        if (query.listId === 'none')
            scopedWhere.listId = null;
        else if (query.listId) {
            if (!UUID_RE.test(query.listId))
                throw new common_1.BadRequestException('invalid task list id');
        ...
        if (query.assigneeId) {
            if (!UUID_RE.test(query.assigneeId))
                throw new common_1.BadRequestException('invalid assignee id');
        ...
        if (labelFilter.length)
            scopedWhere.labels = { hasEvery: labelFilter };
        ...
        const filteredWhere = runningOnly
            ? { ...scopedWhere, sessions: { some: { status: client_1.RunStatus.RUNNING } } }
            : { ...scopedWhere };
        if (statuses)
            filteredWhere.status = { in: statuses };
        ...
        const [rows, ownFilteredTotal, counts] = await Promise.all([
            runnableOnly
                ...
                : this.prisma.task.findMany({
                    ...
                    take: limit + 1,
                    select: exports.TASK_LIST_SELECT,
                }),
        ...
        const items = withRun.map((task) => {
            ...
            return { ...task, dependencyState, blocked: !(0, task_dependencies_1.canRun)(dependencyState) };
        });
        const nextCursor = hasMore && items.length ? encodeTaskPageCursor(items[items.length - 1]) : null;
        if (!wantTotal)
    ...
    async scopeCounts(scope) {
        const [statusGroups, running, queued, runnable] = await Promise.all([
            ...
            this.prisma.task.count({
                where: { ...scope, sessions: { some: { status: client_1.RunStatus.RUNNING } } },
            }),
            this.prisma.task.count({
                where: {
                    ...scope,
                    sessions: { some: { status: client_1.RunStatus.PENDING } },
                    NOT: { sessions: { some: { status: client_1.RunStatus.RUNNING } } },
                },
            }),
        ...
        return {
            ...
            open: byStatus.get(shared_1.TaskStatus.OPEN) ?? 0,
            inProgress: byStatus.get(shared_1.TaskStatus.IN_PROGRESS) ?? 0,
            done: byStatus.get(shared_1.TaskStatus.DONE) ?? 0,
            failed: byStatus.get(shared_1.TaskStatus.FAILED) ?? 0,
            cancelled: byStatus.get(shared_1.TaskStatus.CANCELLED) ?? 0,
            running,
            queued,
            runnable,
        };
     ...
     */
    async taskCounts(ownerId, query = {}) {
        const scope = { ownerId };
        if (query.listId === 'none')
            scope.listId = null;
        else if (query.listId) {
        ...
        const labelFilter = parseLabelsQuery(query.labels);
        if (labelFilter.length)
            scope.labels = { hasEvery: labelFilter };
     ...
     */
    async activeTasks(ownerId, query = {}) {
        const scope = { ownerId };
        if (query.listId === 'none')
            scope.listId = null;
        else if (query.listId) {
        ...
        const where = {
            ...scope,
            OR: [
                { sessions: { some: { status: { in: [client_1.RunStatus.RUNNING, client_1.RunStatus.PENDING] } } } },
                { status: { in: [shared_1.TaskStatus.IN_PROGRESS, shared_1.TaskStatus.FAILED] } },
            ],
        };
        const [rows, total] = await Promise.all([
            this.prisma.task.findMany({
                ...
                orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
                take: exports.ACTIVE_TASK_STRIP_MAX,
                select: exports.TASK_LIST_SELECT,
            }),
     ...
     */
    async labelSummary(ownerId, query = {}) {
        const scope = { ownerId };
        if (query.listId === 'none')
            scope.listId = null;
        else if (query.listId) {
      ...
      top AS (SELECT label FROM totals ORDER BY n DESC, label LIMIT ${exports.TASK_LABEL_SUMMARY_MAX})
     ...
     */
    async runnableTaskPage(scope, opts) {
        const clauses = [taskScopeSql(scope), RUNNABLE_TASK_SQL];
        if (opts.search)
            clauses.push(client_1.Prisma.sql `t.title ILIKE '%' || ${opts.search} || '%'`);
        if (opts.cursor) {
            ...
            clauses.push(client_1.Prisma.sql `(t.created_at < ${at}::timestamp
    ...
    async runnableTaskCount(scope, search) {
        const clauses = [taskScopeSql(scope), RUNNABLE_TASK_SQL];
        if (search)
            clauses.push(client_1.Prisma.sql `t.title ILIKE '%' || ${search} || '%'`);
        const [row] = await this.prisma.$queryRaw `
      SELECT count(*)::int AS count FROM task t WHERE ${client_1.Prisma.join(clauses, ' AND ')}`;
        return row?.count ?? 0;
    }
    /**
     ...
     */
    async withRunning(ownerId, tasks, restrictToTaskIds = false) {
        if (tasks.length === 0)
            return [];
        const busy = await this.prisma.session.groupBy({
            ...
            where: {
                ...
                status: { in: [client_1.RunStatus.PENDING, client_1.RunStatus.RUNNING] },
            },
            _count: { _all: true },
        });
        const running = new Set(busy.filter((b) => b.status === client_1.RunStatus.RUNNING).map((b) => b.taskId));
        const queued = new Set(busy.filter((b) => b.status === client_1.RunStatus.PENDING).map((b) => b.taskId));
        return tasks.map((t) => ({
            ...t,
     ...
     */
    async dependencyGraph(ownerId, focusTaskId, query = {}) {
        if (!UUID_RE.test(focusTaskId))
            throw new common_1.NotFoundException('task not found');
        const rawDirection = query.direction ?? 'upstream';
        if (rawDirection !== 'upstream' && rawDirection !== 'both') {
            throw new common_1.BadRequestException('direction must be upstream or both');
        }
        const direction = rawDirection;
        const pairUnary = dependencyGraphBoolean(query.pairUnary, 'pairUnary');
        const maxDepth = dependencyGraphLimit(query.maxDepth, DEFAULT_DEPENDENCY_GRAPH_MAX_DEPTH, 'maxDepth', MAX_DEPENDENCY_GRAPH_MAX_DEPTH);
        const maxNodes = dependencyGraphLimit(query.maxNodes, DEFAULT_DEPENDENCY_GRAPH_MAX_NODES, 'maxNodes', MAX_DEPENDENCY_GRAPH_MAX_NODES);
        const maxEdges = Math.min(maxNodes * DEPENDENCY_GRAPH_EDGES_PER_NODE, MAX_DEPENDENCY_GRAPH_EDGES);
        ...
        const scheduledFrontiers = new Map();
        ...
        while (frontier.length > 0 && traversedDepth < maxDepth) {
            ...
            const fetchAdjacent = async (where) => this.prisma.taskDependency.findMany({
                where,
            ...
            const rows = [];
            for (let prerequisiteIndex = 0, dependentIndex = 0; rows.length < maxEdges &&
            ...
            const upstreamCandidateIds = new Set();
            const downstreamCandidateIds = new Set();
            for (const row of rows) {
            ...
            const unaryByCandidate = new Map();
            if (pairUnary && nextDepth < maxDepth && directCandidateIds.size > 0) {
                const candidateCounts = await this.dependencyConnectionCounts(ownerId, [...directCandidateIds]);
                const unaryUpstreamIds = [...upstreamCandidateIds].filter((id) => candidateCounts.get(id)?.prerequisiteCount === 1);
                const unaryDownstreamIds = [...downstreamCandidateIds].filter((id) => candidateCounts.get(id)?.dependentCount === 1);
                ...
                for (const row of unaryUpstreamRows)
                    unaryByCandidate.set(row.taskId, row);
                for (const row of unaryDownstreamRows)
                    unaryByCandidate.set(row.dependsOnTaskId, row);
            }
            const next = new Set();
            for (const row of rows) {
                const isUpstream = direction === 'upstream' || frontierIds.has(row.taskId);
                ...
                if (!nodes.has(candidate.id)) {
                    ...
                    const admitUnary = unaryCandidate !== undefined &&
                    ...
                    discoveryEdges.set(`${discoveryEdge.sourceTaskId}:${discoveryEdge.targetTaskId}`, discoveryEdge);
                    if (admitUnary && unaryRow && unaryCandidate) {
                        const unaryNode = {
                            ...unaryCandidate,
                            status: unaryCandidate.status,
                            depth: nextDepth + 1,
                        };
                        nodes.set(unaryNode.id, unaryNode);
                        deepestDepth = Math.max(deepestDepth, nextDepth + 1);
                        const scheduled = scheduledFrontiers.get(nextDepth + 1) ?? new Set();
            ...
            for (const scheduledId of scheduledFrontiers.get(nextDepth) ?? [])
        ...
        if (truncationReasons.size === 0 && frontier.length > 0 && traversedDepth === maxDepth) {
            ...
            const hiddenWhere = direction === 'upstream'
                ? {
                    ...ownerScope,
                    taskId: { in: frontier },
                    dependsOnTaskId: { notIn: visitedIds },
        ...
        if (fetchedEdgeRows.length > maxEdges)
            truncationReasons.add('maxEdges');
        const edges = new Map();
        ...
        for (const [key, edge] of discoveryEdges)
            edges.set(key, edge);
        ...
        for (const edge of edges.values()) {
            ...
            if (incomingSources)
                incomingSources.push(edge.sourceTaskId);
            else
                incoming.set(edge.targetTaskId, [edge.sourceTaskId]);
            const outgoingTargets = outgoing.get(edge.sourceTaskId);
            if (outgoingTargets)
                outgoingTargets.push(edge.targetTaskId);
            else
                outgoing.set(edge.sourceTaskId, [edge.targetTaskId]);
        }
        const reachable = (adjacency) => {
            ...
            while (pending.length > 0) {
                ...
                if (id === focusTaskId || reached.has(id))
                    continue;
                reached.add(id);
        ...
        const done = upstreamNodes.filter((node) => node.status === shared_1.TaskStatus.DONE).length;
        const failed = upstreamNodes.filter((node) => node.status === shared_1.TaskStatus.FAILED || node.status === shared_1.TaskStatus.CANCELLED).length;
        const collapsedGroups = this.dependencyGraphCollapsedGroups(ownerId, focusTaskId, direction, nodeIds, edges.values(), connectionCounts);
        return {
            focusTaskId,
            direction,
            pairUnary,
     ...
     */
    async dependencyGraphNodes(ownerId, focusTaskId, taskIds) {
        if (!UUID_RE.test(focusTaskId))
            throw new common_1.NotFoundException('task not found');
        if (!Array.isArray(taskIds) || taskIds.length < 1) {
            throw new common_1.BadRequestException('taskIds must contain at least one task');
        }
        if (taskIds.length > MAX_DEPENDENCY_GRAPH_EXPANDED_NODES) {
            throw new common_1.BadRequestException(`taskIds must contain at most ${MAX_DEPENDENCY_GRAPH_EXPANDED_NODES} tasks`);
        }
        if (taskIds.some((id) => !UUID_RE.test(id))) {
            throw new common_1.BadRequestException('dependency graph task ids must be UUIDs');
        }
        const uniqueTaskIds = [...new Set(taskIds)];
        if (!uniqueTaskIds.includes(focusTaskId)) {
            throw new common_1.BadRequestException('taskIds must include the focus task');
        }
        ...
        return {
            nodes,
            edges,
            collapsedGroups: this.dependencyGraphCollapsedGroups(ownerId, focusTaskId, 'both', existingTaskIds, edges, connectionCounts),
            missingTaskIds,
            truncatedEdges,
        };
    }
     ...
     */
    async expandDependencyGraph(ownerId, focusTaskId, query) {
        if (!UUID_RE.test(focusTaskId) || !UUID_RE.test(query.anchorTaskId)) {
            throw new common_1.NotFoundException('task not found');
        }
        if (query.direction !== 'prerequisites' && query.direction !== 'dependents') {
            throw new common_1.BadRequestException('direction must be prerequisites or dependents');
        }
        ...
        if (!Array.isArray(query.knownTaskIds) || query.knownTaskIds.length < 1) {
            throw new common_1.BadRequestException('knownTaskIds must contain at least one task');
        }
        if (query.knownTaskIds.length > MAX_DEPENDENCY_GRAPH_EXPANDED_NODES) {
            throw new common_1.BadRequestException(`knownTaskIds must contain at most ${MAX_DEPENDENCY_GRAPH_EXPANDED_NODES} tasks`);
        }
        if (!Array.isArray(query.loadedNeighborTaskIds)) {
            throw new common_1.BadRequestException('loadedNeighborTaskIds must be an array');
        }
        ...
        if (knownTaskIds.length !== query.knownTaskIds.length) {
            throw new common_1.BadRequestException('knownTaskIds must not contain duplicates');
        }
        if (loadedNeighborTaskIds.length !== query.loadedNeighborTaskIds.length) {
            throw new common_1.BadRequestException('loadedNeighborTaskIds must not contain duplicates');
        }
        if (knownTaskIds.some((id) => !UUID_RE.test(id)) ||
        ...
        if (!knownTaskIdSet.has(focusTaskId) || !knownTaskIdSet.has(query.anchorTaskId)) {
            throw new common_1.BadRequestException('knownTaskIds must include the focus and anchor tasks');
        }
        if (loadedNeighborTaskIds.some((id) => !knownTaskIdSet.has(id))) {
            throw new common_1.BadRequestException('loadedNeighborTaskIds must be present in knownTaskIds');
        }
        const cursor = decodeDependencyGraphBranchCursor(query.cursor);
        if (cursor.ownerScope !== dependencyGraphOwnerScope(ownerId) ||
            cursor.focusTaskId !== focusTaskId ||
            cursor.anchorTaskId !== query.anchorTaskId ||
            cursor.direction !== direction) {
            throw new common_1.BadRequestException('dependency graph expansion cursor does not match request');
        }
        ...
        const fetchExpansionRows = async (where, take) => this.prisma.taskDependency.findMany({
            where,
            orderBy: [{ createdAt: 'asc' }, { id: 'asc' }],
            take,
            select: {
        ...
        const knownAdjacencyWhere = direction === 'prerequisites'
            ? {
                ...ownerScope,
        ...
        const unknownAdjacencyWhere = direction === 'prerequisites'
            ? {
                ...ownerScope,
                taskId: query.anchorTaskId,
        ...
        const unknownNeighborRows = remainingBatchSlots > 0
        ...
        const pageRows = [...knownNeighborRows];
        ...
        for (const row of unknownNeighborRows) {
            ...
            if (!knownTaskIdSet.has(neighbor.id) &&
        ...
        const pageNeighborIds = pageRows.map((row) => direction === 'prerequisites' ? row.dependsOnTaskId : row.taskId);
        const updatedLoadedNeighborIds = [...loadedNeighborTaskIds, ...pageNeighborIds];
        const remainingWhere = direction === 'prerequisites'
            ? {
                ...ownerScope,
                taskId: query.anchorTaskId,
        ...
        const directConnectionCounts = await this.dependencyConnectionCounts(ownerId, directNewTaskIds);
        ...
        const unaryCandidates = unaryAnchorIds.length === 0
            ? []
            : await this.prisma.taskDependency.findMany({
                where: direction === 'prerequisites'
                    ? { ...ownerScope, taskId: { in: unaryAnchorIds } }
                    : {
                        ...ownerScope,
        ...
        const inducedRows = newTaskIds.length === 0
            ? []
            : await this.prisma.taskDependency.findMany({
                where: {
                    ...ownerScope,
        ...
        for (const [key, edge] of requiredEdges)
            edges.set(key, edge);
        ...
        const remainingKnownNeighborCount = atNodeCapacity && remainingCount > 0
        ...
        const nextCursor = remainingCount > 0 && (!atNodeCapacity || remainingKnownNeighborCount > 0)
            ? encodeDependencyGraphBranchCursor(ownerId, focusTaskId, query.anchorTaskId, direction)
            : null;
        ...
        for (const node of responseNodes) {
            const loadedPrerequisites = new Set(responseEdges
                .filter((edge) => edge.targetTaskId === node.id)
                .map((edge) => edge.sourceTaskId)).size;
            const loadedDependents = new Set(responseEdges
                .filter((edge) => edge.sourceTaskId === node.id)
                .map((edge) => edge.targetTaskId)).size;
            ...
            for (const [nodeDirection, hiddenCount] of hiddenByDirection) {
                ...
                collapsedGroups.push({
                    ...
                    hiddenCount,
                    cursor: augmentedKnownTaskIds.length < MAX_DEPENDENCY_GRAPH_EXPANDED_NODES
                        ? encodeDependencyGraphBranchCursor(ownerId, focusTaskId, node.id, nodeDirection)
                        : null,
                });
            }
        }
    ...
    async get(ownerId, id) {
        ...
        const prerequisiteEpochs = await (0, verification_epoch_read_1.loadVerificationEpochGates)(this.prisma, task.dependsOn.map((d) => d.dependsOnTaskId));
        const dependencyState = (0, task_dependencies_1.computeDependencyState)(task.dependsOn.map((d) => ({
            status: d.dependsOnTask.status,
            verificationGate: (0, task_dependencies_1.dependencyEpochGate)(d.dependsOnTaskId, prerequisiteEpochs, task.verifiesTaskId),
        })));
        const supersession = await this.supersession(ownerId, task);
        return {
            ...task,
            sessions: (task.sessions ?? []).map((session) => (0, session_state_1.withSessionState)(session)),
            creatorSession: task.creatorSession ? (0, session_state_1.withSessionState)(task.creatorSession) : null,
     ...
     */
    async supersession(ownerId, task) {
        const predecessors = task.supersedes ?? [];
        const forward = task.supersededByTaskId == null
            ? []
            ...
            WITH RECURSIVE walk AS (
              ...
              SELECT n."id", n."title", n."status"::text, n."superseded_by_task_id", n."terminal_reason", w.depth + 1
               ...
               WHERE w.depth < ${task_supersession_1.TASK_SUPERSESSION_MAX_HOPS}
            )
        ...
        const edges = new Map([[task.id, task.supersededByTaskId]]);
        for (const row of forward)
            edges.set(row.id, row.supersededByTaskId);
        const { chain, truncated } = (0, task_supersession_1.successorChain)(task.id, edges);
        const byId = new Map(forward.map((row) => [row.id, row]));
        return {
            outcome: (0, task_supersession_1.taskOutcome)(task),
            supersededByTaskIdAbsentReason: (0, task_supersession_1.supersededByAbsentReason)(task),
            supersedes: predecessors,
            supersedesEmptyReason: predecessors.length > 0 ? null : 'SUPERSEDES_NOTHING',
     ...
     */
    async resolveCommentAuthors(comments) {
        if (comments.length === 0)
            return [];
        const userIds = comments.filter((c) => c.authorType === client_1.CreatorType.USER).map((c) => c.authorId);
        const workspaceIds = comments.filter((c) => c.authorType === client_1.CreatorType.AGENT).map((c) => c.authorId);
     ...
     */
    async update(ownerId, id, dto, actingSessionId) {
        ...
        const touchesHierarchy = dto.projectId !== undefined ||
            dto.parentTaskId !== undefined ||
            dto.verifiesTaskId !== undefined;
        const dependsOnTaskIds = dto.dependsOnTaskIds === undefined ? undefined : [...new Set(dto.dependsOnTaskIds)];
        ...
        const data = {
            ...
            acceptanceCriteria: dto.acceptanceCriteria === undefined ? undefined : (dto.acceptanceCriteria ?? null),
            autoRunWhenReady: dto.autoRunWhenReady,
            completionPolicy: dto.completionPolicy,
        ...
        const verifiesTaskId = dto.verifiesTaskId === undefined ? before.verifiesTaskId : (dto.verifiesTaskId ?? null);
        ...
        if (dto.verdict != null && !verifiesTaskId) {
            throw new common_1.BadRequestException('Only a verification task can carry a verdict — this task does not verify anything');
        }
        ...
        if (dto.verifiesTaskId !== undefined &&
            ...
            throw new common_1.BadRequestException('This verification has already concluded — its subject can no longer be changed. File a ' +
        ...
        if (concludesVerdict && actingSessionId && verifiesTaskId) {
            ...
            if (actingSession?.taskId && actingSession.taskId === verifiesTaskId) {
                throw new common_1.BadRequestException('A verification cannot be concluded from the session that ran the task it verifies — ' +
        ...
        if (dto.completionPolicy != null) {
            ...
            if (row && (0, task_aggregation_1.foremanPolicyConflict)({
                isForeman: row.isForeman,
                completionPolicy: dto.completionPolicy,
            })) {
                throw new common_1.BadRequestException('This task is a coordinating (foreman) task, so its own run is what finishes it — it ' +
        ...
        if (dto.status === shared_1.TaskStatus.DONE && before.status !== shared_1.TaskStatus.DONE) {
            ...
            if (policy && policy !== 'MANUAL') {
                ...
                if (children > 0) {
                    throw new common_1.BadRequestException(`This task completes by ${policy}, not by a status write — its ${children} subtask(s)` +
        ...
        if (dto.status !== undefined
            ...
            && !task_supersession_1.TASK_SUPERSEDABLE_STATUSES.has(dto.status)
            && (0, task_supersession_1.taskRetirement)(before) != null) {
            throw new common_1.BadRequestException(`task ${(0, shared_1.uuidToBase62)(id)} is recorded as ${before.terminalReason ?? 'SUPERSEDED'}` +
                (before.supersededByTaskId
                    ? ` by ${(0, shared_1.uuidToBase62)(before.supersededByTaskId)}`
        ...
        if (dto.supersededByTaskId !== undefined || dto.terminalReason !== undefined) {
            ...
            const projectAfter = dto.projectId === undefined ? before.projectId : (dto.projectId ?? null);
            const successorId = dto.supersededByTaskId === undefined
                ? before.supersededByTaskId
            ...
            if (successorId && dto.terminalReason != null && dto.terminalReason !== 'SUPERSEDED') {
                throw new common_1.BadRequestException(`A task with a successor is SUPERSEDED, not ${dto.terminalReason} — pass one or the other`);
            }
            if (dto.terminalReason === 'SUPERSEDED' && !successorId) {
                throw new common_1.BadRequestException('SUPERSEDED names the attempt that took over — pass supersededByTaskId, or use ' +
                    'ABANDONED for a task nothing replaced');
            }
            if (dto.terminalReason != null && !task_supersession_1.TASK_SUPERSEDABLE_STATUSES.has(statusAfter)) {
                throw new common_1.BadRequestException(`A terminal reason belongs to a task that has stopped — this one is ${statusAfter}`);
            }
        ...
        const touchesAcceptanceFacts = (0, task_lock_order_1.touchesAcceptanceFact)(dto);
        ...
        const restructures = dependsOnTaskIds !== undefined ||
            touchesHierarchy ||
            concludesVerdict ||
            !!supersession ||
        ...
        const updated = await (!restructures && !(touchesAcceptanceFacts && before.projectId)
            ...
            : (0, transaction_retry_1.withTransactionRetry)(this.prisma, async (tx) => {
                if (restructures)
                    await this.lockDependencyGraph(tx, ownerId);
                ...
                await (0, lock_order_1.lockTaskLists)(tx, [dto.listId]);
                if (rewritesTaskRow)
                    await this.preLockCreatorSessions(tx, [], [id]);
                ...
                if (acceptanceProjects.length > 0) {
                    await (0, task_lock_order_1.assertAcceptanceScopeUnmoved)(tx, ownerId, [id], [...acceptanceProjects, ...(dto.projectId ? [dto.projectId] : [])]);
                }
                ...
                // to avoid it.
                if (supersession || movesProject)
                    await this.lockTaskForSupersessionWrite(tx, id);
                if (touchesHierarchy) {
                ...
                if (dto.projectId !== undefined && dto.projectId !== current.projectId) {
                    ...
                    await this.assertNotRunningBeforeRetiring(tx, ownerId, id).catch((error) => {
                        if (!(error instanceof common_1.ConflictException))
                            throw error;
                        throw new common_1.ConflictException(`task ${(0, shared_1.uuidToBase62)(id)} has a live run, so it cannot be moved to another ` +
                ...
                if (dto.projectId !== undefined) {
                    ...
                    if (stranded.length > 0) {
                        throw new common_1.BadRequestException(`task ${(0, shared_1.uuidToBase62)(id)} is the successor of ` +
                            `${stranded.map((row) => (0, shared_1.uuidToBase62)(row.id)).join(', ')}, and a successor ` +
                ...
                if (dependsOnTaskIds?.length) {
                    ...
                    if ((0, task_dependencies_1.wouldReplacementCreateCycle)(edges, id, dependsOnTaskIds)) {
                        throw new common_1.BadRequestException('These dependencies would create a cycle');
                    }
                }
                ...
                const writeSupersession = async () => {
                    ...
                    // first retirement.
                    const retiredBefore = (0, task_supersession_1.taskRetirement)(before) != null;
        ...
        if (dependsOnTaskIds !== undefined) {
            this.realtime.publishForUser(ownerId, shared_1.RunEventType.TASK_CHANGED, id);
        }
        ...
        if (dto.status === shared_1.TaskStatus.DONE && before.status !== 'DONE') {
            await this.triggerDependents(ownerId, id).catch((e) => this.logger.warn(`triggerDependents failed for task ${id}: ${e?.message ?? e}`));
            ...
            await this.fileVerification(ownerId, id).catch((e) => this.logger.warn(`verification dispatch for task ${id} failed: ${e?.message ?? e}`));
        }
        ...
        const movesAggregationInput = dto.status !== undefined ||
        ...
        if (movesAggregationInput) {
            await this.recomputeAggregates(ownerId, [
                id,
                before.parentTaskId,
                ...
                before.supersededByTaskId,
                dto.supersededByTaskId ?? undefined,
            ].filter((taskId) => !!taskId)).catch((e) => this.logger.warn(`aggregation after update of ${id} failed: ${e?.message ?? e}`));
        }
        if (before.status === 'DONE' && dto.status !== undefined && dto.status !== shared_1.TaskStatus.DONE && before.listId) {
            ...
            await this.recordListEvent(before.listId, 'completion_reverted', `任务「${before.title}」(${(0, shared_1.uuidToBase62)(id)}) 从 DONE 被退回 ${dto.status}` +
     ...
     */
    async fileVerification(ownerId, taskId) {
        const task = await this.prisma.task.findFirst({
        ...
        if (!task || task.isForeman || task.verifiesTaskId)
            return;
        ...
        const executed = await this.prisma.session.count({
            where: {
                ...
                OR: [{ numTurns: { gt: 0 } }, { status: client_1.RunStatus.RUNNING }],
            },
        });
        ...
        if (!unevidenced && !task.list?.verifyOnDone)
            return;
        ...
        const already = await this.prisma.task.count({
            where: { verifiesTaskId: taskId, status: { not: shared_1.TaskStatus.CANCELLED } },
        });
        if (already >= exports.MAX_VERIFICATIONS_PER_TASK) {
            this.logger.log(`verification skipped for task ${taskId} — already checked ${already} time(s)`);
            return;
        }
        const verification = await this.prisma.task.create({
            data: {
                ...
                creatorType: client_1.CreatorType.USER,
                creatorId: ownerId,
            },
            select: { id: true },
        });
     ...
     * the id becomes text.
     */
    buildVerificationBrief(title, taskId, unevidenced = false) {
        return (`任务「${title}」（id: ${(0, shared_1.uuidToBase62)(taskId)}）刚刚被标记为 DONE。请独立核实它是否真的完成了，然后结束本次运行。\n\n` +
            (unevidenced
     ...
     */
    async triggerDependents(ownerId, doneTaskId) {
        const edges = await this.prisma.taskDependency.findMany({
        ...
        for (const dep of dependents) {
            if ((states.get(dep.id) ?? 'NONE') !== 'READY')
                continue;
            if (dep.status !== 'OPEN')
                continue; // already running/done/cancelled — leave it
            if (!dep.autoRunWhenReady)
                continue; // gate kept, manual trigger only
            ...
            if (dep.runAt && dep.runAt > now)
                continue;
            if (!dep.assignee?.runnerId)
                continue; // nothing to run it on — stays ready for later
            try {
                ...
                this.logger.warn(`auto-run of dependent task ${dep.id} failed: ${e instanceof Error ? e.message : e}`);
            }
        }
    }
    /**
    ...
    async reconcileReadyTasks() {
      ...
      WHERE ${AUTO_RUN_READY_SQL}`;
        ...
        const seeds = await (0, workspace_provider_1.lastProviderByWorkspace)(this.prisma, rows.map((row) => row.workspaceId));
        ...
        const ready = rows.map((row) => ({
            ...
            ownerId: row.ownerId,
            assignee: {
                provider: (seeds.get(row.workspaceId) ?? workspace_provider_1.DEFAULT_AGENT_PROVIDER).provider,
        ...
        const blindTaskIds = new Set(ready
            .filter((t) => {
            const runnerId = t.assignee?.runnerId;
            return !!runnerId && quotaBlind.has(`${runnerId}:${t.assignee.provider}`);
        })
            .map((t) => t.id));
        const heldOff = await this.autoRunHoldOff(ready.map((t) => t.id), blindTaskIds);
        ...
        const holdFor = (listId) => {
            if (!listId)
                return null;
            let e = perList.get(listId);
            if (!e)
                perList.set(listId, (e = { quota: 0, disk: 0 }));
            return e;
        };
        for (const t of ready) {
            ...
            if (blockedUntil) {
                quotaHeld += 1;
                if (!resumesAt || blockedUntil < resumesAt)
                    resumesAt = blockedUntil;
                const e = holdFor(t.listId);
                if (e) {
                    e.quota += 1;
                    if (!e.resumesAt || blockedUntil < e.resumesAt)
                        e.resumesAt = blockedUntil;
                }
                continue;
            }
            ...
            try {
                ...
                this.logger.warn(`reconcile auto-run of task ${t.id} failed: ${e instanceof Error ? e.message : e}`);
            }
        }
        ...
        if (quotaHeld > 0) {
            this.logger.log(`auto-run: ${quotaHeld} ready task(s) held — provider quota exhausted, earliest reset ${resumesAt?.toISOString()}`);
        }
        if (diskHeld > 0) {
            this.logger.log(`auto-run: ${diskHeld} ready task(s) held — free disk below the runner's floor`);
        }
        if (atCapacity > 0) {
            ...
            this.logger.log(`auto-run: ${atCapacity} ready task(s) left for a later sweep — no free slot to claim them`);
        }
        if (rescheduled > 0) {
            this.logger.log(`auto-run: ${rescheduled} ready task(s) were rescheduled mid-sweep — their new schedule stands`);
        }
        for (const [listId, e] of perList) {
            if (e.quota > 0) {
                await this.recordListEvent(listId, 'quota_hold', `${e.quota} 个就绪任务被配额挡住` +
                    (e.resumesAt ? `，最早 ${e.resumesAt.toISOString()} 恢复` : '，没有拿到恢复时间'));
            }
            if (e.disk > 0) {
                await this.recordListEvent(listId, 'disk_hold', `${e.disk} 个就绪任务被磁盘下限挡住 —— 空间要由人来腾，不会自己恢复`);
            }
        }
    }
    /**
     ...
     */
    async dispatchDueScheduledTasks() {
      ...
      LIMIT ${exports.SCHEDULED_DISPATCH_MAX_PER_SWEEP}`;
        if (due.length === 0)
            return;
        ...
        for (const t of due) {
            ...
            try {
                ...
                this.logger.warn(`scheduled run of task ${t.id} failed: ${e instanceof Error ? e.message : e}`);
            }
        }
        if (dispatched > 0)
            this.logger.log(`scheduled: ${dispatched} due task(s) dispatched`);
        if (rescheduled > 0) {
            ...
            this.logger.log(`scheduled: ${rescheduled} due task(s) were rescheduled mid-sweep — their new schedule stands`);
        }
        if (atCapacity > 0) {
            this.logger.log(`scheduled: ${atCapacity} due task(s) left for a later sweep — no free slot to claim them`);
        }
    }
    /**
     ...
     */
    async recordListEvent(listId, kind, detail) {
        try {
            ...
            this.logger.warn(`could not record ${kind} on list ${listId}: ${e instanceof Error ? e.message : e}`);
        }
    }
    /**
     ...
     */
    async dispatchStalledListForemen() {
      ...
      WHERE tl.foreman_workspace_id IS NOT NULL
        ...
        for (const list of stalled) {
            ...
            if (prior) {
                if (prior.count >= exports.MAX_CONSECUTIVE_FOREMEN) {
                    heldOff += 1;
                    continue;
                }
                const since = now - prior.lastAt.getTime();
                if (since < exports.FOREMAN_RETRY_BACKOFF_MS[prior.count - 1]) {
                    heldOff += 1;
                    continue;
                }
            }
            try {
                ...
                await this.recordListEvent(list.id, 'foreman_filed', `停滞约 ${list.minutes} 分钟，已自动派出协调任务 ${(0, shared_1.uuidToBase62)(task.id)} 去诊断`);
            }
            catch (e) {
                this.logger.warn(`foreman dispatch for list ${list.id} failed: ${e instanceof Error ? e.message : e}`);
            }
        }
        if (heldOff > 0) {
            this.logger.log(`foreman: ${heldOff} stalled list(s) held — a previous coordinator did not clear the stall`);
        }
    }
    /**
    ...
    async foremanHistory(listIds) {
      ...
      GROUP BY ft.list_id`;
        for (const row of rows)
            out.set(row.listId, { count: Number(row.count), lastAt: row.lastAt });
        return out;
    }
    /**
     ...
     * behalf.
     */
    buildForemanBrief(title, minutes) {
        return (`任务列表「${title}」已停滞约 ${minutes} 分钟：仍有未完成的任务，但没有任何任务在运行，也没有新的运行被发起。\n\n` +
     ...
     */
    async quotaGate(tasks) {
        const runnerIds = [
            ...new Set(tasks.map((t) => t.assignee?.runnerId).filter((id) => typeof id === 'string')),
        ];
        ...
        const usageByRunner = new Map(runners.map((r) => [r.id, r.planUsage]));
        const now = new Date();
        for (const t of tasks) {
            const assignee = t.assignee;
            if (!assignee?.runnerId)
                continue;
            const key = `${assignee.runnerId}:${assignee.provider}`;
            const usage = usageByRunner.get(assignee.runnerId);
            if (!(0, shared_1.planUsageReported)(usage, assignee.provider))
                blind.add(key);
            if (blocked.has(key))
                continue;
            const until = (0, shared_1.planUsageBlockedUntil)(usage, assignee.provider, now);
            if (until)
     ...
     */
    async autoRunHoldOff(taskIds, quotaBlindTaskIds) {
        const held = new Set();
        const uniqueIds = [...new Set(taskIds)];
        const usageLimitIs = (negated) => {
            const clauses = shared_1.USAGE_LIMIT_ERROR_MARKERS.map((marker) => ({
                error: { contains: marker, mode: client_1.Prisma.QueryMode.insensitive },
            }));
        ...
        for (let offset = 0; offset < uniqueIds.length; offset += TASK_ID_QUERY_CHUNK) {
            ...
            for (const row of failures) {
                ...
                if (failed >= task_retry_policy_1.MAX_AUTO_RUN_FAILURES) {
                    held.add(row.taskId);
                    continue;
                }
                ...
                if (lastFailedAt === undefined)
                    continue;
                if (now - lastFailedAt < task_retry_policy_1.AUTO_RUN_RETRY_BACKOFF_MS[failed - 1])
                    held.add(row.taskId);
            }
            ...
            for (const row of quotaFailures) {
                if (!row.taskId || held.has(row.taskId))
                    continue;
                const lastFailedAt = row._max.createdAt?.getTime();
                if (lastFailedAt === undefined)
                    continue;
                if (now - lastFailedAt < task_retry_policy_1.QUOTA_BLIND_RETRY_BACKOFF_MS)
                    held.add(row.taskId);
            }
        }
        return held;
     ...
     */
    async previewDag(ownerId, listId, ops) {
        if (ops.length === 0)
            throw new common_1.BadRequestException('no dependency changes proposed');
        if (ops.length > dto_1.MAX_DAG_OPS) {
            throw new common_1.BadRequestException(`at most ${dto_1.MAX_DAG_OPS} dependency changes at a time`);
        }
        const list = await this.prisma.taskList.findFirst({
            where: { id: listId, ownerId },
        ...
        if (inList !== dependents.length) {
            throw new common_1.BadRequestException('every edited task must belong to the list being restructured');
        }
        const current = await this.prisma.taskDependency.findMany({
            where: { task: { ownerId } },
            select: { taskId: true, dependsOnTaskId: true },
        });
        const { effective, noop } = (0, task_dag_1.effectiveOps)(current, ops);
        const after = (0, task_dag_1.resultingEdges)(current, ops);
        const cycle = (0, task_dag_1.findCycle)(after);
        ...
        for (const e of [...current, ...after]) {
            if (touched.has(e.taskId))
                touched.add(e.dependsOnTaskId);
        }
        const tasks = await this.prisma.task.findMany({
        ...
        const changes = (0, task_dag_1.stateChanges)(current, after, new Map(tasks.map((t) => [t.id, t.status]))).filter((c) => titleOf.has(c.taskId));
        const name = (id) => titleOf.get(id) ?? id;
        return {
            listId: list.id,
     ...
     */
    async applyDag(ownerId, listId, ops) {
        const preview = await this.previewDag(ownerId, listId, ops);
        if (preview.cycle) {
            throw new common_1.BadRequestException(`These changes would create a cycle: ${preview.cycle.map((c) => c.title).join(' → ')}`);
        }
        ...
        const applied = await (0, transaction_retry_1.withTransactionRetry)(this.prisma, async (tx) => {
            await this.lockDependencyGraph(tx, ownerId);
            ...
            const cycle = (0, task_dag_1.findCycle)((0, task_dag_1.resultingEdges)(current, ops));
            if (cycle) {
                throw new common_1.ConflictException('the graph changed while this was awaiting approval, and these changes would now create a cycle');
            }
            ...
            const effective = (0, task_dag_1.effectiveOps)(current, ops).effective;
        ...
        this.realtime.publishForUser(ownerId, shared_1.RunEventType.TASK_CHANGED, listId);
        ...
        await this.reconcileReadyTasks().catch((e) => this.logger.warn(`reconcile after DAG change failed: ${e instanceof Error ? e.message : e}`));
        return { ...applied, preview };
    }
    /** Add a "task depends on dependsOnTaskId" edge; rejects self-deps and cycles. */
    async addDependency(ownerId, taskId, dependsOnTaskId) {
        ...
        try {
            ...
            await (0, transaction_retry_1.withTransactionRetry)(this.prisma, async (tx) => {
                ...
                if ((0, task_dependencies_1.wouldCreateCycle)(edges, taskId, dependsOnTaskId)) {
    ...
    async removeDependency(ownerId, taskId, dependsOnTaskId) {
        await this.assertOwnedTasks(ownerId, [taskId]);
        if (!UUID_RE.test(dependsOnTaskId))
            throw new common_1.NotFoundException('task not found');
    ...
    async remove(ownerId, id) {
        ...
        if (survivingLinks.length) {
            await this.recomputeAggregates(ownerId, survivingLinks).catch((e) => this.logger.warn(`aggregation after delete of ${id} failed: ${e?.message ?? e}`));
        }
        return { ok: true };
    }
    /**
     ...
     */
    async deleteAndStopRuns(ownerId, ids) {
        ...
        const { deleted, runs, links } = await this.runTaskWorkTranslatingFences(() => (0, transaction_retry_1.withTransactionRetry)(this.prisma, async (tx) => {
          ...
          FOR UPDATE`;
            ...
            const doomed = await (0, task_lock_order_1.cascadedTaskClosure)(tx, ownerId, ids);
            const occupying = await tx.session.findMany({
                where: { ownerId, taskId: { in: doomed }, status: { in: reclaim_stalled_task_1.TASK_OCCUPYING } },
                select: { id: true },
            });
        ...
        for (const sessionId of runs) {
            ...
            // the reaper.
            await this.sessions
                .cancel(ownerId, sessionId, shared_1.SessionEndReason.TASK_CANCELLED)
                .catch((e) => this.logger.warn(`task delete: could not end run ${sessionId}: ${e instanceof Error ? e.message : e}`));
        }
        if (runs.length > 0) {
        ...
        const survivingLinks = [
            ...new Set((links ?? [])
                ...
                .filter((id) => !!id && !doomed.has(id))),
        ];
        return { deleted, survivingLinks };
    }
    async addComment(ownerId, id, dto, author) {
        ...
        if (mentioned.length !== requested.length) {
            ...
            throw new common_1.BadRequestException(`these mentioned agents do not exist or are not yours: ` +
                `${missing.map((id) => (0, shared_1.uuidToBase62)(id)).join(', ')}. ` +
        ...
        const comment = await this.prisma.taskComment.create({
            data: {
                taskId: id,
                // Defaults to the human (user-facing API); the runner path passes the workspace.
                authorType: author?.type ?? client_1.CreatorType.USER,
                ...
                mentionDeliveryVersion: exports.TASK_COMMENT_MENTION_DELIVERY_VERSION,
            },
        });
        ...
        if (task.creatorSessionId)
            this.realtime.publishTaskChanged(task.creatorSessionId, id);
        ...
        void this.deliverMentions().catch((e) => this.logger.warn(`mention delivery kick after comment ${comment.id} failed: ${e?.message ?? e}`));
        return comment;
    }
    /** Filter mention ids down to workspaces this user owns; dedupe. Returns id + runnerId. */
    async resolveMentionedWorkspaces(ownerId, ids) {
        if (!ids?.length)
            return [];
        const unique = [...new Set(ids)];
     ...
     */
    async createTaskSessionOrReadWinner(ownerId, task, workspace, dto, opts) {
        try {
            ...
            if (!this.isDuplicateKey(e) || !isExecutionClaimConflict(e))
                throw e;
            ...
            if (!winner) {
                ...
                const holder = await this.prisma.session.findFirst({
                    where: {
                        ...
                        deletedAt: null,
                        status: {
                            in: [...SINGLE_RUN_DEDUP, client_1.RunStatus.AWAITING_INPUT, client_1.RunStatus.INTERRUPTED],
                        },
                    },
                    orderBy: { createdAt: 'desc' },
                ...
                if (!holder)
                    throw e;
                throw new common_1.ConflictException(`task ${(0, shared_1.uuidToBase62)(task.id)} could not be started: session ` +
                    `${(0, shared_1.uuidToBase62)(holder.id)} (${holder.status}` +
            ...
            if (task.provider && winner.provider !== task.provider) {
                throw new common_1.ConflictException(`task ${(0, shared_1.uuidToBase62)(task.id)} is pinned to ${task.provider}, but session ` +
                    `${(0, shared_1.uuidToBase62)(winner.id)} holds its execution claim on ${winner.provider}. Let that ` +
            ...
            if (task.model != null && winner.model !== task.model) {
                throw new common_1.ConflictException(`task ${(0, shared_1.uuidToBase62)(task.id)} is pinned to model ${task.model}, but session ` +
                    `${(0, shared_1.uuidToBase62)(winner.id)} holds its execution claim on ` +
            ...
            if (!winner.startsTaskWork || winner.cancelRequestedAt) {
                throw new common_1.ConflictException(`task ${(0, shared_1.uuidToBase62)(task.id)} could not be started: session ` +
                    `${(0, shared_1.uuidToBase62)(winner.id)} holds its execution claim and is ` +
            ...
            this.logger.log(`task ${(0, shared_1.uuidToBase62)(task.id)}: lost the execution claim to session ` +
                `${(0, shared_1.uuidToBase62)(winner.id)}; returning it`);
            return winner;
        }
    }
    /**
     ...
     */
    async deliverMentions(now = new Date()) {
        const holder = `${process.pid}:${(0, crypto_1.randomUUID)()}`;
           ...
           WHERE "id" = ${row.id}::uuid
             AND "status" = 'DELIVERING' AND "lease_holder" = ${holder}`;
                if (settled === 0) {
                    ...
                    this.realtime.publishForUser(row.ownerId, shared_1.RunEventType.TASK_CHANGED, row.taskId);
                }
            }
            catch (error) {
     ...
     */
    async parkMentionDelivery(delivery, holder, error) {
        ...
        const [parked] = await this.prisma.$queryRaw(client_1.Prisma.sql `
        UPDATE "task_comment_mention_delivery"
      ...
      `);
        if (!parked)
            return;
        this.realtime.publishForUser(delivery.ownerId, shared_1.RunEventType.TASK_CHANGED, delivery.taskId);
        this.logger.warn(`mention delivery ${delivery.id} ${parked.status === 'DEAD' ? 'gave up' : 'parked'} ` +
    ...
    delivery, holder) {
        ...
        const [comment] = await this.prisma.$queryRaw(client_1.Prisma.sql `
    ...
    `);
        if (!comment)
            throw new MentionUndeliverable('COMMENT_GONE', 'the comment no longer exists');
        if (!comment.workspaceExists) {
            ...
            throw new MentionUndeliverable('WORKSPACE_GONE', 'the mentioned agent no longer exists', false, 'mention an agent that still exists, in a new comment');
        }
        if (!comment.workspaceEnabled) {
            ...
            throw new MentionUndeliverable('WORKSPACE_DISABLED', 'the mentioned agent is disabled', true, 're-enable this agent; the mention then delivers itself with no further action');
        }
        if (!comment.runnerId) {
            // Availability, not failure: binding a runner clears it, and the budget is not spent.
            throw new MentionUndeliverable('NO_RUNNER', 'the mentioned agent is not bound to a runner', true, 'bind this agent to a runner; the mention then delivers itself with no further action');
        }
        ...
        // still failed.
        const publicTaskId = (0, shared_1.uuidToBase62)(delivery.taskId);
        const prompt = `你在任务「${comment.taskTitle}」(${publicTaskId}) 的评论区被 @ 提到。\n\n` +
            `评论内容：\n${comment.body}\n\n` +
        ...
        if (remembered) {
            ...
            const receiptKey = ours
                ? sessions_service_1.SessionsService.initialTurnClientId(remembered.id)
                : clientTurnId;
            ...
            if (already) {
                ...
                if (consumed)
                    return { sessionId: remembered.id, turnId: already.id, consumed: true };
                const alive = !remembered.deletedAt
                    && (sessions_service_1.SessionsService.LIVE.includes(remembered.status)
                        || remembered.status === client_1.RunStatus.PENDING);
                if (alive)
                    return { sessionId: remembered.id, turnId: already.id, consumed: false };
                throw new MentionUndeliverable('TURN_STRANDED', `the run this mention was queued in (${(0, shared_1.uuidToBase62)(remembered.id)}) ended before ` +
                    'reading it', true, 'no action needed — the mention will be re-delivered into a fresh conversation');
            }
            // No receipt yet.
            //
            ...
            const live = !remembered.deletedAt
                && (sessions_service_1.SessionsService.LIVE.includes(remembered.status)
                    || remembered.status === client_1.RunStatus.PENDING);
            if (ours && live)
                return { sessionId: remembered.id, turnId: null, consumed: false };
            if (live) {
            ...
            const rotated = (0, crypto_1.randomUUID)();
            const moved = await this.prisma.$executeRaw `
        UPDATE "task_comment_mention_delivery"
           SET "desired_session_id" = ${rotated}::uuid,
               "target_session_id" = NULL, "turn_id" = NULL,
               "last_error" = ${`ROTATED: ${(0, shared_1.uuidToBase62)(remembered.id)} ended (${remembered.status}) before carrying this mention`},
               "updated_at" = ${new Date()}
         WHERE "id" = ${delivery.id}::uuid
           AND "status" = 'DELIVERING' AND "lease_holder" = ${holder}`;
            if (moved === 0) {
        ...
        if (!rememberedId) {
            const liveWork = await this.prisma.session.findFirst({
                where: {
                    ...
                    deletedAt: null,
                    startsTaskWork: true,
                    status: { in: [...SINGLE_RUN_DEDUP, client_1.RunStatus.AWAITING_INPUT, client_1.RunStatus.INTERRUPTED] },
                },
                orderBy: { createdAt: 'desc' },
                select: { id: true },
            });
        ...
        const seed = await (0, workspace_provider_1.agentProviderSeed)(this.prisma, delivery.workspaceId);
        ...
        if (!Object.values(shared_1.AgentProvider).includes(seed.provider)) {
            ...
            if (!usable) {
                throw new MentionUndeliverable('PROVIDER_UNAVAILABLE', `this agent runs on "${seed.provider}", which is disabled or no longer configured`, true, `re-enable the "${seed.provider}" provider, or point this agent at one that is enabled; ` +
                    'the mention then delivers itself');
            }
        }
        const sessionId = delivery.desiredSessionId ?? (0, crypto_1.randomUUID)();
        await this.bindMentionTarget(delivery.id, holder, sessionId);
        try {
            await this.createMentionSession(delivery, sessionId, prompt, comment.taskTitle, seed, comment.workspaceModel);
        }
        catch (error) {
            ...
            if ((0, engine_signin_preflight_1.isEngineSignedOut)(error)) {
                throw new MentionUndeliverable('PROVIDER_UNAVAILABLE', `the ${error.runtime} engine is signed out on this agent's runner`, true, `sign in to ${error.runtime} on that runner; the mention then delivers itself`);
            }
            throw error;
        }
    ...
    async createMentionSession(delivery, sessionId, prompt, taskTitle, seed, model) {
        await this.sessions.create(delivery.ownerId, {
            prompt,
            workspaceId: delivery.workspaceId,
            ...
            source: 'user',
            dispatchOrigin: client_1.SessionDispatchOrigin.USER,
            runSource: client_1.SessionRunSource.MANUAL,
     ...
     */
    async enqueueMentionTurn(ownerId, sessionId, clientTurnId, prompt) {
        try {
            ...
            if (!(error instanceof sessions_service_1.SessionNotSendable)) {
                ...
                const spent = !now || now.deletedAt != null || now.cancelRequestedAt != null
                    || sessions_service_1.SessionsService.TERMINAL.includes(now.status);
                if (!spent)
                    throw error;
            }
            throw new MentionUndeliverable('TURN_STRANDED', `the run this mention was going into (${(0, shared_1.uuidToBase62)(sessionId)}) stopped accepting ` +
                `messages first: ${error instanceof Error ? error.message : String(error)}`, true, 'no action needed — the mention will be re-delivered into a fresh conversation');
        }
    }
    /**
     ...
     */
    async bindMentionTarget(deliveryId, holder, sessionId) {
     ...
     */
    async runWorkspaceOnTask(ownerId, task, workspace, prompt, newSessionTitle, 
    ...
    batch, provenance) {
        ...
        if (occupying?.startsTaskWork && !occupying.cancelRequestedAt)
            return occupying.id;
        if (occupying?.cancelRequestedAt && occupying.startsTaskWork) {
            throw new common_1.ConflictException(`task ${(0, shared_1.uuidToBase62)(task.id)} has a run that is ending (session ` +
                `${(0, shared_1.uuidToBase62)(occupying.id)}); wait for it to reach its terminal status, then start ` +
                'the task again');
        }
        ...
        const latest = await this.prisma.session.findFirst({
            where: {
                ...
                ownerId,
                deletedAt: null,
                status: { in: [client_1.RunStatus.AWAITING_INPUT, client_1.RunStatus.INTERRUPTED] },
        ...
        if (latest && task.provider && task.provider !== latest.provider) {
            ...
            throw new common_1.ConflictException(`task ${(0, shared_1.uuidToBase62)(task.id)} has a paused run on ${latest.provider} and is now pinned to ` +
        ...
        if (latest) {
            try {
                await this.sessions.resume(ownerId, latest.id, {
        ...
        const session = await this.createTaskSessionOrReadWinner(ownerId, task, workspace, {
            prompt,
            workspaceId: workspace.id,
            taskId: task.id,
        ...
        // explicit so a future default change cannot silently move them back to System.
        {
            source: 'user',
            batch,
            dispatchOrigin: provenance?.dispatchOrigin ?? client_1.SessionDispatchOrigin.USER,
            runSource: provenance?.runSource ?? client_1.SessionRunSource.MANUAL,
     ...
     */
    async recordManualProjectTriggers(ownerId, projectIds) {
        const distinct = [...new Set(projectIds.filter((id) => !!id))].sort();
    ...
    async execute(ownerId, id, auto) {
        ...
        const refusal = (0, task_supersession_1.taskWorkRefusal)({
        ...
        }, shared_1.uuidToBase62);
        if (refusal) {
            if (auto)
                return { ok: false, skipped: 'superseded' };
            throw new common_1.ConflictException(`task ${(0, shared_1.uuidToBase62)(id)}: ${refusal}`);
        }
        ...
        if ((0, task_aggregation_1.isAggregateParent)({
            completionPolicy: task.completionPolicy,
            hasDirectChildren: task.children.length > 0,
        })) {
            if (auto)
                return { ok: false, skipped: 'aggregate-parent' };
            throw new common_1.ConflictException(`task ${(0, shared_1.uuidToBase62)(id)}: this task is completed by aggregating its subtasks `
        ...
        if (auto && task.dispatchAuthority === 'COORDINATOR') {
            return { ok: false, skipped: 'coordinator-authority' };
        }
        ...
        if (auto && !autoDispatchStillValid(task.runAt, auto.observedRunAt)) {
            return { ok: false, skipped: 'rescheduled' };
        }
        const depFacts = (await this.dependencyFactsFor([id])).get(id) ?? [];
        const depState = (0, task_dependencies_1.computeDependencyState)(depFacts);
        if (!(0, task_dependencies_1.canRun)(depState)) {
            ...
            throw new common_1.BadRequestException(gate
                ? `A prerequisite cannot be treated as complete: ${(0, task_dependencies_1.verificationGateMessage)(gate)}`
        ...
        if (task.dispatchHold) {
            throw new common_1.BadRequestException('This task list is paused — resume it before running');
        }
        if (!task.assignee)
            throw new common_1.BadRequestException('Assign a responsible workspace to the task first');
        if (!task.assignee.runnerId)
            throw new common_1.BadRequestException('The responsible workspace is not bound to a runner, cannot run');
        ...
        const sessionId = await this.runTaskWorkTranslatingFences(() => this.runWorkspaceOnTask(ownerId, task, { id: task.assignee.id, runnerId: task.assignee.runnerId }, prompt, `执行任务：${task.title}`, 
        ...
        task.listId && task.list?.maxConcurrent != null
            ...
            : undefined, auto
            ? {
                dispatchOrigin: client_1.SessionDispatchOrigin.LEGACY_SWEEP,
                runSource: client_1.SessionRunSource.TASK_LIST_AUTO,
                startsTaskWork: true,
            }
            : {
        ...
        if (!auto && task.projectId) {
            await this.recordManualProjectTriggers(ownerId, [task.projectId]).catch((e) => this.logger.warn(`manual project trigger for task ${id} failed: ${e?.message ?? e}`));
        }
        if (task.status === shared_1.TaskStatus.FAILED) {
            await this.clearFailedForRetry(ownerId, task.id).catch((e) => this.logger.warn(`clearFailedForRetry for task ${id} failed: ${e?.message ?? e}`));
        }
        ...
        // its schedule first.
        await this.consumeRunAt(ownerId, task.id, task.runAt).catch((e) => this.logger.warn(`consumeRunAt for task ${id} failed: ${e?.message ?? e}`));
     ...
     */
    async consumeRunAt(ownerId, taskId, runAt) {
        if (!runAt)
            return;
     ...
     */
    async clearFailedForRetry(ownerId, taskId) {
        ...
        const res = await this.prisma.task.updateMany({
            ...
            data: { status: shared_1.TaskStatus.IN_PROGRESS },
        });
        if (res.count > 0) {
            this.realtime.publishForUser(ownerId, shared_1.RunEventType.TASK_CHANGED, taskId);
        }
     ...
     */
    async runTaskWorkTranslatingFences(run) {
        try {
            return await run();
        }
        catch (error) {
            const fenced = (0, task_supersession_1.taskFenceConflictMessage)(error);
            if (fenced)
                throw new common_1.ConflictException(fenced);
            throw error;
        }
    }
    async batchExecute(ownerId, taskIds, maxConcurrent) {
        const tasks = await this.prisma.task.findMany({
        ...
        const occupied = new Set((await this.prisma.session.findMany({
        ...
        for (const t of tasks) {
            ...
            const workRefusal = (0, task_supersession_1.taskWorkRefusal)({
            ...
            const retirement = (0, task_supersession_1.taskRetirement)(t);
            ...
            if (workRefusal) {
                skipped.push({
                    ...
                    reason: retirement
                        ? (t.supersededByTaskId
                            ? `Superseded by ${(0, shared_1.uuidToBase62)(t.supersededByTaskId)}`
            ...
            else if ((0, task_aggregation_1.isAggregateParent)({
                ...
                skipped.push({
                    ...
                    title: t.title,
                    reason: gate
                        ? `Prerequisite not verified: ${(0, task_dependencies_1.verificationGateMessage)(gate)}`
        ...
        const batch = maxConcurrent != null ? { id: (0, crypto_1.randomUUID)(), maxConcurrent } : undefined;
        const dispatch = async (t) => {
            try {
                const sessionId = await this.runTaskWorkTranslatingFences(() => this.runWorkspaceOnTask(ownerId, t, { id: t.assignee.id, runnerId: t.assignee.runnerId }, this.buildExecutePrompt(t), `执行任务：${t.title}`, batch));
                ...
                if (t.status === shared_1.TaskStatus.FAILED) {
                    await this.clearFailedForRetry(ownerId, t.id).catch((e) => this.logger.warn(`batchExecute: clearFailedForRetry for ${t.id} failed: ${e}`));
                }
                ...
                await this.consumeRunAt(ownerId, t.id, t.runAt).catch((e) => this.logger.warn(`batchExecute: consumeRunAt for ${t.id} failed: ${e}`));
                return { id: t.id, ok: true, sessionId };
            }
            catch (e) {
        ...
        const results = new Array(runnable.length);
        let nextIndex = 0;
        const worker = async () => {
            for (;;) {
        ...
        await Promise.all(Array.from({ length: Math.min(exports.BATCH_EXECUTE_DISPATCH_CONCURRENCY, runnable.length) }, () => worker()));
        ...
        await this.recordManualProjectTriggers(ownerId, results
            .filter((result) => result.ok)
            .map((result) => runnable.find((task) => task.id === result.id)?.projectId ?? null)).catch((e) => this.logger.warn(`batchExecute: manual project triggers failed: ${e?.message ?? e}`));
        return {
     ...
     */
    async batchStop(ownerId, taskIds) {
        const sessions = await this.prisma.session.findMany({
            where: { ownerId, taskId: { in: taskIds }, status: { in: reclaim_stalled_task_1.TASK_OCCUPYING } },
     ...
     */
    async batchDelete(ownerId, taskIds) {
        const uniqueIds = [...new Set(taskIds)];
        if (uniqueIds.length === 0)
            return { deleted: 0 };
        const { deleted, survivingLinks } = await this.deleteAndStopRuns(ownerId, uniqueIds);
        if (deleted > 0) {
            this.realtime.publishForUser(ownerId, shared_1.RunEventType.TASK_CHANGED, uniqueIds[0]);
            ...
            if (survivingLinks.length) {
                await this.recomputeAggregates(ownerId, survivingLinks).catch((e) => this.logger.warn(`aggregation after batch delete failed: ${e?.message ?? e}`));
            }
        }
        return { deleted };
    }
    /** Set (or clear, when assigneeId is null) the responsible workspace on many tasks at once. */
    async batchAssign(ownerId, taskIds, assigneeId) {
        await this.assertOwnedWorkspace(ownerId, assigneeId);
        const ids = (0, lock_order_1.orderedIds)(taskIds);
        if (ids.length === 0)
            return { updated: 0 };
        ...
        const res = await (0, transaction_retry_1.withTransactionRetry)(this.prisma, async (tx) => {
            await this.lockDependencyGraph(tx, ownerId);
    ...
    async removeComment(ownerId, id, commentId) {
        await this.get(ownerId, id);
...
exports.TasksService = TasksService = TasksService_1 = __decorate([
    ...
    __metadata("design:paramtypes", [prisma_service_1.PrismaService, sessions_service_1.SessionsService, realtime_service_1.RealtimeService])
], TasksService);
