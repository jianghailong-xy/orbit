...
exports.MERGE_RECEIPT_MAX_CONFLICTS = exports.MERGE_RECEIPT_RECORDERS = exports.MERGE_RECEIPT_RESULTS = void 0;
exports.normalizeSha = normalizeSha;
exports.mergeReceiptIdempotencyKey = mergeReceiptIdempotencyKey;
exports.resultLanded = resultLanded;
exports.mergeStatusForResult = mergeStatusForResult;
exports.mergeReceiptRow = mergeReceiptRow;
 ...
 */
exports.MERGE_RECEIPT_RESULTS = ['MERGED', 'ALREADY_MERGED', 'CONFLICT', 'ERROR'];
/** WHO recorded it. Provenance, never an input to a decision. */
exports.MERGE_RECEIPT_RECORDERS = ['RUNNER', 'AGENT', 'USER'];
...
exports.MERGE_RECEIPT_MAX_CONFLICTS = 200;
...
function normalizeSha(value, field) {
    ...
    if (!SHA_RE.test(trimmed)) {
        throw new Error(`${field} must be a full 40-character git object name — an abbreviated SHA cannot be ` +
 ...
 */
function mergeReceiptIdempotencyKey(input) {
    return (0, node_crypto_1.createHash)('sha256')
        .update(['mr:v1', input.sessionId, input.sourceSha.toLowerCase(), input.targetBranch, input.result].join(' '))
        .digest('hex');
}
 ...
 */
function mergeStatusForResult(result) {
    if (resultLanded(result))
        return 'merged';
    return result === 'CONFLICT' ? 'conflict' : 'error';
}
function mergeReceiptRow(r) {
    return {
        id: r.id,
        ...
        rebaseBaseShaAbsentReason: r.rebaseBaseSha === null ? 'NOT_REBASED' : null,
...
//# sourceMappingURL=merge-receipt.js.map