...
exports.SessionsService = exports.SessionNotSendable = void 0;
...
const attachments_media_1 = require("../attachments/attachments.media");
const session_tags_service_1 = require("../session-tags/session-tags.service");
const naming_1 = require("./naming");
const search_query_1 = require("./search-query");
const system_noise_1 = require("../common/system-noise");
...
const preset_overlay_1 = require("../providers/preset-overlay");
const session_inbox_fence_1 = require("../common/session-inbox-fence");
const task_aggregation_1 = require("../projects/task-aggregation");
const truncate_payload_1 = require("./truncate-payload");
const engine_signin_preflight_1 = require("./engine-signin-preflight");
...
function assertPromptSize(text, field) {
    if (text.length > shared_1.MAX_PROMPT_CHARS) {
        throw new common_1.BadRequestException(`${field} is too long: ${text.length} characters (max ${shared_1.MAX_PROMPT_CHARS})`);
    }
}
/**
 ...
 */
function assertKnownPermissionMode(mode) {
    if (mode === undefined)
        return;
    const modes = Object.values(shared_1.PermissionMode);
    if (!modes.includes(mode)) {
        throw new common_1.BadRequestException(`unknown permissionMode "${mode}"; use one of: ${modes.join(', ')}`);
    }
}
function runnerScopeWhere(scope) {
    if (!scope)
        return {};
    return {
...
function legacyArtifactMime(filePath) {
...
function legacyArtifactDisposition(filename) {
 ...
 */
const stripMarks = (col) => client_1.Prisma.sql `replace(replace(${col}, '*', ''), '\`', '')`;
 ...
 */
const eventBodySql = (id) => client_1.Prisma.sql `
  body AS (
    SELECT
      seq, type, created_at, payload,
      ${stripMarks(client_1.Prisma.sql `
        concat_ws(' ',
          payload->>'text',
          payload->>'name',
          (payload->'input')::text,
...
const eventMatchSql = (norm) => client_1.Prisma.join(norm.patterns.map((term) => client_1.Prisma.sql `(${client_1.Prisma.join(term.map((p) => client_1.Prisma.sql `text ILIKE ${p}`), ' OR ')})`), ' AND ');
/**
 ...
 */
class SessionNotSendable extends common_1.ConflictException {
}
exports.SessionNotSendable = SessionNotSendable;
let SessionsService = class SessionsService {
    static { SessionsService_1 = this; }
    prisma;
    queue;
    realtime;
    logger = new common_1.Logger(SessionsService_1.name);
    constructor(prisma, queue, realtime) {
        this.prisma = prisma;
        this.queue = queue;
     ...
     */
    async assertOwnedRefs(ownerId, refs) {
        if (refs.assignedRunnerId) {
            ...
            if (!runner)
                throw new common_1.ForbiddenException('runner not found');
        }
        if (refs.workspaceId) {
    ...
    async create(ownerId, dto, opts) {
        ...
        let assignedRunnerId = dto.assignedRunnerId;
        ...
        let provider = shared_1.AgentProvider.CLAUDE;
        ...
        const accountPermissionMode = dto.permissionMode
            ? undefined
            : (0, permission_mode_1.accountDefaultPermissionMode)(await this.prisma.user.findUnique({
                where: { id: ownerId },
        ...
        if (!assignedRunnerId && dto.workspaceId) {
            ...
            if (workspace.enabled === false)
                throw new common_1.ForbiddenException('workspace is disabled');
        ...
        if (!assignedRunnerId) {
            throw new common_1.BadRequestException('pick a workspace bound to a runner, or pass assignedRunnerId');
        }
        ...
        if (!dto.provider && dto.workspaceId) {
            ({ provider, providerBuiltin } = await (0, workspace_provider_1.agentProviderSeed)(this.prisma, dto.workspaceId));
        }
        ...
        if (dto.provider) {
            ...
            providerBuiltin = Object.values(shared_1.AgentProvider).includes(dto.provider);
            if (!providerBuiltin) {
        ...
        let rootRefusedFallback;
        const requestedMode = dto.permissionMode ?? accountPermissionMode;
        if (requestedMode && shared_1.ROOT_REFUSED_PERMISSION_MODES.has(requestedMode)) {
            ...
            if (target?.runsAsRoot && dto.permissionMode) {
                throw new common_1.BadRequestException(`runner "${target.name}" runs as root, and Claude Code refuses "${dto.permissionMode}" ` +
                    `under root — the session would exit before its first turn. Use ` +
                    `"${shared_1.ROOT_FALLBACK_PERMISSION_MODE}", which also never asks.`);
            }
            ...
            if (target?.runsAsRoot)
                rootRefusedFallback = shared_1.ROOT_FALLBACK_PERMISSION_MODE;
        }
        ...
        const title = explicitTitle ?? (0, naming_1.titleFromPrompt)(dto.prompt);
        let branch = enableWorktree ? (0, naming_1.makeBranchName)(title) : null;
        ...
        const runtime = borrowedRuntime
            ? (0, runtime_provider_1.normalizeRuntimeProvider)(borrowedRuntime)
            : (0, runtime_provider_1.normalizeRuntimeProvider)(provider, providerBuiltin);
        ...
        const refusal = targetRunner &&
            (0, engine_signin_preflight_1.signedOutEngineRefusal)({
                runtime,
        ...
        if (refusal)
            throw new engine_signin_preflight_1.EngineSignedOutConflict(runtime, refusal);
        ...
        const runtimeSessionId = (0, crypto_1.randomUUID)();
        const session = await this.prisma.session.create({
            data: {
                ...
                branch,
                prompt: dto.prompt,
                status: client_1.RunStatus.PENDING,
                provider,
                providerBuiltin,
                ...
                runtimeSessionId: runtime === shared_1.AgentProvider.CLAUDE ? runtimeSessionId : null,
                model: dto.model,
                ...
                effort: (0, runtime_provider_1.normalizeEffortForProvider)(runtime, dto.effort),
                ...
                dispatchOrigin: opts?.dispatchOrigin ?? client_1.SessionDispatchOrigin.USER,
                runSource: opts?.runSource ?? client_1.SessionRunSource.MANUAL,
        ...
        if (dto.shell) {
            await this.insertTurn(session.id, {
                kind: 'shell',
                content: dto.prompt,
                clientTurnId: SessionsService_1.initialTurnClientId(session.id),
            });
        }
        ...
        return (0, session_state_1.withSessionState)(session);
    }
    /**
     ...
     */
    async beautifySessionLater(ownerId, sessionId, prompt, fallbackTitle) {
        try {
            ...
            const known = await this.prisma.sessionTag.findMany({
                ...
                take: naming_1.MAX_KNOWN_TAGS_PROMPTED,
            });
            const { title, tags } = await (0, naming_1.enqueueBeautifySession)({
                prompt,
     ...
     * neither half.
     */
    async applyAutoTags(ownerId, sessionId, names, known) {
        ...
        if ((await this.prisma.sessionTagLink.count({ where: { sessionId } })) > 0)
            return false;
        const byName = new Map(known.map((t) => [t.name.toLowerCase(), t.id]));
        const tagIds = names.map((name) => byName.get(name.toLowerCase())).filter((id) => !!id);
        ...
        const room = naming_1.MAX_KNOWN_TAGS_PROMPTED - known.length;
     ...
     */
    async createAutoTags(ownerId, names) {
        const agg = await this.prisma.sessionTag.aggregate({
            where: { ownerId },
            _max: { position: true },
        });
        const start = (agg._max.position ?? session_tags_service_1.SESSION_TAG_PALETTE.length - 1) + 1;
        await this.prisma.sessionTag.createMany({
            data: names.map((name, i) => ({
                name,
                ownerId,
                isSystem: false,
                color: session_tags_service_1.SESSION_TAG_PALETTE[(start + i) % session_tags_service_1.SESSION_TAG_PALETTE.length],
                position: start + i,
            })),
            skipDuplicates: true,
        });
        ...
        if (rows.length > 0)
            this.realtime.publishForUser(ownerId, shared_1.RunEventType.TAG_CHANGED, rows[0].id);
    ...
    static MAX_SPAWN_DEPTH = 5;
    /** Rolling window for {@link SPAWN_TREE_RATE}. */
    static SPAWN_RATE_WINDOW_MS = 60 * 60_000;
    /**
     ...
     */
    async spawnFromSession(ownerId, parentSessionId, dto) {
        if (!dto.prompt)
            throw new common_1.BadRequestException('prompt is required');
        ...
        if (!parent)
            throw new common_1.NotFoundException('parent session not found');
        if (!parent.workspace?.enableOrchestration) {
            throw new common_1.ForbiddenException('orchestration is not enabled for this workspace');
        }
        if (parent.spawnDepth >= SessionsService_1.MAX_SPAWN_DEPTH) {
            throw new common_1.ForbiddenException(`spawn depth limit (${SessionsService_1.MAX_SPAWN_DEPTH}) reached`);
        }
        ...
        const outstanding = await this.prisma.session.count({
            where: { rootSessionId, status: { in: session_scheduling_1.UNSETTLED_SESSION_STATUSES } },
        });
        if (outstanding >= session_scheduling_1.SPAWN_TREE_OUTSTANDING) {
            throw new common_1.HttpException(`this run already has ${outstanding} unfinished sessions (limit ${session_scheduling_1.SPAWN_TREE_OUTSTANDING}); ` +
                `wait for some to finish before starting more`, common_1.HttpStatus.TOO_MANY_REQUESTS);
        }
        ...
        const startedThisHour = await this.prisma.session.count({
            where: {
                rootSessionId,
                createdAt: { gt: new Date(Date.now() - SessionsService_1.SPAWN_RATE_WINDOW_MS) },
            },
        });
        if (startedThisHour >= SessionsService_1.SPAWN_TREE_RATE) {
            throw new common_1.HttpException(`this run has started ${startedThisHour} sessions in the past hour ` +
                `(limit ${SessionsService_1.SPAWN_TREE_RATE}); it is looping rather than making progress`, common_1.HttpStatus.TOO_MANY_REQUESTS);
        }
        ...
        const workspaceId = wantId ?? (wantName ? await this.resolveWorkspaceByName(ownerId, wantName) : undefined);
    ...
    async resolveDefaultEffort(ownerId, workspaceId) {
        if (workspaceId) {
            const workspace = await this.prisma.workspace.findFirst({
            ...
            if (workspace && workspace.effort !== null)
                return workspace.effort;
        }
    ...
    async resolveWorkspaceByName(ownerId, name) {
        ...
        if (matches.length === 0)
            throw new common_1.BadRequestException(`no workspace named "${name}"`);
        if (matches.length > 1)
            throw new common_1.BadRequestException(`multiple workspaces named "${name}"; use workspaceId`);
        return matches[0].id;
    }
    /**
     ...
     */
    async assertHostedByRunner(ownerId, scope, id) {
     ...
     */
    async spawnForServiceToken(ownerId, scope, dto) {
        if (!dto.prompt)
            throw new common_1.BadRequestException('prompt is required');
        assertKnownPermissionMode(dto.permissionMode);
        const workspace = await this.prisma.workspace.findFirst({
        ...
        if (!workspace)
            throw new common_1.NotFoundException('workspace not found on this runner');
        ...
        const created = await this.create(ownerId, {
            ...
            permissionMode: dto.permissionMode,
            effort,
        }, { batch: { id: scope.tokenId, maxConcurrent: session_scheduling_1.SERVICE_TOKEN_CONCURRENCY } });
        return {
            id: created.id,
            status: created.status,
            runStatus: created.runStatus,
     ...
     *  callers. */
    async listForOrchestration(ownerId, filters) {
        ...
        return sessions.map((session) => (0, session_state_1.withSessionState)(session));
    }
    /**
     ...
     */
    async getForOrchestration(ownerId, id, scope) {
        const session = await this.prisma.session.findFirst({
    ...
    async search(ownerId, q, limit) {
        ...
        const norm = (0, search_query_1.normalizeSearchQuery)((0, search_query_1.stripEmphasis)(q ?? ''));
        if (!norm) {
        ...
        const wide = (0, search_query_1.broaden)(norm);
        if (!wide) {
     ...
     */
    async searchRows(ownerId, norm, take) {
        ...
        const matches = (col) => client_1.Prisma.sql `(${client_1.Prisma.join((norm?.patterns ?? [['']]).map((term) => client_1.Prisma.sql `(${client_1.Prisma.join(term.map((p) => client_1.Prisma.sql `${col} ILIKE ${p}`), ' OR ')})`), ' AND ')})`;
        ...
        const contentCte = norm && norm.searchContent
            ? client_1.Prisma.sql `
            ...
            SELECT session_id, ${stripMarks(client_1.Prisma.sql `raw_text`)} AS match_text
            FROM (
              ...
              WHERE s.owner_id = ${ownerId}::uuid
                ...
                AND ${matches(stripMarks(client_1.Prisma.sql `e.payload->>'text'`))}
              ORDER BY e.session_id, e.seq DESC
            ) c
          `
            : client_1.Prisma.sql `SELECT NULL::uuid AS session_id, NULL::text AS match_text WHERE false`;
        /**
         ...
         */
        const textField = (field, col) => ({
            field,
            proj: client_1.Prisma.sql `${stripMarks(col)} AS ${client_1.Prisma.raw(`"${field}"`)}`,
            test: matches(client_1.Prisma.raw(`x."${field}"`)),
            col: client_1.Prisma.raw(`x."${field}"`),
        });
        const fields = [
          ...
          CASE
            ...
            ELSE s.id::text
          END AS "id_text"`,
                test: client_1.Prisma.sql `x."id_hit"`,
                col: client_1.Prisma.sql `x."id_text"`,
            },
            textField('title', client_1.Prisma.sql `s.title`),
            ...(norm?.searchContent
                ? [
                    textField('prompt', client_1.Prisma.sql `s.prompt`),
                    textField('reply', client_1.Prisma.sql `s.last_assistant_text`),
                ]
                : []),
            textField('branch', client_1.Prisma.sql `s.branch`),
            textField('agent', client_1.Prisma.sql `a.name`),
            textField('task', client_1.Prisma.sql `t.title`),
        ];
        const matchFieldCase = client_1.Prisma.sql `CASE ${client_1.Prisma.join(fields.map((f) => client_1.Prisma.sql `WHEN ${f.test} THEN ${f.field}::text`), ' ')} END`;
        const matchTextCase = client_1.Prisma.sql `CASE ${client_1.Prisma.join(fields.map((f) => client_1.Prisma.sql `WHEN ${f.test} THEN ${f.col}`), ' ')} END`;
        const retest = client_1.Prisma.join(fields.map((f) => f.test), ' OR ');
        const projection = client_1.Prisma.join(fields.map((f) => f.proj), ',\n          ');
        ...
        const sessionPredicate = norm?.searchContent
            ? matches(stripMarks(client_1.Prisma.sql `
            coalesce(s.title, '') || ' ' ||
            coalesce(s.prompt, '') || ' ' ||
          ...
          `))
            : client_1.Prisma.sql `(
          ${matches(stripMarks(client_1.Prisma.sql `s.title`))}
          OR ${matches(stripMarks(client_1.Prisma.sql `s.branch`))}
        )`;
        const rows = norm
          ...
          WITH meta_ids AS (
            ...
            FROM session s
            WHERE s.owner_id = ${ownerId}::uuid
              AND replace(s.id::text, '-', '') LIKE ${norm.sessionIdPrefix ? `${norm.sessionIdPrefix}%` : null}
            UNION
            ...
            WHERE s.owner_id = ${ownerId}::uuid
              AND s.workspace_id IN (
                SELECT id FROM workspace WHERE ${matches(stripMarks(client_1.Prisma.sql `name`))}
              )
            UNION
            SELECT s.id FROM session s
            WHERE s.owner_id = ${ownerId}::uuid
              AND s.task_id IN (
                SELECT id FROM task WHERE ${matches(stripMarks(client_1.Prisma.sql `title`))}
              )
          ),
          meta AS (
            SELECT
        ...
        `)
          ...
          LIMIT ${take}::int
        `);
        const hits = rows.map((r) => (0, session_state_1.withSessionState)({
            id: r.id,
            title: r.title,
            status: r.status,
     ...
     */
    async workspaceSessionCounts(ownerId) {
        const open = {
            ownerId,
            completedAt: null,
            archivedAt: null,
        ...
        const [active, blocked] = await Promise.all([
            this.prisma.session.groupBy({
                by: ['workspaceId'],
                where: { ...open, status: { in: [client_1.RunStatus.RUNNING, client_1.RunStatus.PENDING] } },
                _count: { _all: true },
            }),
            ...
            this.prisma.session.findMany({
                where: { ...open, ...session_generating_1.GENERATING_SESSION_FILTER, approvals: { some: { status: 'PENDING' } } },
        ...
        for (const group of active) {
            if (group.workspaceId)
                row(group.workspaceId).active = group._count._all;
        }
    ...
    async list(ownerId, filters) {
        ...
        const view = filters.view === 'active'
            ...
            : filters.view === 'archived'
                ...
                : filters.view === 'deleted'
                    ? 'trash'
                    : (filters.view ?? 'open');
        const visibility = view === 'trash'
            ? client_1.Prisma.sql `s.deleted_at IS NOT NULL`
            : view === 'system'
                ? client_1.Prisma.sql `FALSE`
                : view === 'completed'
                    ? client_1.Prisma.sql `COALESCE(s.completed_at, s.archived_at) IS NOT NULL AND s.deleted_at IS NULL`
                    : client_1.Prisma.sql `COALESCE(s.completed_at, s.archived_at) IS NULL AND s.deleted_at IS NULL`;
        const runnerFilter = filters.runnerId
            ? client_1.Prisma.sql `AND s.assigned_runner_id = ${filters.runnerId}::uuid`
        ...
        const workspaceFilter = filters.workspaceId
            ? client_1.Prisma.sql `AND s.workspace_id = ${filters.workspaceId}::uuid`
            : client_1.Prisma.empty;
        ...
        const pageLimit = typeof filters.limit === 'number' && Number.isFinite(filters.limit) && filters.limit > 0
            ? client_1.Prisma.sql `LIMIT ${Math.floor(filters.limit)}::int`
            : client_1.Prisma.empty;
        ...
        const orderBy = view === 'completed'
            ? client_1.Prisma.sql `COALESCE(s.completed_at, s.archived_at) DESC NULLS LAST, s.created_at DESC`
            : client_1.Prisma.sql `(s.pinned_at IS NOT NULL) DESC, COALESCE(s.last_turn_at, s.created_at) DESC, s.created_at DESC`;
      ...
      SELECT
        ...
        s.effort,
        left(s.last_assistant_text, ${SessionsService_1.PREVIEW_LEN}::int) AS "lastAssistantText",
        s.last_tool_use   AS "lastToolUse",
        left(s.last_user_text, ${SessionsService_1.PREVIEW_LEN}::int) AS "lastUserText",
      ...
      LEFT JOIN LATERAL (
        SELECT reason, active, "limit" FROM (
          SELECT
            CASE
              WHEN ${(0, session_tree_sql_1.runnerActiveTurns)(client_1.Prisma.sql `s.assigned_runner_id`)} >= r.max_concurrent
                THEN 'runner_at_capacity'
              WHEN s.root_session_id IS NOT NULL
                   AND ${(0, session_tree_sql_1.treeActiveTurns)('s')} >= ${(0, session_tree_sql_1.treeCeiling)(client_1.Prisma.sql `s.assigned_runner_id`)}
                THEN 'tree_at_capacity'
              WHEN s.batch_id IS NOT NULL
                   AND ${(0, session_tree_sql_1.batchActiveTurns)('s')} >= s.batch_max_concurrent
                THEN 'batch_at_capacity'
            END AS reason,
            ${(0, session_tree_sql_1.runnerActiveTurns)(client_1.Prisma.sql `s.assigned_runner_id`)} AS runner_active,
            r.max_concurrent AS runner_limit,
            ${(0, session_tree_sql_1.treeActiveTurns)('s')} AS tree_active,
            ${(0, session_tree_sql_1.treeCeiling)(client_1.Prisma.sql `s.assigned_runner_id`)} AS tree_limit,
            ${(0, session_tree_sql_1.batchActiveTurns)('s')} AS batch_active,
    ...
    `);
        ...
        const sessions = rows.map((r) => (0, session_state_1.withSessionCapabilities)({
            id: r.id,
            status: r.status,
            title: r.title,
            createdAt: r.createdAt,
        ...
        const generating = sessions.filter(session_generating_1.isSessionGenerating).map((s) => s.id);
        if (generating.length === 0)
            return sessions.map((s) => ({ ...s, pendingApprovals: 0 }));
    ...
    async get(ownerId, id) {
        ...
        const projected = session.provider === 'claude'
            ...
            ? { ...session, runtimeSessionId: SessionsService_1.RESUMABLE_PROJECTION, numTurns: 0 }
            : session;
        ...
        return (0, session_state_1.withSessionCapabilities)({ ...rest, tags });
    }
    /**
    ...
    async enableShare(ownerId, id) {
        ...
        const updated = await this.prisma.session.update({
            where: { id },
            data: { shareToken: (0, crypto_1.randomBytes)(24).toString('base64url'), sharedAt: new Date() },
            select: { shareToken: true, sharedAt: true },
        });
     ...
     */
    async cancelAutoRetry(ownerId, id) {
        const session = await this.prisma.session.findFirst({
            where: { id, ownerId },
            select: { id: true },
        });
     ...
     */
    async armAutoRetry(ownerId, id, retryAt) {
        const at = new Date(retryAt);
        const now = Date.now();
        if (Number.isNaN(at.getTime()) || at.getTime() <= now)
            throw new common_1.BadRequestException('retryAt must be a future instant');
        if (at.getTime() - now > MAX_ARM_AHEAD_MS)
            throw new common_1.BadRequestException('retryAt is too far out');
        ...
        const armed = await (0, transaction_retry_1.withTransactionRetry)(this.prisma, async (tx) => {
            const [target] = await tx.$queryRaw(client_1.Prisma.sql `
      ...
      `);
          ...
          `);
                ...
                const [locked] = await tx.$queryRaw(client_1.Prisma.sql `
        ...
        `);
                ...
                if ((locked?.projectId ?? null) !== (target.projectId ?? null)) {
                    throw new common_1.ConflictException('this task changed project while the request was being prepared — nothing was '
                        + 'changed; retry');
                }
                ...
                const refusal = await this.taskWorkRefusalFor(tx, target.taskId, false, target.startsTaskWork);
                if (refusal) {
                    throw new common_1.ConflictException(`this retry cannot be armed: ${refusal}`);
                }
            }
            return tx.session.updateMany({
                where: {
                    ...
                    OR: [
                        { status: client_1.RunStatus.AWAITING_INPUT, cancelRequestedAt: null },
                        { status: client_1.RunStatus.FAILED },
                    ],
                },
                data: { retryAt: at },
            });
        }, (0, transaction_retry_1.loggedRetry)(this.logger, 'sessions.armAutoRetry'));
        if (!armed.count)
            throw new common_1.BadRequestException('session is not waiting on a retry');
        return { retryAt: at };
    }
    /**
    ...
    async getShared(token) {
        ...
        if (!session)
            throw new common_1.NotFoundException('shared session not found');
        const stateful = (0, session_state_1.withSessionState)(session);
     ...
     */
    async getEventPage(userId, id, opts) {
        const session = await this.prisma.session.findFirst({
        ...
        const before = typeof opts.before === 'number' && Number.isFinite(opts.before)
            ? client_1.Prisma.sql `AND seq < ${Math.trunc(opts.before)}`
            : client_1.Prisma.empty;
      ...
      WHERE session_id = ${id}::uuid
        ${before}
        AND ${system_noise_1.notNoiseSql}
      ORDER BY seq DESC
      LIMIT ${take + 1}
        ...
        return {
            ...
            events: page.map((e) => {
                const cut = opts.maxPayload
                    ? (0, truncate_payload_1.truncatePayload)(e.type, e.payload, opts.maxPayload)
     ...
     */
    async searchEvents(userId, id, q, limit) {
        const session = await this.prisma.session.findFirst({
            where: { id, ownerId: userId },
            select: { id: true },
        });
     ...
     */
    async getEventFull(userId, id, seq) {
        const session = await this.prisma.session.findFirst({
            where: { id, ownerId: userId },
            select: { id: true },
        });
     ...
     */
    async getBackgroundShells(userId, id) {
        const session = await this.prisma.session.findFirst({
            where: { id, ownerId: userId },
        ...
        const sessionLive = !SessionsService_1.TERMINAL_STATUSES.includes(session.status);
        return (0, shared_1.deriveBackgroundShells)(rows.map((e) => ({
            seq: e.seq,
            type: e.type,
            payload: e.payload,
     ...
     */
    async selectBackgroundEvents(id) {
        const calls = await this.prisma.$queryRaw `
      SELECT seq, type, payload, created_at AS "createdAt"
      FROM run_event
      WHERE session_id = ${id}::uuid
        AND (
          type IN (${shared_1.RunEventType.BACKGROUND_TASK}, ${shared_1.RunEventType.BACKGROUND_OUTPUT})
          OR (
            type = ${shared_1.RunEventType.TOOL_USE}
            AND (
    ...
    `;
        const toolUseIds = [
            ...new Set(calls
                .filter((r) => r.type === shared_1.RunEventType.TOOL_USE)
                .map((r) => r.payload?.id)
                .filter((v) => v != null)
                .map(String)),
        ];
      ...
      FROM run_event
      WHERE session_id = ${id}::uuid
        AND type = ${shared_1.RunEventType.TOOL_RESULT}
        AND payload->>'toolUseId' IN (${client_1.Prisma.join(toolUseIds)})
      ORDER BY seq ASC
    `;
        return [...calls, ...results].sort((a, b) => a.seq - b.seq);
    }
    async getLegacyArtifactForOwner(ownerId, id, rawPath) {
        const session = await this.prisma.session.findFirst({
            where: { id, ownerId },
            select: { id: true },
        });
    ...
    async getLegacyArtifactForShared(token, rawPath) {
        const session = await this.prisma.session.findFirst({
            where: { shareToken: token, deletedAt: null },
            select: { id: true },
        });
    ...
    async getLegacyArtifact(sessionId, rawPath) {
        ...
        if (!mentioned)
            throw new common_1.NotFoundException('artifact not found');
        const filename = path_1.default.basename(resolved.file);
        ...
        let st;
        try {
            st = await fs_1.promises.stat(localFile);
        }
        catch {
            return this.requestAndWaitForLegacyArtifact(sessionId, resolved.original, filename);
        }
        if (!st.isFile() || st.size <= 0 || st.size > attachments_media_1.MAX_UPLOAD_BYTES) {
            return this.requestAndWaitForLegacyArtifact(sessionId, resolved.original, filename);
        }
        const data = await fs_1.promises.readFile(localFile);
        const local = {
            data,
    ...
    async requestAndWaitForLegacyArtifact(sessionId, artifactPath, filename) {
    ...
    async enqueueLegacyArtifactRequest(sessionId, artifactPath) {
        const key = (0, crypto_1.createHash)('sha256').update(artifactPath).digest('hex').slice(0, 32);
    ...
    async resolveLegacyArtifactPath(sessionId, rawPath) {
        ...
        if (!path_1.default.isAbsolute(decoded) || decoded.split(/[\\/]+/).includes('..')) {
            throw new common_1.NotFoundException('artifact not found');
        }
        const normalized = path_1.default.normalize(decoded);
        const parts = normalized.split(path_1.default.sep).filter(Boolean);
        const marker = parts.findIndex((part, i) => part === '.orbit' && parts[i + 1] === 'uploads' && parts[i + 2] === sessionId);
        if (marker < 0 || parts.length <= marker + 3) {
            throw new common_1.NotFoundException('artifact not found');
        }
        const root = path_1.default.join(path_1.default.sep, ...parts.slice(0, marker + 3));
        return { original: decoded, file: normalized, root };
    }
    async resolveExistingLocalArtifactFile(root, file) {
        let realRoot;
        let realFile;
        try {
            realRoot = await fs_1.promises.realpath(root);
            realFile = await fs_1.promises.realpath(file);
        }
        catch {
            return null;
        }
        if (realFile !== realRoot && !realFile.startsWith(realRoot + path_1.default.sep)) {
            throw new common_1.NotFoundException('artifact not found');
        }
        return realFile;
    }
    async legacyArtifactPathIsMentioned(sessionId, artifactPath) {
    ...
    async getLegacyArtifactAttachment(sessionId, filename) {
    ...
    async persistLegacyArtifactAttachment(sessionId, filename, mimeType, data) {
        ...
        await this.prisma.attachment.create({
            data: {
                ...
                fileName: filename,
                data: (0, attachments_media_1.toBytes)(data),
            },
        });
    }
    /**
     ...
     */
    async requestDiffRefresh(ownerId, id) {
        const session = await this.prisma.session.findFirst({
            where: { id, ownerId },
            select: { id: true, status: true },
        });
        if (!session)
            throw new common_1.NotFoundException('session not found');
        if (!SessionsService_1.LIVE.includes(session.status))
            return;
        ...
        if (!pending) {
            await this.insertTurn(id, { kind: 'diff', clientTurnId: (0, crypto_1.randomUUID)() });
        }
    ...
    static resumeBlocked(reason) {
        switch (reason) {
            case 'TRASHED':
                return new common_1.ConflictException('the session is in Trash; restore it before sending a message');
            case 'ENDING':
                return new common_1.ConflictException('the session is ending');
            case 'NOT_TERMINAL':
                return new common_1.ConflictException('the session has not started yet');
            case 'NOT_STARTED':
            case 'MISSING_CONTEXT':
                return new common_1.ConflictException('this session never ran and cannot be resumed');
            case 'NO_RUNNER':
                return new common_1.ConflictException('the session has no runner to resume on');
            case 'RUNNER_OFFLINE':
                return new common_1.ConflictException('the runner is offline; it must be online to resume this session');
        }
    }
    /** Load an owner's session and assert it's still live (not ended/cancelled). */
    async getLive(ownerId, id) {
        const session = await this.prisma.session.findFirst({ where: { id, ownerId } });
        if (!session)
            throw new common_1.NotFoundException('session not found');
        if (!SessionsService_1.LIVE.includes(session.status) || session.cancelRequestedAt) {
            throw new common_1.ConflictException('the session has ended');
        }
        return session;
    }
    /**
    ...
    async getSendable(ownerId, id) {
        ...
        if (SessionsService_1.TERMINAL.includes(session.status) || session.cancelRequestedAt) {
     ...
     */
    async ensurePromptSeeded(tx, session) {
    ...
    static initialTurnClientId(sessionId) {
     ...
     */
    async engineTurnInFlight(tx, sessionId) {
     ...
     */
    async runtimeTakesSteer(tx, session) {
        ...
        const runner = session.assignedRunnerId
            ...
            : null;
        return (0, shared_1.supportsMidTurnSteer)(await (0, custom_provider_1.sessionExecRuntime)(tx, session), runner?.capabilities);
    }
    async insertTurnLocked(tx, sessionId, data) {
        const existing = await tx.conversationTurn.findUnique({
     ...
     */
    async insertTurn(sessionId, data) {
        ...
        }, (0, transaction_retry_1.loggedRetry)(this.logger, 'sessions.insertTurn'));
    }
    /**
     ...
     */
    async assertLinkableAttachments(ownerId, sessionId, attachmentIds, tx = this.prisma) {
        ...
        if (found.length !== ids.length) {
            throw new common_1.BadRequestException('one or more attachments are unknown, not yours, or already attached');
        }
        return ids;
    }
    /**
     ...
     */
    async assertScopableAttachments(ownerId, attachmentIds) {
        const ids = [...new Set(attachmentIds ?? [])];
        if (ids.length === 0)
            return [];
        ...
        if (found.length !== ids.length) {
            throw new common_1.BadRequestException('one or more attachments are unknown, not yours, or already used');
        }
        return ids;
    }
    ...
    async linkAttachments(turnId, attachmentIds, tx = this.prisma) {
        if (attachmentIds.length === 0)
            return;
        await tx.attachment.updateMany({
    ...
    async createTurn(ownerId, id, dto, opts) {
        ...
        const queued = await (0, transaction_retry_1.withTransactionRetry)(this.prisma, async (tx) => {
        ...
        FOR UPDATE`;
            ...
            if (existing) {
                return {
                    turn: existing,
                    wakeQueue: session.status === client_1.RunStatus.PENDING,
                    wakeInbox: session.status === client_1.RunStatus.RUNNING,
                };
            }
            ...
            const mergeExecuting = (0, session_inbox_fence_1.pendingWorktreeOperationMayBeExecuting)(session.mergeStatus, session.mergeOperationId, session.mergeOperationOwner, session.mergeRequestedAt);
            const commitExecuting = (0, session_inbox_fence_1.pendingWorktreeOperationMayBeExecuting)(session.commitStatus, session.commitOperationId, session.commitOperationOwner, session.commitRequestedAt);
            ...
            const nextStatus = (0, session_scheduling_1.statusAfterTurnEnqueued)(session.status);
            ...
            return {
                turn,
                wakeQueue: nextStatus === client_1.RunStatus.PENDING,
                wakeInbox: nextStatus === client_1.RunStatus.RUNNING,
            };
        }, (0, transaction_retry_1.loggedRetry)(this.logger, 'sessions.createTurn'));
        if (queued.wakeQueue)
            this.queue.notifySessionQueued();
        if (queued.wakeInbox)
            this.realtime.notifyInbox(id);
     ...
     */
    async interrupt(ownerId, id, dto) {
        const content = dto?.content ?? '';
        const followUp = content.trim().length > 0 || (dto?.attachmentIds?.length ?? 0) > 0;
        if (followUp) {
            assertPromptSize(content, 'message');
            if (!dto?.clientTurnId) {
                throw new common_1.BadRequestException('clientTurnId is required when interrupting with a follow-up');
            }
        }
        ...
        FOR UPDATE`;
            ...
            if (followUp && session.deletedAt) {
                throw new common_1.ConflictException('the session is in Trash; restore it before sending a message');
            }
            ...
            const interruptClientTurnId = followUp
                ? SessionsService_1.interruptClientId(dto.clientTurnId)
            ...
            if (session.status === client_1.RunStatus.RUNNING) {
                ...
                if (executable === 0) {
                    ...
                    throw new common_1.ConflictException('the next turn is already starting');
                }
            }
            ...
            if (!followUp)
                return { turn: null, wakeQueue: false };
            ...
            return { turn, wakeQueue: nextStatus === client_1.RunStatus.PENDING };
        }, (0, transaction_retry_1.loggedRetry)(this.logger, 'sessions.interrupt'));
        ...
        this.realtime.publishQueuedTurnsChanged(id);
        return queued.turn
            ? { ok: true, turnId: queued.turn.id, seq: queued.turn.seq }
            : { ok: true };
    }
    ...
    static interruptClientId(clientTurnId) {
    ...
    async listQueuedTurns(ownerId, id) {
        const session = await this.prisma.session.findFirst({
            where: { id, ownerId },
            select: { id: true },
        });
        ...
        const turns = await this.prisma.conversationTurn.findMany({
            ...
            where: {
                ...
                status: 'PENDING',
                clientTurnId: { not: SessionsService_1.initialTurnClientId(id) },
            },
            orderBy: { seq: 'asc' },
    ...
    async cancelQueuedTurn(ownerId, id, turnId) {
        ...
        FOR UPDATE`;
            ...
            if (res.count === 0) {
                ...
                throw new common_1.ConflictException(steer
            ...
            if (executable === 0 && session.status === client_1.RunStatus.RUNNING) {
                ...
                throw new common_1.ConflictException('message already started or not found');
            }
            if (executable === 0 && session.status === client_1.RunStatus.PENDING) {
                await tx.session.update({
                    where: { id },
                    data: { status: client_1.RunStatus.AWAITING_INPUT, lastTurnAt: new Date() },
                });
            }
        }, (0, transaction_retry_1.loggedRetry)(this.logger, 'sessions.cancelQueuedTurn'));
        this.realtime.notifyInbox(id);
        this.realtime.publishQueuedTurnsChanged(id);
        return { ok: true };
    }
    /** End a live session (closes the runner's claude process). */
    async end(ownerId, id) {
        const session = await this.getSendable(ownerId, id);
        await this.endOpen(ownerId, id, shared_1.SessionEndReason.ENDED);
        return { ok: true };
    }
    /**
     ...
     */
    async mergeToMain(ownerId, id, targetBranch) {
        ...
        const workspaceId = await (0, transaction_retry_1.withTransactionRetry)(this.prisma, async (tx) => {
        ...
        FOR UPDATE`;
            ...
            if (session.isolationStatus !== 'worktree' || !session.branch) {
                throw new common_1.BadRequestException('session has no worktree branch to merge');
            }
            if (!session.assignedRunnerId) {
                throw new common_1.ConflictException('no runner is associated with this session');
            }
            if (target && target === session.branch) {
                throw new common_1.BadRequestException("can't merge a branch into itself");
            }
            if (session.mergeStatus === 'pending')
                return null;
            if (session.commitStatus === 'pending') {
                throw new common_1.ConflictException('wait for the pending worktree commit to finish');
            }
            if (!SessionsService_1.TERMINAL.includes(session.status) &&
                (session.status !== client_1.RunStatus.AWAITING_INPUT || !!session.cancelRequestedAt)) {
                throw new common_1.ConflictException('wait for the current turn to finish before merging');
            }
            await tx.session.update({
                where: { id },
                data: {
                    ...
                    mergeOperationId: (0, crypto_1.randomUUID)(),
                    mergeOperationOwner: null,
                    mergeError: null,
                    mergedAt: null,
                    mergedSourceSha: null,
        ...
        }, (0, transaction_retry_1.loggedRetry)(this.logger, 'sessions.mergeToMain'));
     ...
     */
    async commitWorktree(ownerId, id) {
        const session = await this.prisma.session.findFirst({ where: { id, ownerId } });
        if (!session)
            throw new common_1.NotFoundException('session not found');
        if (session.isolationStatus !== 'worktree' || !session.branch) {
            throw new common_1.BadRequestException('session has no worktree to commit');
        }
        if (!SessionsService_1.LIVE.includes(session.status) || session.cancelRequestedAt) {
            throw new common_1.ConflictException('the session has ended — its work is already committed');
        }
        if (session.status !== client_1.RunStatus.AWAITING_INPUT) {
            throw new common_1.ConflictException('wait for the current turn to finish before committing');
        }
        if (session.runningSubagents.length > 0) {
            throw new common_1.ConflictException('wait for the running sub-workspace to finish before committing');
        }
        if (!session.assignedRunnerId) {
            throw new common_1.ConflictException('no runner is associated with this session');
        }
        if (session.commitStatus === 'pending')
            return { ok: true };
        if (session.mergeStatus === 'pending') {
            throw new common_1.ConflictException('wait for the pending branch merge to finish');
        }
        ...
        const queued = await this.prisma.session.updateMany({
            where: {
                id,
                ownerId,
                status: client_1.RunStatus.AWAITING_INPUT,
            ...
            data: {
                ...
                commitOperationId: (0, crypto_1.randomUUID)(),
                commitOperationOwner: null,
                commitError: null,
            },
        });
        if (queued.count === 0) {
            ...
            if (current?.commitStatus === 'pending' &&
                current.status === client_1.RunStatus.AWAITING_INPUT &&
            ...
            throw new common_1.ConflictException('the session is no longer idle — wait for its current work to finish');
        }
        return { ok: true };
    }
    /**
     ...
     */
    async adoptWorktreeBranch(ownerId, id) {
        ...
        FOR UPDATE`;
            ...
            if (session.isolationStatus !== 'worktree') {
                throw new common_1.BadRequestException('session has no worktree to adopt a branch from');
            }
            if ((0, session_inbox_fence_1.pendingWorktreeOperationMayBeExecuting)(session.mergeStatus, session.mergeOperationId, session.mergeOperationOwner, session.mergeRequestedAt) ||
                (0, session_inbox_fence_1.pendingWorktreeOperationMayBeExecuting)(session.commitStatus, session.commitOperationId, session.commitOperationOwner, session.commitRequestedAt)) {
                throw new common_1.ConflictException('wait for the pending worktree operation to finish');
            }
            const target = session.worktreeBranch?.trim();
            if (!target) {
                throw new common_1.ConflictException('the runner has not reported the worktree branch yet');
            }
            if (target === session.branch) {
                throw new common_1.BadRequestException('the worktree is already on the tracked branch');
            }
            await tx.session.update({
                where: { id },
                data: {
        ...
        }, (0, transaction_retry_1.loggedRetry)(this.logger, 'sessions.adoptWorktreeBranch'));
    }
    /**
     ...
     */
    async cancel(ownerId, id, reason = shared_1.SessionEndReason.CANCELLED) {
        const session = await this.prisma.session.findFirst({ where: { id, ownerId } });
        if (!session)
            throw new common_1.NotFoundException('session not found');
        if (SessionsService_1.TERMINAL.includes(session.status) || session.cancelRequestedAt)
     ...
     */
    async endOpen(ownerId, sessionId, reason) {
        const ended = await this.transitionEnd(ownerId, sessionId, reason);
        if (!ended.changed)
            return false;
        if (ended.runnerId)
            this.realtime.requestCancel(ended.runnerId, sessionId);
     ...
     */
    async transitionEnd(ownerId, sessionId, reason, lifecycle) {
        ...
        FOR UPDATE`;
            ...
            if (lifecycle === 'completedAt' && session.deletedAt != null) {
                throw new common_1.ConflictException('a session in Trash must be moved to Open before it can be completed');
            }
            const now = new Date();
            ...
            const lifecycleState = (0, shared_1.deriveSessionLifecycleState)({
                completedAt: finalCompletedAt,
                deletedAt: finalDeletedAt,
            });
            const filingState = (0, shared_1.deriveSessionFilingState)({
                completedAt: finalCompletedAt,
                deletedAt: finalDeletedAt,
            });
            if (SessionsService_1.TERMINAL.includes(session.status) || session.cancelRequestedAt) {
                ...
                return {
                    ...
                    lifecycleState,
                    filingState,
                    endReason: session.endReason == null ? null : session.endReason,
                };
            }
            if (session.status === client_1.RunStatus.PENDING) {
                await tx.session.update({
                    where: { id: sessionId },
                    data: {
                        ...lifecycleData,
                        status: client_1.RunStatus.CANCELLED,
                        endReason: reason,
                        cancelRequestedAt: now,
                        finishedAt: now,
                    },
                });
                await (0, session_inbox_fence_1.retireSessionInboxGeneration)(tx, sessionId);
                ...
                await this.insertTurnLocked(tx, sessionId, {
                    kind: 'end',
                    clientTurnId: (0, crypto_1.randomUUID)(),
                });
            }
            return {
                changed: true,
                runnerId: session.assignedRunnerId,
                status: session.status === client_1.RunStatus.PENDING ? client_1.RunStatus.CANCELLED : session.status,
                lifecycleState,
                filingState,
                endReason: reason,
            };
        }, (0, transaction_retry_1.loggedRetry)(this.logger, 'sessions.transitionEnd'));
    }
    /** Emit runner/control-plane effects for a newly persisted end intent. */
    publishEndIntent(sessionId, ended) {
        if (!ended.changed)
            return;
        if (ended.runnerId)
            this.realtime.requestCancel(ended.runnerId, sessionId);
    ...
    async listApprovals(ownerId, id, status) {
        const session = await this.prisma.session.findFirst({ where: { id, ownerId }, select: { id: true } });
        if (!session)
            throw new common_1.NotFoundException('session not found');
    ...
    async decideApproval(ownerId, id, approvalId, dto) {
        const session = await this.prisma.session.findFirst({
            where: { id, ownerId },
        ...
        if (dto.behavior !== 'allow' && dto.behavior !== 'deny') {
            throw new common_1.BadRequestException('behavior must be "allow" or "deny"');
        }
        ...
        const res = await this.prisma.approval.updateMany({
            ...
            data: {
                ...
                answers: dto.answers ? dto.answers : client_1.Prisma.DbNull,
                ...
                rememberRule: dto.behavior === 'allow' && dto.rememberRules?.length
                    ? dto.rememberRules
        ...
        if (res.count > 0) {
            ...
            this.realtime.publish(id, {
                seq: 0,
                type: shared_1.RunEventType.APPROVAL_RESOLVED,
    ...
    async rememberForWorkspace(workspaceId, ownerId, approvalId, rules) {
        const stored = (0, permission_rules_1.normalizePermissionRules)(rules);
        if (!stored.length)
            return;
     ...
     */
    static RESUMABLE_PROJECTION = '00000000-0000-4000-8000-000000000000';
    /**
     ...
     */
    static AGGREGATE_PARENT_REFUSAL = 'this task is completed by aggregating its subtasks, so it has no work of its own to run — '
     ...
     */
    async taskWorkRefusalFor(db, taskId, locked = false, startsTaskWork = true) {
        ...
        const [facts] = await db.$queryRaw(client_1.Prisma.sql `
      SELECT t."terminal_reason" AS "terminalReason",
       ...
       ${locked ? client_1.Prisma.sql `FOR SHARE OF t` : client_1.Prisma.empty}
    `);
        if (!facts)
            return null;
        if (startsTaskWork && (0, task_aggregation_1.isAggregateParent)({
            completionPolicy: facts.completionPolicy,
            hasDirectChildren: facts.hasDirectChildren,
        })) {
            return SessionsService_1.AGGREGATE_PARENT_REFUSAL;
        }
        return (0, task_supersession_1.taskWorkRefusal)(facts, shared_1.uuidToBase62);
    }
    async resume(ownerId, id, dto, opts) {
        assertPromptSize(dto.content, 'message');
        const session = await this.prisma.session.findFirst({
            where: { id, ownerId },
            include: {
        ...
        const continuesSalvage = SessionsService_1.LIVE.includes(session.status)
        ...
        if (session.taskId && !continuesSalvage) {
            ...
            const refusal = await this.taskWorkRefusalFor(this.prisma, session.taskId, false, effectiveStartsTaskWork);
            if (refusal) {
                throw new common_1.ConflictException(`this session's run may not be resumed: ${refusal}`);
            }
        }
        ...
        const needsRuntimeRepair = session.provider === 'claude' && session.numTurns > 0 && !session.runtimeSessionId;
        if (needsRuntimeRepair) {
            ...
            session.runtimeSessionId = (0, crypto_1.randomUUID)();
            session.numTurns = 0;
        }
        const initialCapabilities = (0, session_state_1.deriveSessionCapabilities)(session);
        if (initialCapabilities.resumeBlockedReason === 'TRASHED') {
            throw SessionsService_1.resumeBlocked('TRASHED');
        }
        if (initialCapabilities.resumeBlockedReason === 'ENDING') {
            throw SessionsService_1.resumeBlocked('ENDING');
        }
        ...
        if (initialCapabilities.resumeBlockedReason &&
            ...
            throw SessionsService_1.resumeBlocked(initialCapabilities.resumeBlockedReason);
        }
        ...
        const revived = await (0, transaction_retry_1.withTransactionRetry)(this.prisma, async (tx) => {
            ...
            const [scope] = await tx.$queryRaw(client_1.Prisma.sql `
        ...
        `);
            ...
            SELECT 1 FROM "project" WHERE "id" = ${scope.projectId}::uuid FOR NO KEY UPDATE`;
                ...
                let confirmed;
                try {
                    confirmed = await tx.$queryRaw(client_1.Prisma.sql `
          ...
          `);
                    ...
                    if (!(0, task_supersession_1.isLockNotAvailable)(error))
                        throw error;
                    throw new common_1.ConflictException('this session\'s task is being written right now, so the run cannot be resumed in ' +
                ...
                if ((confirmed[0]?.projectId ?? null) !== (scope.projectId ?? null)) {
                    throw new common_1.ConflictException('this session\'s task moved to another project while the resume was being prepared — ' +
          ...
          FOR UPDATE NOWAIT`;
                ...
                if (!(0, task_supersession_1.isLockNotAvailable)(error))
                    throw error;
                throw new common_1.ConflictException('this session is being written right now, so it cannot be revived in this request — ' +
                    'nothing was changed; retry');
            }
            if (locked.length === 0)
                throw new common_1.NotFoundException('session not found');
            ...
            if ((current.taskId ?? null) !== (scope?.taskId ?? null)) {
                throw new common_1.ConflictException('this session was rebound to another task while the resume was being prepared — ' +
                    'nothing was changed; retry');
            }
            ...
            if (current.taskId) {
                const refusal = await this.taskWorkRefusalFor(tx, current.taskId, true, current.startsTaskWork || opts?.startsTaskWork === true);
                if (refusal) {
                    throw new common_1.ConflictException(`this session's run may not be resumed: ${refusal}`);
                }
            }
            ...
            const capabilities = (0, session_state_1.deriveSessionCapabilities)(current);
            ...
            if (!SessionsService_1.TERMINAL.includes(current.status)) {
                if (existing)
                    return { turn: existing, wasCompleted: false, wasRevived: false };
                throw SessionsService_1.resumeBlocked('NOT_TERMINAL');
            }
            if (capabilities.resumeBlockedReason &&
                ...
                throw SessionsService_1.resumeBlocked(capabilities.resumeBlockedReason);
            }
            if (existing)
                return { turn: existing, wasCompleted: false, wasRevived: false };
            if (capabilities.resumeBlockedReason) {
            ...
            if ((0, session_inbox_fence_1.pendingWorktreeOperationMayBeExecuting)(current.mergeStatus, current.mergeOperationId, current.mergeOperationOwner, current.mergeRequestedAt) ||
                (0, session_inbox_fence_1.pendingWorktreeOperationMayBeExecuting)(current.commitStatus, current.commitOperationId, current.commitOperationOwner, current.commitRequestedAt)) {
                throw new common_1.ConflictException('wait for the pending worktree operation to finish');
            }
            ...
            await (0, session_inbox_fence_1.retireSessionInboxGeneration)(tx, id);
            const turn = await this.insertTurnLocked(tx, id, {
            ...
            const normalizedEffort = dto.effort !== undefined
                ? (0, runtime_provider_1.normalizeEffortForProvider)((0, runtime_provider_1.normalizeRuntimeProvider)(next.provider, next.providerBuiltin), dto.effort)
                : undefined;
            await tx.session.update({
                where: { id },
                data: {
                    ...
                    inboxLeaseOwner: (0, session_inbox_fence_1.newTerminalResumeHandoffOwner)(),
                    cancelRequestedAt: null,
                    endReason: null,
                    finishedAt: null,
                    error: null,
        ...
        }, (0, transaction_retry_1.loggedRetry)(this.logger, 'sessions.resume'));
        ...
        if (revived.wasCompleted)
            this.realtime.publishSessionCreated(id);
        else if (revived.wasRevived)
     ...
     */
    async complete(ownerId, id) {
        const ended = await this.transitionEnd(ownerId, id, shared_1.SessionEndReason.COMPLETED, 'completedAt');
        ...
        this.realtime.publishSessionLifecycleChanged(id, ended.status, ended.endReason, ended.lifecycleState);
        return { ok: true };
    }
    /**
     ...
     */
    async resolveProviderSwitch(tx, session, requested) {
        const declared = session.provider;
        const currentRow = (0, custom_provider_1.isBuiltinProvider)(declared, session.providerBuiltin)
            ? null
        ...
        const providerBuiltin = Object.values(shared_1.AgentProvider).includes(requested);
        ...
        const from = (0, custom_provider_1.execRuntime)({
            declaredProvider: declared,
            declaredProviderBuiltin: session.providerBuiltin,
            customRow: currentRow,
        });
        const to = (0, custom_provider_1.execRuntime)({
            declaredProvider: requested,
            declaredProviderBuiltin: providerBuiltin,
            customRow: targetRow,
        });
        if (from !== to) {
            throw new common_1.BadRequestException(`a ${from} session cannot switch to a provider that runs on ${to}`);
        }
        return {
            provider: requested,
            providerBuiltin,
            customRow: targetRow,
            changed: true,
            keepsModel: !targetRow || (0, preset_overlay_1.ownsModel)(targetRow, session.model ?? ''),
        };
    }
    /**
     ...
     */
    async updateConfig(ownerId, id, dto) {
        ...
        const needsReload = await (0, transaction_retry_1.withTransactionRetry)(this.prisma, async (tx) => {
        ...
        FOR UPDATE`;
            ...
            const exec = (0, custom_provider_1.resolveProviderExec)({
                ...
                workspaceEnv: session.workspace?.env,
            });
            const requestedPermissionMode = dto.permissionMode ??
                (0, permission_mode_1.resolvePermissionMode)(session.permissionMode, session.owner);
            const normalizedPermissionMode = (0, runtime_provider_1.normalizeBuiltinPermissionMode)(exec.provider, exec.model, requestedPermissionMode, next.customRow?.enabled === true, session.assignedRunner?.runsAsRoot);
            const normalizedEffort = dto.effort !== undefined
                ? (0, runtime_provider_1.normalizeEffortForProvider)(exec.provider, dto.effort)
                : undefined;
            await tx.session.update({
                where: { id },
                data: {
            ...
            return true;
        }, (0, transaction_retry_1.loggedRetry)(this.logger, 'sessions.updateConfig'));
        if (needsReload)
            this.realtime.notifyInbox(id);
        return { ok: true };
    }
     ...
     */
    async rename(ownerId, id, rawTitle) {
        const title = (rawTitle ?? '').trim();
        if (!title)
            throw new common_1.BadRequestException('title must not be empty');
        if (title.length > 200)
            throw new common_1.BadRequestException('title is too long (max 200 chars)');
        const session = await this.prisma.session.findFirst({ where: { id, ownerId } });
        if (!session)
            throw new common_1.NotFoundException('session not found');
    ...
    async remove(ownerId, id) {
        const ended = await this.transitionEnd(ownerId, id, shared_1.SessionEndReason.DELETED, 'deletedAt');
        this.publishEndIntent(id, ended);
    ...
    async restore(ownerId, id) {
        ...
        }, (0, transaction_retry_1.loggedRetry)(this.logger, 'sessions.restore'));
    ...
    async purge(ownerId, id) {
        ...
        FOR UPDATE`;
            ...
            if (!session)
                throw new common_1.NotFoundException('session not found');
            if (!session.deletedAt) {
                throw new common_1.BadRequestException('session must be in Trash before it can be permanently deleted');
            }
            await tx.session.delete({ where: { id: session.id } });
        }, (0, transaction_retry_1.loggedRetry)(this.logger, 'sessions.purge'));
        return { ok: true };
    }
};
...
exports.SessionsService = SessionsService = SessionsService_1 = __decorate([
    ...
    __metadata("design:paramtypes", [prisma_service_1.PrismaService, queue_service_1.QueueService, realtime_service_1.RealtimeService])
], SessionsService);
