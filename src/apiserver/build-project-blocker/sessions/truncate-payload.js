...
exports.MAX_IMAGE_PAYLOAD = exports.DEFAULT_MAX_PAYLOAD = void 0;
exports.parseMaxPayload = parseMaxPayload;
...
exports.DEFAULT_MAX_PAYLOAD = 2048;
/**
...
function clipDeep(v, cap, cut) {
    if (typeof v === 'string')
        return clip(v, cap, cut);
    if (Array.isArray(v))
        return v.map((x) => clipDeep(x, cap, cut));
    if (v && typeof v === 'object') {
        const out = {};
        for (const [k, val] of Object.entries(v))
            out[k] = clipDeep(val, cap, cut);
        return out;
    }
    return v;
}
 ...
 */
function clipImage(block, cut) {
    const source = block.source;
    if (!source || typeof source !== 'object')
        return block;
    const s = source;
    if (typeof s.data !== 'string' || s.data.length <= exports.MAX_IMAGE_PAYLOAD)
        return block;
    cut.hit = true;
 ...
 */
function clipContent(content, cap, cut) {
    if (typeof content === 'string')
        return clip(content, cap, cut);
 ...
 */
function truncatePayload(type, payload, cap) {
    if (!payload || typeof payload !== 'object')
        return { payload, truncated: false };
    const cut = { hit: false };
    if (type === shared_1.RunEventType.TOOL_RESULT) {
        const p = payload;
        if (!('content' in p))
            return { payload, truncated: false };
    ...
    if (type === shared_1.RunEventType.TOOL_USE) {
        const p = payload;
        if (!('input' in p))
            return { payload, truncated: false };
...
//# sourceMappingURL=truncate-payload.js.map