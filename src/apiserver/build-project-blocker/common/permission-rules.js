...
exports.AUTO_ALLOWED_MESSAGE = void 0;
exports.normalizePermissionRule = normalizePermissionRule;
exports.normalizePermissionRules = normalizePermissionRules;
exports.permissionRuleToken = permissionRuleToken;
exports.dispatchAllowedTools = dispatchAllowedTools;
exports.serverMatchedRuntime = serverMatchedRuntime;
exports.ruleCoversApproval = ruleCoversApproval;
 ...
 */
function narrowsOneGrant(content) {
...
function normalizePermissionRule(rule) {
    ...
    if (!toolName || toolName.length > MAX_RULE_LEN || !TOOL_NAME.test(toolName))
        return null;
    const ruleContent = (rule?.ruleContent ?? '').trim();
    if (ruleContent.length > MAX_RULE_LEN || !narrowsOneGrant(ruleContent))
        return null;
...
function permissionRuleToken(rule) {
 ...
 */
function dispatchAllowedTools(provider, base, rules) {
    const out = [...base];
    if (provider !== shared_1.AgentProvider.CLAUDE)
        return out;
 ...
 */
const SERVER_MATCHED_RUNTIMES = new Set([
...
function serverMatchedRuntime(provider) {
...
exports.AUTO_ALLOWED_MESSAGE = 'Allowed automatically — this workspace has a standing grant for this call.';
...
function prefixCovers(rulePrefix, commandPrefix) {
...
function bashRulePrefixes(rules) {
    return rules
 ...
 */
function commandCovered(command, rules) {
    ...
    if (prefixes.length === 0)
        return false;
    const segments = (0, shared_1.bashSegments)(trimmed);
    ...
    return segments.every((segment) => {
        ...
        if (!text)
            return false;
        if (UNREADABLE_SHELL_SYNTAX.some((syntax) => text.includes(syntax)))
            return false;
        const commandPrefix = (0, shared_1.bashPrefix)(text);
        if (!commandPrefix)
            return false;
 ...
 */
function ruleCoversApproval(provider, toolName, input, rules) {
    if (!SERVER_MATCHED_RUNTIMES.has(provider) || !toolName)
        return false;
