...
exports.treeActiveTurns = treeActiveTurns;
exports.treeCeiling = treeCeiling;
exports.runnerActiveTurns = runnerActiveTurns;
exports.batchActiveTurns = batchActiveTurns;
const client_1 = require("@prisma/client");
const session_scheduling_1 = require("./session-scheduling");
/**
 ...
 */
function treeActiveTurns(sessionAlias) {
    const s = client_1.Prisma.raw(sessionAlias);
 ...
 */
function treeCeiling(runnerIdSql) {
    return client_1.Prisma.sql `GREATEST(
    1,
    (SELECT r."max_concurrent" FROM "runner" r WHERE r.id = ${runnerIdSql})
      - ${session_scheduling_1.SPAWN_TREE_RUNNER_RESERVE}
  )`;
}
/** Active turns already held on a runner. A slot is an active turn, not a warm process. */
function runnerActiveTurns(runnerIdSql) {
    return client_1.Prisma.sql `(
...
function batchActiveTurns(sessionAlias) {
    const s = client_1.Prisma.raw(sessionAlias);
    return client_1.Prisma.sql `(
...
//# sourceMappingURL=session-tree-sql.js.map