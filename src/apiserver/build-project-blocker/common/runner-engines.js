...
exports.REPORTED_ENGINES = exports.LOGIN_ENGINES = void 0;
exports.isLoginEngine = isLoginEngine;
exports.isReportedEngine = isReportedEngine;
exports.sanitizeRunnerEngines = sanitizeRunnerEngines;
/** The engines a runner can sign into (LoginEngine's full set), in the order they're shown. */
exports.LOGIN_ENGINES = ['claude', 'codex', 'kimi'];
function isLoginEngine(value) {
    return typeof value === 'string' && exports.LOGIN_ENGINES.includes(value);
}
/**
 ...
 */
exports.REPORTED_ENGINES = [...exports.LOGIN_ENGINES, 'opencode'];
function isReportedEngine(value) {
    return typeof value === 'string' && exports.REPORTED_ENGINES.includes(value);
}
/**
 ...
 */
function sanitizeRunnerEngines(value) {
    ...
    for (const raw of value) {
        if (!raw || typeof raw !== 'object')
            continue;
        const entry = raw;
        if (!isReportedEngine(entry.engine) || byEngine.has(entry.engine))
            continue;
    ...
    if (!byEngine.size)
        return null;
    return exports.REPORTED_ENGINES.map((engine) => byEngine.get(engine)).filter((entry) => !!entry);
}
 ...
 */
function sanitizeEngineUpdate(value) {
    ...
    const latest = typeof raw.latest === 'string' && raw.latest.trim() ? raw.latest.trim().slice(0, 120) : undefined;
...
//# sourceMappingURL=runner-engines.js.map