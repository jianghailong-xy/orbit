...
exports.setModelCatalog = setModelCatalog;
exports.catalogModels = catalogModels;
exports.catalogDefaultModel = catalogDefaultModel;
exports.presetCatalog = presetCatalog;
exports.catalogFingerprint = catalogFingerprint;
exports.parseModelCatalog = parseModelCatalog;
 ...
 */
function catalogDefaultModel(preset) {
...
function presetCatalog(presets = shared_1.PROVIDER_PRESETS) {
    return Object.fromEntries(presets.map((p) => [p.slug, { models: catalogModels(p), defaultModel: catalogDefaultModel(p) }]));
}
 ...
 */
function parseModelCatalog(payload, presets = shared_1.PROVIDER_PRESETS) {
...
function usable(m, match) {
    if (typeof m.id !== 'string' || !match.test(m.id))
        return false;
    if (m.tool_call !== true)
        return false;
