...
exports.PROJECT_BLOCKER_NON_BLOCKING_REFUSALS = exports.PROJECT_BLOCKER_REFUSAL_KINDS = exports.PROJECT_BLOCKER_RESOLUTION_CHAIN_KINDS = exports.PROJECT_BLOCKER_TURN_KINDS = exports.PROJECT_BLOCKER_POLICY = exports.PROJECT_BLOCKER_DEFAULT_ESCALATION_MS = exports.PROJECT_BLOCKER_SUBJECT_TYPES = exports.PROJECT_BLOCKER_KINDS = void 0;
exports.blockerKindForRefusal = blockerKindForRefusal;
exports.projectDeadLetterCondition = projectDeadLetterCondition;
exports.projectBlockerDedupeKey = projectBlockerDedupeKey;
exports.projectBlockerConditionVersion = projectBlockerConditionVersion;
exports.raiseBlockerIdempotencyKey = raiseBlockerIdempotencyKey;
exports.clearBlockerIdempotencyKey = clearBlockerIdempotencyKey;
exports.planProjectBlockers = planProjectBlockers;
exports.blockerRunState = blockerRunState;
...
exports.PROJECT_BLOCKER_SUBJECT_TYPES = [
    'PROJECT',
    'TASK',
    'SESSION',
    'PROVIDER',
...
exports.PROJECT_BLOCKER_DEFAULT_ESCALATION_MS = 30 * 60_000;
/**
 ...
 */
exports.PROJECT_BLOCKER_POLICY = {
    WHO_UNRESOLVED: {
        owner: 'COORDINATOR',
        recovery: 'EVENT',
        opensTurn: true,
        severity: 'WARNING',
        pollMs: 5 * 60_000,
        escalateMs: exports.PROJECT_BLOCKER_DEFAULT_ESCALATION_MS,
...
exports.PROJECT_BLOCKER_TURN_KINDS = exports.PROJECT_BLOCKER_KINDS
    .filter((kind) => exports.PROJECT_BLOCKER_POLICY[kind].opensTurn);
/**
 ...
 */
exports.PROJECT_BLOCKER_RESOLUTION_CHAIN_KINDS = [
    'WHO_UNRESOLVED',
    'WHO_NOT_IN_TEAM',
    'WHO_DISABLED',
    'PROVIDER_UNAVAILABLE',
 ...
 */
exports.PROJECT_BLOCKER_REFUSAL_KINDS = {
 ...
 */
exports.PROJECT_BLOCKER_NON_BLOCKING_REFUSALS = new Set([
    'POLICY_ALLOWED',
    'APPROVAL_GRANTED',
    'APPROVAL_DENIED',
    'APPROVAL_EXPIRED',
 ...
 */
function blockerKindForRefusal(code) {
    if (!code)
        return 'UNKNOWN_FAILURE';
    if (exports.PROJECT_BLOCKER_NON_BLOCKING_REFUSALS.has(code))
        return null;
    return exports.PROJECT_BLOCKER_REFUSAL_KINDS[code] ?? 'UNKNOWN_FAILURE';
}
/**
 ...
 * closed means here.
 */
function projectDeadLetterCondition(projectId, events) {
...
function projectBlockerDedupeKey(kind, subjectType, subjectId) {
 ...
 * second does not.
 */
function projectBlockerConditionVersion(kind, subjectType, subjectId, facts) {
...
function raiseBlockerIdempotencyKey(projectId, kind, subjectId, lifecycleGeneration) {
...
function clearBlockerIdempotencyKey(projectId, blockerId) {
 ...
 */
function planProjectBlockers(input) {
    ...
    for (const [dedupeKey, condition] of [...observed].sort(byKey)) {
        const policy = exports.PROJECT_BLOCKER_POLICY[condition.kind];
        const conditionVersion = projectBlockerConditionVersion(condition.kind, condition.subjectType, condition.subjectId, condition.facts);
        ...
        if (existing) {
            ...
            const nextCheckAt = nextCheckInstant(policy, epochMs, Date.parse(existing.firstSeenAt), condition.recoveryAt);
            touches.push({
    ...
    const known = new Set(exports.PROJECT_BLOCKER_KINDS);
    for (const blocker of [...input.open].sort((a, b) => compare(a.dedupeKey, b.dedupeKey))) {
        if (observed.has(blocker.dedupeKey))
            continue;
    ...
    for (const projection of openAfter) {
        ...
        const dueAt = Date.parse(projection.firstSeenAt)
            + exports.PROJECT_BLOCKER_POLICY[projection.kind].escalateMs;
        if (epochMs < dueAt)
            continue;
        escalations.push({
 ...
 */
function blockerRunState(open) {
    if (open.some((blocker) => blocker.owner === 'USER'))
 ...
 */
function nextCheckInstant(policy, epochMs, firstSeenMs, recoveryAt) {
    ...
    if (policy.recovery === 'EVENT') {
        return new Date(epochMs + (policy.pollMs ?? exports.PROJECT_BLOCKER_DEFAULT_ESCALATION_MS)).toISOString();
    }
    return new Date(firstSeenMs + policy.escalateMs).toISOString();
}
function byKey(a, b) {
...
//# sourceMappingURL=project-blocker.js.map