...
exports.AGGREGATION_SCOPE_MAX_TASKS = void 0;
exports.collectAggregationScope = collectAggregationScope;
exports.applyTaskAggregations = applyTaskAggregations;
 ...
 */
exports.AGGREGATION_SCOPE_MAX_TASKS = 500;
const SCOPE_SELECT = {
    id: true,
    status: true,
    parentTaskId: true,
 ...
 */
async function collectAggregationScope(db, ownerId, seedTaskIds) {
    const byId = new Map();
    ...
    while (frontier.length > 0) {
        ...
        for (const row of rows) {
            if (!byId.has(row.id)) {
                byId.set(row.id, row);
                if (byId.size > exports.AGGREGATION_SCOPE_MAX_TASKS)
    ...
    return {
        facts: [...byId.values()].map((row) => ({
            ...
            retirement: (0, task_supersession_1.taskRetirement)(row),
        })),
        truncated: false,
    };
}
 ...
 */
async function applyTaskAggregations(db, ownerId, aggregations, now) {
    const moved = [];
    for (const aggregation of aggregations) {
        const rows = await db.$executeRaw(client_1.Prisma.sql `
      UPDATE "task"
...
//# sourceMappingURL=task-aggregation-writer.js.map