...
exports.settleAlert = settleAlert;
...
function settleAlert(s) {
    ...
    if (s.status === client_1.RunStatus.SUCCEEDED)
        return { kind: 'finished', body: 'Finished' };
    ...
    if (s.status !== client_1.RunStatus.FAILED)
        return null;
...
function failureLine(error) {
    ...
    if (!line)
        return 'Failed';
    const excerpt = line.length > MAX_ERROR_CHARS ? `${line.slice(0, MAX_ERROR_CHARS - 1).trimEnd()}…` : line;
    return `Failed · ${excerpt}`;
}
//# sourceMappingURL=settle-alert.js.map