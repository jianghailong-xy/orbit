...
exports.TURN_REASON_ORDER = void 0;
exports.openCoordinatorTurnIdempotencyKey = openCoordinatorTurnIdempotencyKey;
exports.coordinatorTurnGeneration = coordinatorTurnGeneration;
exports.chooseTurnReason = chooseTurnReason;
exports.turnReasonDigest = turnReasonDigest;
exports.turnReasonFactsOf = turnReasonFactsOf;
exports.planCoordinatorTurn = planCoordinatorTurn;
exports.openCoordinatorTurnClientTurnId = openCoordinatorTurnClientTurnId;
 ...
 */
function openCoordinatorTurnIdempotencyKey(projectId, generation, reasonDigest) {
 ...
 */
function coordinatorTurnGeneration(idempotencyKey) {
 ...
 */
function chooseTurnReason(facts) {
    const trueReasons = exports.TURN_REASON_ORDER.filter((code) => Object.hasOwn(facts, code));
...
function turnReasonDigest(reasonCode, turnFacts) {
 ...
 */
function turnReasonFactsOf(input, context) {
    const facts = {};
    ...
    if (conclusions.length > 0)
        facts.VERDICT = conclusions;
    const episodes = failureEpisodes(input);
    if (episodes.length > 0)
        facts.TASK_FAILURE = episodes;
    ...
    const blockers = context.openBlockers
        .filter((blocker) => project_blocker_1.PROJECT_BLOCKER_POLICY[blocker.kind].opensTurn && blocker.escalatedAt === null)
        .map((blocker) => `${blocker.kind}:${blocker.subjectId}#${blocker.lifecycleGeneration}:${blocker.conditionVersion}`)
        .sort();
    if (blockers.length > 0)
        facts.BLOCKER_DECISION = blockers;
    return facts;
}
/**
 ...
 */
function failureEpisodes(input) {
    const retirementOf = new Map(input.world.tasks.map((task) => [task.id, (0, task_supersession_1.taskRetirement)({
            supersededByTaskId: task.supersededByTaskId ?? null,
            terminalReason: task.terminalReason ?? null,
        })]));
    return input.world.tasks
        .filter((task) => {
        const disposition = (0, project_failed_retry_1.failedTaskDisposition)({
            status: task.status,
        ...
        return disposition !== 'NOT_FAILED'
            ...
            && disposition !== 'RETIRED'
            && !(0, project_failed_retry_1.loopCanRetryFailedTask)(disposition);
    })
 ...
 */
function planCoordinatorTurn(input, context) {
    ...
    const idempotencyKey = openCoordinatorTurnIdempotencyKey(input.world.project.id, generation, reasonDigest);
    const internalKey = openCoordinatorTurnIdempotencyKey((0, shared_1.base62ToUuid)(input.world.project.id), generation, reasonDigest);
    const base = {
...
function lastAppliedTurn(input, idempotencyKey) {
    const wanted = (0, node_crypto_1.createHash)('sha256').update(idempotencyKey).digest('hex');
    for (let i = input.world.actions.length - 1; i >= 0; i -= 1) {
        ...
        if (action.type !== 'OPEN_COORDINATOR_TURN' || action.status !== 'APPLIED')
            continue;
        if (action.idempotencyKeyHash !== wanted)
            continue;
 ...
 */
function coordinatorRunIsBusy(input) {
 ...
 */
function openCoordinatorTurnClientTurnId(idempotencyKey) {
    return idempotencyKey;
}
//# sourceMappingURL=project-turn-reason.js.map