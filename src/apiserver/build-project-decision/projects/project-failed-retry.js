...
exports.failedTaskBlockerKind = failedTaskBlockerKind;
exports.failedTaskDisposition = failedTaskDisposition;
exports.loopCanRetryFailedTask = loopCanRetryFailedTask;
exports.failedTaskNeedsAttention = failedTaskNeedsAttention;
 ...
 */
function failedTaskBlockerKind(task) {
    ...
    if (task.failureCount > 0 && !task.failureAttributable)
        return 'UNKNOWN_FAILURE';
    if (task.failureCount >= (task.maxAutoRunFailures ?? task_retry_policy_1.MAX_AUTO_RUN_FAILURES))
        return 'TEST_FAILED';
    return null;
}
function failedTaskDisposition(task) {
    ...
    if (task.retirement != null || task.subjectRetirement != null)
        return 'RETIRED';
    ...
    if (terminal === 'UNKNOWN_FAILURE')
        return 'UNATTRIBUTABLE';
    if (terminal === 'TEST_FAILED')
 ...
 */
function loopCanRetryFailedTask(disposition) {
 ...
 */
function failedTaskNeedsAttention(disposition) {
    return disposition === 'UNATTRIBUTABLE' || disposition === 'ATTEMPTS_EXHAUSTED';
}
//# sourceMappingURL=project-failed-retry.js.map