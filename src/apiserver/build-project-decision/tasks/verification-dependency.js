...
exports.VERIFICATION_EPOCH_GATES_NEEDING_A_HUMAN = exports.VERIFICATION_RUN_END_REASON = void 0;
exports.verificationCountsTowardEpoch = verificationCountsTowardEpoch;
exports.verificationRunSettled = verificationRunSettled;
exports.verificationEpochGate = verificationEpochGate;
exports.verificationEpochOpenSql = verificationEpochOpenSql;
exports.verificationVerdictActionKeyOf = verificationVerdictActionKeyOf;
exports.verificationEpochGates = verificationEpochGates;
 ...
 */
exports.VERIFICATION_RUN_END_REASON = 'task_done';
/**
 ...
 * not a pass.
 */
function verificationRunSettled(check) {
    const live = check.runs.some((run) => reclaim_stalled_task_1.TASK_OCCUPYING.includes(run.runStatus));
    if (live)
        return false;
    return check.runs.some((run) => run.deletedAt == null
        && run.runStatus === shared_1.RunStatus.SUCCEEDED
        && run.endReason === exports.VERIFICATION_RUN_END_REASON
 ...
 */
function verificationEpochGate(subject, checks) {
    ...
    if (!newest)
        return 'NO_LIVE_VERIFICATION';
    if (newest.status !== 'DONE')
        return 'VERIFICATION_IN_FLIGHT';
    if (!newest.verdict)
    ...
    if (!(BigInt(newest.verdictRevision) > 0n))
        return 'VERDICT_UNREVISIONED';
    if (!verificationRunSettled(newest))
        return 'RUN_NOT_SETTLED';
    if (newest.verdictApplied === false)
        return 'VERDICT_NOT_APPLIED';
    if (subject.status !== 'DONE')
        return 'SUBJECT_NOT_DONE';
    return null;
}
/**
 ...
 */
function verificationEpochOpenSql(alias = 't', dependent) {
    const occupying = reclaim_stalled_task_1.TASK_OCCUPYING.map((status) => `'${status}'`).join(', ');
    ...
    OR EXISTS (
      ...
      SELECT 1 FROM "task" epoch_retired
       WHERE epoch_retired."id" = ${subject}
         AND NOT ${(0, task_supersession_1.taskNotRetiredSql)('epoch_retired')}
    )
    OR EXISTS (
            ...
            WHERE epoch_check."id" = (
                    SELECT epoch_newest."id" FROM "task" epoch_newest
                     WHERE epoch_newest."verifies_task_id" = epoch_subject."id"
                       ...
                       AND ${(0, task_supersession_1.taskNotRetiredSql)('epoch_newest')}
              ...
              AND EXISTS (
                    SELECT 1 FROM "session" epoch_run
                     WHERE epoch_run."task_id" = epoch_check."id"
                       ...
                       AND epoch_run."status"::text = '${shared_1.RunStatus.SUCCEEDED}'
                       AND epoch_run."end_reason" = '${exports.VERIFICATION_RUN_END_REASON}'
 ...
 */
function verificationVerdictActionKeyOf(projectId, verifierTaskId, verdictRevision) {
    return `pc:v1:${projectId}:verdict:${verifierTaskId}:${verdictRevision}`;
}
function verificationVerdictActionKeySqlExpr(alias) {
 ...
 */
function verificationEpochGates(tasks, runsByTaskId) {
    const byId = new Map(tasks.map((task) => [task.id, task]));
    const checksBySubject = new Map();
    for (const task of tasks) {
        ...
        if (list)
            ...
            checksBySubject.set(task.verifiesTaskId, [fact]);
    }
    ...
    for (const [subjectTaskId, checks] of checksBySubject) {
        ...
        const gate = subject
            ...
            : 'SUBJECT_NOT_DONE';
        if (subject)
            epochs.set(subjectTaskId, { subjectTaskId, gate });
        for (const check of checks)
            epochs.set(check.id, { subjectTaskId, gate });
    }
    return epochs;
}
//# sourceMappingURL=verification-dependency.js.map