...
test('reaper keeps the short runtime-initialization watchdog for Kimi', async () => {
      ...
      findMany: async () => [
        {
          ...
          endReason: null,
          task: null,
          workspace: { provider: AgentProvider.KIMI },
...
test('reaper leaves a slow dynamic runtime inside the shared-state initialization window', async () => {
  const sessionId = '11111111-1111-4111-8111-111111111111';
  const runnerId = '22222222-2222-4222-8222-222222222222';
  const prisma = {
    session: {
      findMany: async () => [
        {
          ...
          endReason: null,
          task: null,
          workspace: { provider: AgentProvider.CODEX },
          assignedRunner: { status: 'ONLINE', lastHeartbeatAt: new Date() },
        },
      ],
    },
...
test('reaper still finalizes Codex after the shared-state backstop expires', async () => {
  const sessionId = '11111111-1111-4111-8111-111111111111';
  const runnerId = '22222222-2222-4222-8222-222222222222';
  const prisma = {
    session: {
      findMany: async () => [
        {
          ...
          provider: AgentProvider.CODEX,
          runtimeSessionId: null,
          lastTurnAt: new Date(Date.now() - 26 * 60_000),
          cancelRequestedAt: null,
          endReason: null,
          task: null,
          workspace: { provider: AgentProvider.CODEX },
  ...
  assert.deepEqual(failure, { id: sessionId, reason: 'codex runtime not initialized' });
});
