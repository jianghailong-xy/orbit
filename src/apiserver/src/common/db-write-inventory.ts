...
export const TRANSACTION_PARTICIPANTS: readonly TransactionParticipant[] = [
  ...
  { at: 'projects/project-promotion.service.ts#queuePromotionJob', under: 'projectPromotion.considerCandidate for a CHECK_PROMOTION, projectPromotion.decide for a LAND_PROMOTION — one project_integration_job row (rank 60), idempotent on `ij:v1:<kind>:<promotionId>:<generation>`' },
...
export const STATEMENT_UNITS: readonly StatementUnit[] = [
  ...
  { at: "projects/project-open-item.service.ts#resolveByFact", class: "MANY_ROWS", statements: 1, note: "Closes the items of tasks that moved on, re-derived from the task rows: done, cancelled, replaced, or being attempted again. Its own predicate is what makes a re-issue safe — it matches only OPEN items whose task already says so — and the rows it takes are one project's items, never a cross-owner selection." },
