...
export const TRANSACTION_UNITS: readonly TransactionUnit[] = [
  ...
  },
  {
    ...
    shape: 'TX_RETRIED',
    locks: 'Unlocked reads of the named task, session and approval rows first — the permission check and the snapshot are the same read — then the new watch row, whose foreign keys take FOR KEY SHARE on the owner user (rank 10) and, for a SESSION observer, on that session (rank 30), in that order; then watch_target, watch_match and watch_delivery rows only this transaction can see. Monotone, and no row another transaction can see is written.',
