...
export const TRANSACTION_UNITS: readonly TransactionUnit[] = [
  ...
  },
  {
    ...
    isolation: '',
    attempts: 4,
    replay: 'Every declaration, confirmation, recorded command result and current conclusion is re-read inside the closure after the project lock. A victim exposes no event or DONE; a retry derives against the committed winner and either emits only newer facts or returns the standing answer.',
    effects: 'None.',
  ...
  },
  {
    ...
    shape: 'TX_RETRIED',
    locks: 'session FOR UPDATE via lockSessionLeaseOwner (rank 30); for an executable judgment result, its project FOR NO KEY UPDATE (rank 40) then task FOR UPDATE (rank 50); conversation_turn ACK, task_completion_evidence/idempotency, task_executable_judgment_result, task_judgment_request and blocker/comment children (rank 60), then the database-maintained task dispatch epoch (rank 70). Later llm_usage and session_diff writes add no wait edge because their Session parent is already held. One write of the Session row per transaction (I3).',
