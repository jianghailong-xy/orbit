...
export const TRANSACTION_UNITS: readonly TransactionUnit[] = [
  ...
  },
  {
    ...
    shape: 'TX_RETRIED',
    locks: 'For a title write with a bound coordinator: that Session FOR UPDATE first (rank 30), then project FOR UPDATE or FOR NO KEY UPDATE (rank 40). Every path then takes project_acceptance_criterion_definition / project_acceptance_run children (rank 60) as needed and writes only rows it already holds. A pointer that changed between the rank-30 pre-read and rank-40 validation aborts and retries with the new Session.',
  ...
  },
  {
    ...
    shape: 'TX_RETRIED',
    locks: 'task FOR UPDATE (rank 50), then task_completion_evidence, task_completion_evidence_idempotency, task_judgment_request and optional legacy project_blocker children (rank 60). The Session and optional TaskAttempt provenance reads take no row lock and no FK is written to either snapshot id.',
    ...
    effects: 'None inside. The transaction never updates Task/Session lifecycle state, comments, notifications or realtime. After commit only a VERIFICATION request is handed to the deterministic verifier-task upsert/dispatch; replay converges on that same request/task id.',
  ...
  },
  {
    ...
    shape: 'TX_RETRIED',
    locks: 'user FOR UPDATE (10, when it restructures), task_list (20), creator Session (30, when it writes the task row twice), project FOR NO KEY UPDATE (40, when it touches acceptance), then Task rows in UUID order (50; verifier plus subject for a verdict), followed by task_judgment_request / legacy blocker children (60) and the derived subject status trigger boundary (70). Supersession/move paths retain their Task NOWAIT fence.',
...
export const TRANSACTION_PARTICIPANTS: readonly TransactionParticipant[] = [
  ...
  { at: 'tasks/task-completion-evidence.service.ts#requestForEvidenceFact', under: 'taskCompletionEvidence.submit — after the rank-50 Task mutex, it inserts the evidence-bound request, supersedes older OPEN requests and resolves only legacy blockers at rank 60' },
...
export const TRIGGER_WRITE_SOURCES: readonly TriggerWriteSource[] = [
  ...
  { table: "project_acceptance_criterion_definition", trigger: "project_acceptance_definition_normalize", event: "BEFORE INSERT OR UPDATE OF \"text\", \"revision\", \"content_hash\"", kind: "ROW/STATEMENT", since: "0172_structured_project_acceptance_criteria", takes: [] },
  ...
  { table: "project_merge_evidence", trigger: "project_acceptance_merge_fact", event: "AFTER INSERT", kind: "ROW/STATEMENT", since: "0127_project_acceptance_run", takes: ["project LOCK", "project WRITE", "project_acceptance_audit WRITE", "project_acceptance_run WRITE"] },
