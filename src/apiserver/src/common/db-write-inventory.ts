...
export const TRANSACTION_UNITS: readonly TransactionUnit[] = [
  ...
  },
  {
    ...
    effects: 'None outside PostgreSQL. This path creates no evidence, judgment or successor and cannot decide a request; the request transition itself cancels retryable device delivery and removes the SQL-view blocker atomically.',
  ...
  {
    at: 'tasks/tasks.service.ts#judge',
    shape: 'TX_RETRIED',
    locks: 'user FOR UPDATE (rank 10), project FOR NO KEY UPDATE when present (40), task FOR UPDATE (50), then the named task_judgment_request UPDATE and its transition guard, the request-terminal delivery-child update and legacy project_blocker resolution (60), followed by the status write on the task row already held and its task_dispatch_epoch trigger (70). Request/delivery/blocker facts precede status intentionally.',
    identity: 'The caller names the current request id and evidence digest. Migration 0224 removed the second task_human_signoff row: the decision now lives on the request, which is unique by id and already bound to one evidence version. decidedAt is fixed above the closure, and a committed replay reads the decided request back rather than deciding it twice; any refusal/aborted attempt consumes no identity.',
    isolation: '',
    attempts: 4,
    replay: 'Every attempt re-locks and re-reads the task criterion/status/request after the owner/project locks. The request decision, every legacy HUMAN_DECISION_REQUIRED resolution and the derived DONE update roll back together; a retry either performs all three or observes the already-decided request.',
    effects: 'None inside. EVIDENCE_JUDGMENT_DECIDED input consumption, dependency release, optional verification filing, aggregation and realtime publication all run after commit. No project task-set scan runs.',
    answer: 'Typed 503 from the global boundary; criterion and retirement refusals have their own structured 400/409 responses.',
  },
  {
