...
export const TRANSACTION_PARTICIPANTS: readonly TransactionParticipant[] = [
  ...
  { at: 'projects/project-integration-job.ts#queuePromotionJob', under: 'projectPromotion.considerCandidate for a CHECK_PROMOTION, projectPromotion.decide for a LAND_PROMOTION, and enqueueForDoneTask for the candidate a MAIN-line task\'s DONE makes (M-F2) — one project_integration_job row (rank 60), idempotent on `ij:v1:<kind>:<promotionId>:<generation>`' },
