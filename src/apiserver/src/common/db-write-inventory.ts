...
export const STATEMENT_UNITS: readonly StatementUnit[] = [
  ...
  { at: "queue/queue.service.ts#buildSessionOrRequeue", class: "ONE_ROW_CAS", statements: 1, note: "The claim's compensation. One statement, outside any transaction on purpose: the transition it undoes committed in a transaction that is already gone, so there is nothing to re-run — and it must not be able to fail the request it is apologising for, which is why a conflict here is logged and swallowed rather than raised. The token in the predicate is what stops a compensation arriving after an activation, a cancel, a resume or a re-claim from putting a session that has moved on back in the queue." },
  ...
  { at: "realtime/reaper.service.ts#releaseExpiredClaim", class: "ONE_ROW_CAS", statements: 1, note: "Undoes one PENDING->RUNNING handover the runner never confirmed. The compare-and-set is the token the claim minted plus the status it is being taken out of plus the deadline, re-checked against the database clock — so a claim that activated, was cancelled, was resumed or was re-claimed between the sweep's read and this write matches nothing. Whether the session goes back is therefore decided here, not by what the sweep read." },
