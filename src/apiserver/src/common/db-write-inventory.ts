...
export const STATEMENT_UNITS: readonly StatementUnit[] = [
  ...
  { at: "projects/coordinator-wake.service.ts#insertClaim", class: "INSERT", statements: 1, note: "The wake claim: one INSERT with ON CONFLICT DO NOTHING against the partial unique index of migration 0173, RETURNING the id so the loser of a race learns it lost without a second read. Not in a transaction, deliberately — authorization runs between this statement and the release below, and holding a row lock across a call this unit does not time is how a claim becomes a queue." },
