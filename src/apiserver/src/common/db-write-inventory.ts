...
export const STATEMENT_UNITS: readonly StatementUnit[] = [
  ...
  { at: "projects/projects.service.ts#bindCodebase", class: "INSERT", statements: 1, note: "The one write that puts a `project_codebase` row in the table (`docs/project-source-contract.md` §3.1). One INSERT, no transaction: the ownership check and the runner check before it are refusals, and holding a row lock across them would make binding a queue rather than an answer. Nothing races it for the outcome — `(project_id, slot)` is UNIQUE, so a second concurrent bind of the same project loses on the index and is answered 409 permanently rather than retried. Its FOR KEY SHARE parents are the project and, for a RUNNER_LOCAL binding, the authority runner; neither is taken FOR UPDATE by anything that also writes here. `config_revision` and `root_commit_sha` are absent from the statement on purpose — the first is the trigger's (SR8) and the second is written once by whatever OBSERVES it (SR37)." },
