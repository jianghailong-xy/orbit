...
export const TRANSACTION_UNITS: readonly TransactionUnit[] = [
  ...
  },
  {
    ...
    shape: 'TX_RETRIED',
    locks: 'session FOR UPDATE via lockSessionLeaseOwner (rank 30); for a typed attempt result, its project FOR NO KEY UPDATE (rank 40) then task FOR UPDATE (rank 50); conversation_turn ACK, task_executable_attempt termination, continuation/diagnosis and legacy blocker/comment children (rank 60), then the database-maintained task dispatch epoch (rank 70). Later llm_usage and session_diff writes add no wait edge because their Session parent is already held. One write of the Session row per transaction (I3).',
  ...
  },
  {
    ...
    identity: 'The evidence uses the caller idempotency key when supplied, otherwise the stable tuple (task, actor type/id, source Session, criterion revision, evidence digest). Both are unique in PostgreSQL, and the task mutex makes the revision allocation exact.',
    isolation: '',
    attempts: 4,
    replay: 'Every attempt re-locks the Task, verifies the source Session belongs to it, derives the criterion snapshot and allocates MAX(revision)+1 inside the closure. A committed retry key returns its original evidence even if criteria later changed.',
    effects: 'None outside PostgreSQL inside. The transaction never updates Task/Session lifecycle state, comments, notifications or realtime. After commit one COMPLETION_EVIDENCE_REVISED input is routed; replay converges on the same fact key. Since 2026-09-02 the ledger derives nothing else: the judgment request this used to raise, and the delivery it filed, were removed.',
  ...
  },
  {
    ...
    identity: 'Both (task, source comment) and (task, caller idempotency key) are unique audit identities. The evidence stable-fact tuple remains independently unique; source and structured SHA-256 digests bind the exact reviewed inputs.',
    isolation: '',
    attempts: 4,
    replay: 'Every attempt re-locks the Task and re-reads the exact comment, source Session, current criterion and latest evidence revision. A committed exact replay returns the original import/evidence; reusing either source or key with changed actor, provenance, digest, review note or delivery policy is a 409.',
    effects: 'No comment parsing or external action occurs inside, and nothing after it: the human request this used to raise was removed on 2026-09-02.',
  ...
  },
  {
    ...
    isolation: '',
    attempts: 4,
    replay: 'The closure re-reads the task inside the owner mutex on every attempt — `current`, acceptance projects, hierarchy and edges are all derived there. A verifier verdict also re-locks both tasks before writing it. The single-statement branch is NOT retried: it owns no transaction.',
    effects: 'A verdict releases its subject through post-commit aggregation, which is what derives the subject status; dependency dispatch, project-acceptance reconciliation and realtime publication are also after commit. No project task-set scan runs.',
...
export const EXCLUDED_SOURCES: readonly ExcludedSource[] = [
  ...
  },
  {
    ...
    why: 'A PostgreSQL-spec fixture helper. It deliberately writes the production VERIFICATION verdict and terminal task facts so integration tests exercise canonical completion instead of bypassing its trigger; no application provider imports it.',
  },
  {
