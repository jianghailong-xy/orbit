...
export const TRANSACTION_UNITS: readonly TransactionUnit[] = [
  ...
  },
  {
    ...
    identity: 'The lease owner plus the ok/failure branch in the request; the ok branch is additionally keyed by the importSourceCwd CAS, which is what makes a retried ok a no-op rather than a re-applied title.',
    isolation: '',
    attempts: 4,
    replay: 'Nothing is derived inside the closure. The ok branch matches only a row that still carries the marker, so after a committed first attempt the re-run applies nothing; the failure branch re-writes the same FAILED + deletedAt shape, which the LIVE filter already excludes.',
...
export const STATEMENT_UNITS: readonly StatementUnit[] = [
  ...
  { at: "projects/project-acceptance.service.ts#confirmStandardSet", class: "INSERT", statements: 1, note: "The account owner confirming that one VERSION of a project's acceptance criteria expresses its goal (migration 0245) — the only writer `CONFIRM_ACCEPTANCE_CRITERIA` has. One INSERT, and deliberately no transaction and no project lock: an edit landing between the digest comparison and this statement can only make the row non-current, which the read reports by comparing the stored digest with the criteria as they stand. Nothing UPDATEs or DELETEs this relation, so a second confirmation is a second row rather than a rewrite of the first. Re-issuing a lost request writes a second row naming the same version, which says exactly what happened and changes no answer." },
