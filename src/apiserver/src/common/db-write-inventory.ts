...
export const TRANSACTION_UNITS: readonly TransactionUnit[] = [
  ...
  },
  {
    ...
    shape: 'TX_RETRIED',
    locks: 'project FOR NO KEY UPDATE (rank 40), then the owner-scoped idempotency receipt and at most one existing PENDING project_criteria_proposal row (rank 60). It never reaches project_acceptance_criterion_definition — that is the point of the unit.',
    ...
    isolation: '',
    attempts: 4,
    replay: 'Every attempt re-reads the effective criteria under the project mutex, so the criteria-set digest a proposal binds is the one that was in force when it committed. A victim leaves no proposal row and no supersession; a retry either returns the winner through its idempotency receipt or supersedes whatever is pending now, recording why.',
    effects: 'None, in the strongest sense this list has: the effective acceptance criteria are unchanged when this commits.',
  ...
  },
  {
    ...
    shape: 'TX_RETRIED',
    locks: 'project FOR NO KEY UPDATE (rank 40), then the owner-scoped decision receipt, the exact project_criteria_proposal row, project_acceptance_criterion_definition and project_completion_contract (rank 60).',
    ...
    isolation: '',
    attempts: 4,
    replay: 'The proposal, its card digest and the criteria set it was drafted against are re-read under the project mutex on every attempt, so a retry decides against the state the winner left. A victim applies no criterion at all: the write and the decision receipt are one transaction.',
    effects: 'None inside. Acceptance reconciliation starts only after the decision commits, and replaying the same key then returns the committed answer.',
    answer: 'Typed 503 from the global boundary. Stale cards, moved bases, settled proposals and non-owner principals are typed refusals, the last one PROJECT_CRITERIA_DECISION_ACTOR_FORBIDDEN.',
  },
  {
...
export const TRIGGER_WRITE_SOURCES: readonly TriggerWriteSource[] = [
  ...
  {"table":"outcome_fact_binding","trigger":"outcome_binding_transition_record","event":"AFTER INSERT","kind":"ROW/STATEMENT","since":"0196_outcome_binding_version_invalidation","takes":["outcome_active_obligation LOCK","outcome_active_obligation WRITE","outcome_binding_transition WRITE","outcome_obligation_event WRITE","outcome_obsolete_obligation WRITE","outcome_proof_obsolescence WRITE"]},
