...
export const TRANSACTION_UNITS: readonly TransactionUnit[] = [
  {
    ...
    shape: 'TX_RETRIED',
    locks: 'project_integration_job (rank 60) by primary key, then whatever the writes it implies take: session_merge_receipt (rank 60) for a landing, task_comment (rank 30\'s child, locked through its task foreign key FOR KEY SHARE) for the signal a no-commit answer leaves, project_open_item (rank 60) for a failure, and — for a landing that answered ALREADY_LANDED about a branch the task work did not end on — one more project_integration_job row (rank 60) for the generation that branch is owed (`queueLandingBehindTheWork`). Nothing above rank 60 is locked — the foreign keys of those children take `session`, `task` and `project` FOR KEY SHARE, which no status write conflicts with, and the job row itself is the only thing two runners could both want.',
...
export const TRANSACTION_PARTICIPANTS: readonly TransactionParticipant[] = [
  ...
  { at: 'projects/project-promotion.service.ts#supersedeLiveCandidates', under: 'projectPromotion.considerCandidate — retiring the candidate that was standing, inside the transaction that makes the new one (M-T6)' },
  { at: 'projects/project-integration-job.ts#queuePromotionJob', under: 'projectPromotion.considerCandidate for a CHECK_PROMOTION, projectPromotion.decide for a LAND_PROMOTION, applyPromotionJobResult for the LAND_PROMOTION the project\'s Automatic setting confirms (M-T11), and enqueueForDoneTask for the candidate a MAIN-line task\'s DONE makes (M-F2) — one project_integration_job row (rank 60), idempotent on `ij:v1:<kind>:<promotionId>:<generation>`' },
  ...
  { at: 'tasks/task-dispatch-refusal.ts#recordDispatchRefusal', under: "runnerApi.finalize — inside the same rank-30 Session transaction that settles a run the runner refused at its checkout, in place of postRunFailureComment and taking the same locks: one UPDATE of the rank-50 task's `dispatch_refusal` (a column no trigger's column list names, so the statement-level AFTER UPDATE triggers find no status or run_at change and write nothing) and one task_comment child row whose task foreign key is FOR KEY SHARE on that same row, after two plain reads (the task's prerequisite edges and their landing receipts) that name the prerequisite behind each missing commit" },
...
export const STATEMENT_UNITS: readonly StatementUnit[] = [
  ...
  { at: "projects/project-acceptance.service.ts#confirmStandardSet", class: "INSERT", statements: 2, note: "The account owner confirming that one VERSION of a project's acceptance criteria expresses its goal (migration 0245) — the only writer `CONFIRM_ACCEPTANCE_CRITERIA` has. Deliberately no transaction and no project lock: an edit landing between the digest comparison and the INSERT can only make the row non-current, which the read reports by comparing the stored digest with the criteria as they stand. Nothing UPDATEs or DELETEs this relation, so a second confirmation is a second row rather than a rewrite of the first. Re-issuing a lost request writes a second row naming the same version, which says exactly what happened and changes no answer. The second statement is that confirmation's other half — it STARTS the project, turning `coordinator_enabled` on, which `projects.service.ts#create` no longer writes. It is ONE_ROW_CAS shaped (`id` + owner + `coordinator_enabled = false`) rather than a blind write, so only the off→on transition writes: that is what keeps the `configRevision` bump every authorization-set write owes to one per change, and what stops a re-confirmation from firing this column's trigger — the completion-contract re-derivation. (It used to be two: 0290 retired `project_dispatch_authority_fanout`, the AFTER trigger this column also carried, which rewrote `dispatch_authority` on every task of the project and held that project's row for the whole rewrite. The CAS is still the right shape — a repeated confirmation is not a change — but the off→on transition no longer costs anything proportional to the project's size.) Not atomic with the INSERT and does not need to be: a confirmation with the second statement lost is re-confirmable, and the read that matters is `coordinator_enabled` itself. After both, and only when the CAS turned the project on, the coordinator conversation is told (projects/project-started.ts#tellCoordinatorProjectStarted, one `sessions.createTurn` keyed by this confirmation's id) — outside any transaction, and a telling that fails is logged rather than failing the confirmation." },
  { at: "projects/project-done-derived.ts#storeDerivedProjectStatus", class: "ONE_ROW_CAS", statements: 1, note: "The projection of `project.status` from committed facts — every stated criterion satisfied, landed and not written by the session producing its own evidence, and a confirmation naming the version of the criteria that stands today. One conditional UPDATE, whose predicate is the value the row must currently hold: OPEN to write DONE, DONE to write OPEN. That is what makes two post-commit edges deriving the same answer write the row once rather than race to write the same value twice, and it is also why CANCELLED is unreachable in both directions without a second statement to say so. Deliberately no transaction and no project lock: everything it reads is already committed, and a task write or a confirmation landing between the read and this statement only means the next edge re-derives from the newer rows — the answer is a function of the rows, so a stale one is corrected rather than compounded. Re-issuing it is the same statement against the same rows and writes nothing the first one did not." },
  ...
  { at: "projects/session-source.ts#freezeSessionSourcePin", class: "ONE_ROW_CAS", statements: 1, note: "The SOURCE pin (`docs/project-source-contract.md` §6.3), reached through `runnerApi.pinSessionSource`. ONE conditional UPDATE decides it: a repeated dispatch, two runners racing through a takeover and a re-issue of a request whose response was lost all arrive here, and at most one can move SELECTED -> PINNED. That is what makes a re-issue safe without a transaction — the second one reads the winner's pin back instead of overwriting it. The reads either side hold nothing between statements, and migration 0231's freeze guard is a second, independent refusal of every later write to these columns." },
  ...
  { at: "runner-api/runner-api.controller.ts#codexAccountRemoveResult", class: "ONE_ROW_CAS", statements: 1, note: "What one account removal came to, as the runner reports it. The predicate is the removal the report names — `pending`, the account, and the `codex_account_remove_at` it was asked at — not just the runner id: a report about a removal the row has moved past (the person asked again), or about a different account, matches no row and is dropped. A success clears the failure message the last one left; re-POSTing the same outcome writes the same values." },
  ...
  { at: "runners/runners.service.ts#removeCodexAccount", class: "ONE_ROW_BY_KEY", statements: 1, note: "Asking one machine to delete one Codex account slot. One slot per runner, like the sign-in relay: a second ask replaces the first, and it answers whatever account was named last. The account itself is not read or written here — it lives on the runner — so this is the request row and nothing else; a runner that has not declared it can remove accounts is refused before the write." },
