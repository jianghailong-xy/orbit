...
export const TRANSACTION_UNITS: readonly TransactionUnit[] = [
  ...
  {
    at: 'projects/project-ownership-refile.service.ts#refile',
    shape: 'TX_RETRIED',
    locks: 'user FOR KEY SHARE (rank 10, the mode the replacement insert own FK takes), both project rows FOR NO KEY UPDATE (40, sorted — the mode `task_supersession_project_lock_order` takes when the abandon fires it), then the mis-filed task FOR UPDATE (50). Same acquisition as `project-handoff.service.ts#declare`, because it is the same crossing from the other side.',
    identity: '(source task, `project.ownership_refiled`) — migration 0156 partial unique index. A double click, two replicas and a retried timeout all reach the one replacement, and the loser reads it back rather than reporting a constraint violation.',
    isolation: '',
    attempts: 4,
    replay: 'The whole plan is re-derived inside the closure from rows read under those locks — the gate answer, the session counts, the target project status and whether a replacement already exists — so a re-run decides against the state the winner left, which for the loser of a race is ALREADY_REFILED. Neither write can be half-applied: the replacement and the abandon are one transaction, and a victim leaves a task that is still mis-filed and still refused.',
    effects: 'None inside. In particular no Session is written, ended, cancelled or completed anywhere on this path — the planner refuses every task that has one.',
    answer: 'Typed 503 from the global boundary; a refused repair is already a 400/409 with its own sentence.',
  },
  {
  ...
  {
    at: 'projects/verification-finding.service.ts#submit',
    shape: 'TX_RETRIED',
    locks: 'task FOR UPDATE (rank 50) through the ledger `lockAndRead` this takes first — the same lock a judgment takes, which is what stops a finding and a judgment about one task interleaving. Then the artifact it files: a defect subtask, a prerequisite task or a `project_blocker` row.',
    identity: "FD1's dedup key, derived above the writes from the project, the subject, the scope revision and the failure fingerprint. The effect key derived from it is the filed Task's `idempotency_key`, so a crash between the row and the Task it files cannot file a second.",
    isolation: '',
    attempts: 4,
    replay: 'The dedup key is looked up FIRST inside the closure, so a re-run returns the committed finding having written nothing — not even the ledger row, which would otherwise charge the counters again before discovering the conflict. The triage is re-derived from the state read under the lock, so a re-run triages what the winner left.',
    effects: 'None inside.',
    answer: 'Typed 503 from the global boundary; a duplicate is not an error — it comes back as the original finding with `duplicate: true`.',
  },
  {
...
export const TRANSACTION_PARTICIPANTS: readonly TransactionParticipant[] = [
  ...
  { at: 'projects/verification-finding.service.ts#fileTask', under: "verificationFinding.submit — §3's defect subtask, prerequisite task or re-plan sibling, filed under the same lock and the same effect key" },
...
export const STATEMENT_UNITS: readonly StatementUnit[] = [
  ...
  { at: "projects/project-ownership-audit.service.ts#scan", class: "RAW_FENCE", statements: 1, note: "Unit L6's boot audit, and the same shape as the failure scan beside it: one conditional UPDATE of `project_runtime.next_wake_at`, guarded so a project already due keeps its own instant and a project mid-pass is left to its holder. It writes no task, no blocker and no action — the row a person acts on is raised by the reconcile pass, so running the scan twice cannot produce two of anything." },
...
export const TRIGGER_WRITE_SOURCES: readonly TriggerWriteSource[] = [
  ...
  { table: "task", trigger: "task_creator_scope_immutable_guard", event: "BEFORE UPDATE OF \"creator_coordinator_project_id\", \"creator_coordinator_generation\"", kind: "ROW/STATEMENT", since: "0156_task_creator_scope_ownership_gate", takes: [] },
  { table: "task", trigger: "task_creator_scope_retire_on_move", event: "BEFORE UPDATE OF \"project_id\"", kind: "ROW/STATEMENT", since: "0156_task_creator_scope_ownership_gate", takes: [] },
