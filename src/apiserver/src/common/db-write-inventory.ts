...
export const TRANSACTION_UNITS: readonly TransactionUnit[] = [
  {
    ...
    shape: 'TX_RETRIED',
    locks: 'project_integration_job (rank 60) by primary key, then whatever the writes it implies take: session_merge_receipt (rank 60) for a landing, task_comment (rank 30\'s child, locked through its task foreign key FOR KEY SHARE) for the signal a no-commit answer leaves, and project_open_item (rank 60) for a failure. Nothing above rank 60 is locked — the foreign keys of those children take `session`, `task` and `project` FOR KEY SHARE, which no status write conflicts with, and the job row itself is the only thing two runners could both want.',
