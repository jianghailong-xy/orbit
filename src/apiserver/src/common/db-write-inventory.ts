...
export const TRANSACTION_UNITS: readonly TransactionUnit[] = [
  ...
  {
    at: 'projects/project-acceptance.service.ts#proposeCriteriaChange',
    shape: 'TX_RETRIED',
    locks: 'project FOR NO KEY UPDATE (rank 40), project_completion_contract FOR UPDATE, then the owner-scoped idempotency receipt and at most one existing PENDING project_criteria_proposal row (rank 60). It never reaches project_acceptance_criterion_definition — that is the point of the unit.',
    identity: 'The caller idempotency key, owner-scoped, plus a digest of the exact submitted proposal. A repeat of the same key and the same body returns the committed proposal; the same key with a different body is a typed collision rather than a second proposal.',
    isolation: '',
    attempts: 4,
    replay: 'Every attempt refreshes the contract under the project mutex, so the base digest a proposal binds is the one that was current when it committed. A victim leaves no proposal row and no supersession; a retry either returns the winner through its idempotency receipt or supersedes whatever is pending now, recording why.',
    effects: 'None, in the strongest sense this list has: the effective acceptance criteria, the contract digest and the standing Owner Ratification are unchanged when this commits.',
    answer: 'Typed 503 from the global boundary. A no-op proposal, an idempotency collision and a malformed criteria set are typed refusals.',
  },
  {
    at: 'projects/project-acceptance.service.ts#decideCriteriaProposal',
    shape: 'TX_RETRIED',
    locks: 'project FOR NO KEY UPDATE (rank 40), the owner-scoped decision receipt and the exact project_criteria_proposal row, project_completion_contract FOR UPDATE, then project_acceptance_criterion_definition, the project_owner_decision_request for the resulting digest and the append-only project_owner_ratification child (rank 60). An APPROVE finally moves matching pending task_auto_dispatch_wakeup children to due-now (rank 70).',
    identity: 'The owner idempotency key plus the exact proposal id and the digest of the rendering the decision was taken on. A decision carrying a card digest the proposal no longer has is refused, not applied.',
    isolation: '',
    attempts: 4,
    replay: 'The proposal, its card digest and the base contract are re-read under the project mutex on every attempt, so a retry decides against the state the winner left. A victim applies no criterion, advances no digest and appends no ratification — the three are one transaction, which is what keeps the project from ever standing on criteria nobody ratified.',
    effects: 'None inside. The wake is a database retry clock; acceptance reconciliation starts only after the decision commits, and replaying the same key then returns the committed answer.',
    answer: 'Typed 503 from the global boundary. Stale cards, moved bases, settled proposals and non-owner principals are typed refusals, the last one OWNER_RATIFICATION_ACTOR_FORBIDDEN.',
  },
  {
  ...
  },
  {
    ...
    shape: 'TX_RETRIED',
    locks: 'session FOR UPDATE (rank 30), then conversation_turn FOR UPDATE SKIP LOCKED and the claim UPDATE plus, for v2 shell work, one immutable task_executable_admission INSERT (rank 60). Rejection ACKs the turn in that same rank-60 lane and never reaches a process.',
    identity: 'The runner and lease generation asking; the claimed turn is chosen inside. A v2 admission is unique by turn id, and binds the current plan digest plus this poller\'s advertised capability.',
    isolation: '',
    attempts: 4,
  ...
  {
    at: 'runner-api/runner-api.controller.ts#startExecutableAcceptanceAttempt',
    shape: 'TX_RETRIED',
    locks: 'The attempt INSERT trigger locks task FOR UPDATE (rank 50), then task_executable_admission FOR UPDATE and writes admission/attempt children (rank 60); the already-held Task row receives the execution-attempt count increment. Foreign-key checks add only compatible parent key-share locks.',
    identity: 'The admission id is unique on task_executable_attempt. A repeated start returns that row; only the insert winner can move spawnCount 0→1 and increment executionAttemptCount.',
    isolation: '',
    attempts: 4,
    replay: 'The admission decision and existing-attempt check are re-read inside the closure. A deadlock victim commits neither spawnCount, attempt nor task budget; retry either performs the one atomic start transition or observes the winner and returns its attempt.',
    effects: 'None. The HTTP success is returned before the runner calls cmd.Start; a rejected or ambiguous database outcome never authorizes process spawn.',
    answer: 'Typed 503; the runner retries this idempotent handshake and still cannot spawn until it receives the committed attempt id.',
  },
  {
    at: 'runner-api/runner-api.controller.ts#turnComplete',
    shape: 'TX_RETRIED',
    locks: 'session FOR UPDATE via lockSessionLeaseOwner (rank 30); for an executable judgment or typed attempt result, its project FOR NO KEY UPDATE (rank 40) then task FOR UPDATE (rank 50); conversation_turn ACK, rolling-v1 task_completion_evidence/idempotency, task_executable_attempt termination, its trigger-written failure receipt, continuation/diagnosis, its trigger-written canonical obligation/wakeup outbox, task_executable_judgment_result, task_judgment_request and legacy blocker/comment children (rank 60), then the database-maintained task dispatch epoch (rank 70). Later llm_usage and session_diff writes add no wait edge because their Session parent is already held. One write of the Session row per transaction (I3).',
    identity: 'The turn id, usage and shell/typed attempt result in the request body are outside the closure. Admission id plus attempt id binds one ADMITTED process; termination is a one-way update and continuation is unique by attempt. A non-success receipt is unique by attempt, while its obligation/outbox key is the digest of goal/task, binding revision, executable attempt generation and failure fingerprint. The OPEN request is already uniquely bound to criterion revision and evidence digest; its executable result is unique by request. A queued acceptance turn also binds its command/plan digest in client_turn_id, so a retry compares the same evidence rather than a declaration edited while it ran. A rolling v1/no-request callback uses `legacy-v1-turn-complete:<turnId>:<evidenceDigest>` plus the unique request fact and request-result keys; it retains the v1 Session/turn/runner/lease identity and creates no admission or typed attempt.',
    isolation: '',
    attempts: 4,
    replay: 'The duplicate-ack check, park, merge-state clear and billing accrual are all taken from rows read under their locks inside the closure. The typed termination, immutable failure receipt, bounded continuation/diagnosis, canonical continuation obligation/outbox, command result, request decision, derived task status and raw-output comment — or the mutually exclusive unavailable signal — are written only by the same first ACK; a victim leaves all of them absent, and a retry re-locks the current declaration/request before deriving anything. Receipt/materialization functions verify a standing digest/key rather than adopting unequal replay. The v1 bridge finds-or-creates the exact evidence/request fact inside the same transaction and verifies a standing result byte-for-byte before deciding it, so a rollback exposes none and an OPEN+result retry never attempts a second unique result.',
    effects: 'None inside; failure-continuation delivery, attempt-budget accounting, EXECUTABLE_RESULT_RECORDED consumption and realtime publication are after commit. The failure-continuation kick is only a latency nudge over the already-committed outbox; the periodic sweep owns crash recovery. The input route is replayed even for a duplicate ACK so an authorization refusal does not burn the immutable result fact. A failed transaction appends CONTROL_PLANE_COMMIT_REJECTED only from a separate best-effort transaction after rollback; a committed ACK appends recovery outside the monitored transaction. Neither observation can turn the callback result into another 5xx, and the independent watchdog derives the same stable obligation if either edge write is absent.',
...
export const TRANSACTION_PARTICIPANTS: readonly TransactionParticipant[] = [
  ...
  { at: 'runner-api/runner-api.controller.ts#ensureLegacyExecutableJudgmentRequest', under: 'runnerApi.turnComplete — after the rank-30 Session, rank-40 Project and rank-50 Task locks; it writes only rolling-v1 evidence/idempotency/request children at rank 60 and is replayed with the callback transaction' },
  ...
  { at: 'sessions/sessions.service.ts#linkStartupFragmentAttachments', under: 'sessions.createTurn — after the rank-30 Session lock and startup-fragment INSERT; it links only prevalidated unscoped attachment children inside that same retried transaction' },
  ...
  { at: 'sessions/current-work-delivery.ts#terminalizePendingStartupContexts', under: 'runnerApi activate/turn-complete/finalize/release, sessions interrupt/end and realtime reaper — each caller already owns the rank-30 Session transaction; this participant settles only matching startup-fragment children' },
...
export const STATEMENT_UNITS: readonly StatementUnit[] = [
  ...
  { at: "outcome-reconciler/delivery-attestation.service.ts#recordAttestation", class: "INSERT", statements: 1, note: "One stored-function call validates the exact current delivery binding and provider replay identity before appending one immutable repository receipt." },
  { at: "outcome-reconciler/delivery-attestation.service.ts#recordVerification", class: "INSERT", statements: 1, note: "One stored-function call appends a clean-target result only when an exact trusted integration receipt already names that SHA/content pair." },
  { at: "outcome-reconciler/delivery-attestation.service.ts#registerBinding", class: "INSERT", statements: 1, note: "One stored-function call checks the current canonical binding and appends one target revision; the idempotency and revision keys collapse replay." },
  ...
  { at: "projects/failure-continuation.service.ts#sweep", class: "RAW_FENCE", statements: 1, note: "One bounded stored-function transaction materializes only committed ACTIVE goal-actionable DIAGNOSIS rows and requeues only a missing/expired/invalid delivery. The immutable receipt/key collapse replay; SKIP LOCKED makes overlapping replicas disjoint." },
  { at: "projects/failure-continuation.service.ts#claimDue", class: "RAW_FENCE", statements: 1, note: "One SELECT ... FOR UPDATE SKIP LOCKED + UPDATE stored-function call claims due rows. Lease token and monotonically increasing generation fence every later result, while the preplanned Session id is stable across generations." },
  { at: "projects/failure-continuation-controller.service.ts#routeClaim", class: "RAW_FENCE", statements: 1, note: "One lease-fenced stored-function call reads the immutable attempt plus one binding/capability snapshot, serializes the project/fingerprint lineage with an advisory lock, and appends exactly one replayable route decision. The decision owns its evidence streak; no process-memory or daily-budget counter participates." },
  { at: "projects/failure-continuation.service.ts#ack", class: "ONE_ROW_CAS", statements: 1, note: "One token+generation compare-and-set records DELIVERED only after it verifies the exact SESSION_OPENED wake and preplanned Session id." },
  { at: "projects/failure-continuation.service.ts#retry", class: "ONE_ROW_CAS", statements: 1, note: "One token+generation compare-and-set returns this leased row to PENDING. A stale claimant matches nothing after takeover." },
  { at: "projects/failure-continuation.service.ts#cancel", class: "ONE_ROW_CAS", statements: 1, note: "One token+generation compare-and-set cancels a source that is no longer current; the periodic sweep cannot revive it unless the canonical ACTIVE/no-successor predicates are true again." },
  { at: "projects/projects.service.ts#requestCompletionAckOwnerDecision", class: "RAW_FENCE", statements: 1, note: "One completion_ack_request_owner_decision call is the whole write transaction: it takes the revision-scoped delivery-authority advisory lock, proves the exact current non-revoked delivery/runner/Session, inserts the request plus append-only binding, and moves the same AGENT-owned standing to OWNER_DECISION. Stable request and binding digests make replay return the same child rather than mint another obligation." },
  { at: "projects/projects.service.ts#decideCompletionAckOwnerDecision", class: "RAW_FENCE", statements: 1, note: "One completion_ack_decide_owner_decision call atomically spends the exact tenant/project/request/revision callback, records the idempotent decision, and schedules the same AGENT-owned coordination to resume. It cannot close or revise the canonical completion obligation." },
...
export const TRIGGER_WRITE_SOURCES: readonly TriggerWriteSource[] = [
  {"table":"completion_ack_coordinator_delivery_adoption","trigger":"completion_ack_delivery_adoption_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0202_completion_ack_persistent_coordinator","takes":[]},
  {"table":"completion_ack_coordinator_delivery_adoption","trigger":"completion_ack_delivery_adoption_insert_guard","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0202_completion_ack_persistent_coordinator","takes":["completion_ack_coordinator_delivery_plan LOCK","completion_ack_coordinator_delivery_receipt LOCK","outcome_coordinator_lease LOCK","outcome_coordinator_obligation LOCK"]},
  {"table":"completion_ack_coordinator_delivery_plan","trigger":"completion_ack_delivery_plan_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0202_completion_ack_persistent_coordinator","takes":[]},
  {"table":"completion_ack_coordinator_delivery_plan","trigger":"completion_ack_delivery_plan_insert_guard","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0202_completion_ack_persistent_coordinator","takes":["completion_ack_obligation_revision LOCK","outcome_coordinator_lease LOCK","outcome_coordinator_obligation LOCK"]},
  {"table":"completion_ack_coordinator_delivery_receipt","trigger":"completion_ack_delivery_receipt_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0202_completion_ack_persistent_coordinator","takes":[]},
  {"table":"completion_ack_coordinator_delivery_receipt","trigger":"completion_ack_delivery_receipt_insert_guard","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0202_completion_ack_persistent_coordinator","takes":["completion_ack_coordinator_delivery_plan LOCK","outcome_coordinator_lease LOCK","outcome_coordinator_obligation LOCK","project LOCK","project_coordinator_wake LOCK","session LOCK"]},
  {"table":"completion_ack_delivery_progress_event","trigger":"completion_ack_delivery_progress_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0203_completion_ack_delivery_liveness","takes":[]},
  {"table":"completion_ack_delivery_progress_event","trigger":"completion_ack_delivery_progress_insert_guard","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0203_completion_ack_delivery_liveness","takes":["completion_ack_coordinator_delivery_receipt LOCK"]},
  {"table":"completion_ack_fact","trigger":"completion_ack_fact_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0201_completion_ack_canonical_obligation","takes":[]},
  {"table":"completion_ack_fact","trigger":"completion_ack_fact_before_insert","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0201_completion_ack_canonical_obligation","takes":[]},
  {"table":"completion_ack_fact","trigger":"completion_ack_fact_observation_after_insert","event":"AFTER INSERT","kind":"ROW/STATEMENT","since":"0201_completion_ack_canonical_obligation","takes":["completion_ack_observation_register WRITE"]},
  {"table":"completion_ack_obligation_event","trigger":"completion_ack_event_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0201_completion_ack_canonical_obligation","takes":[]},
  {"table":"completion_ack_obligation_event","trigger":"completion_ack_event_before_insert","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0201_completion_ack_canonical_obligation","takes":["completion_ack_fact LOCK"]},
  {"table":"completion_ack_obligation_revision","trigger":"completion_ack_revision_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0201_completion_ack_canonical_obligation","takes":[]},
  {"table":"completion_ack_obligation_revision","trigger":"completion_ack_revision_before_insert","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0201_completion_ack_canonical_obligation","takes":[]},
  {"table":"completion_ack_observation_register","trigger":"completion_ack_observation_register_guard","event":"BEFORE INSERT OR UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0201_completion_ack_canonical_obligation","takes":[]},
  {"table":"completion_ack_owner_decision_binding","trigger":"completion_ack_owner_decision_binding_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0204_completion_ack_owner_decision_protocol","takes":[]},
  {"table":"completion_ack_owner_decision_binding","trigger":"completion_ack_owner_decision_binding_insert_guard","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0204_completion_ack_owner_decision_protocol","takes":["outcome_coordinator_owner_decision_request LOCK","session LOCK"]},
  {"table":"completion_ack_remediation_action","trigger":"completion_ack_remediation_action_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0202_completion_ack_persistent_coordinator","takes":[]},
  {"table":"completion_ack_remediation_action","trigger":"completion_ack_remediation_action_insert_guard","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0202_completion_ack_persistent_coordinator","takes":["completion_ack_coordinator_delivery_plan LOCK","completion_ack_coordinator_delivery_receipt LOCK","task LOCK"]},
  {"table":"completion_ack_rollout_epoch","trigger":"completion_ack_rollout_epoch_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0201_completion_ack_canonical_obligation","takes":[]},
  {"table":"conversation_turn","trigger":"conversation_turn_completion_ack_insert_guard","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0201_completion_ack_canonical_obligation","takes":[]},
  {"table":"conversation_turn","trigger":"conversation_turn_completion_ack_lease_guard","event":"BEFORE UPDATE OF status, delivered_at, lease_generation","kind":"ROW/STATEMENT","since":"0201_completion_ack_canonical_obligation","takes":[]},
  ...
  {"table":"executable_runtime_binding","trigger":"executable_runtime_binding_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0206_watchdog_current_binding","takes":[]},
  {"table":"executable_runtime_binding_fact","trigger":"executable_runtime_binding_fact_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0206_watchdog_current_binding","takes":[]},
  ...
  {"table":"outcome_binding_ratification","trigger":"outcome_binding_ratification_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0196_outcome_binding_version_invalidation","takes":[]},
  ...
  {"table":"outcome_coordinator_attempt_result","trigger":"outcome_coordinator_completion_attempt_result_guard","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0202_completion_ack_persistent_coordinator","takes":["outcome_coordinator_obligation LOCK"]},
  ...
  {"table":"outcome_coordinator_obligation","trigger":"outcome_coordinator_completion_terminal_guard","event":"BEFORE UPDATE OF source_type, obligation_id, obligation_revision, requested_owner, durable_owner, capability, status","kind":"ROW/STATEMENT","since":"0202_completion_ack_persistent_coordinator","takes":[]},
  ...
  {"table":"outcome_coordinator_owner_decision_request","trigger":"outcome_coordinator_completion_owner_decision_binding_required","event":"AFTER INSERT OR UPDATE OF status, decision, decision_digest","kind":"CONSTRAINT","since":"0204_completion_ack_owner_decision_protocol","takes":[]},
  {"table":"outcome_coordinator_owner_decision_request","trigger":"outcome_coordinator_owner_request_binding","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0199_outcome_actor_surfaces","takes":[]},
  ...
  {"table":"outcome_delivery_attestation","trigger":"outcome_delivery_attestation_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0198_outcome_delivery_attestation","takes":[]},
  {"table":"outcome_delivery_binding","trigger":"outcome_delivery_binding_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0198_outcome_delivery_attestation","takes":[]},
  {"table":"outcome_delivery_verification","trigger":"outcome_delivery_verification_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0198_outcome_delivery_attestation","takes":[]},
  ...
  {"table":"outcome_fact_binding","trigger":"outcome_binding_transition_record","event":"AFTER INSERT","kind":"ROW/STATEMENT","since":"0196_outcome_binding_version_invalidation","takes":["outcome_active_obligation LOCK","outcome_active_obligation WRITE","outcome_binding_ratification WRITE","outcome_binding_transition WRITE","outcome_obligation_event WRITE","outcome_obsolete_obligation WRITE","outcome_proof_obsolescence WRITE"]},
  ...
  {"table":"outcome_obligation_reduction","trigger":"outcome_obligation_reduction_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0196_outcome_binding_version_invalidation","takes":[]},
  ...
  {"table":"outcome_obligation_successor","trigger":"outcome_obligation_successor_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0196_outcome_binding_version_invalidation","takes":[]},
  {"table":"outcome_obsolete_obligation","trigger":"outcome_obsolete_obligation_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0196_outcome_binding_version_invalidation","takes":[]},
  ...
  {"table":"outcome_proof_obsolescence","trigger":"outcome_proof_obsolescence_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0196_outcome_binding_version_invalidation","takes":[]},
  {"table":"outcome_proof_successor","trigger":"outcome_proof_successor_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0196_outcome_binding_version_invalidation","takes":[]},
  ...
  {"table":"project_acceptance_run","trigger":"project_acceptance_run_closure_guard","event":"BEFORE UPDATE","kind":"ROW/STATEMENT","since":"0215_acceptance_run_closure","takes":[]},
  ...
  {"table":"project_owner_decision_request","trigger":"project_owner_decision_bind_revision","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0196_outcome_binding_version_invalidation","takes":[]},
  {"table":"project_owner_ratification","trigger":"project_authority_envelope_ratified","event":"AFTER INSERT","kind":"ROW/STATEMENT","since":"0216_project_authority_envelope","takes":["project_completion_contract WRITE"]},
  {"table":"project_owner_ratification","trigger":"project_owner_ratification_bind_revision","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0196_outcome_binding_version_invalidation","takes":[]},
  ...
  {"table":"project_ratified_action_intent","trigger":"project_action_intent_bind_full_revision","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0196_outcome_binding_version_invalidation","takes":["outcome_fact_stream LOCK"]},
  ...
  {"table":"run_event","trigger":"run_event_completion_ack_ingestion_guard","event":"BEFORE INSERT OR UPDATE OF ingested_at, ingested_by_runner_id, ingested_under_lease_generation","kind":"ROW/STATEMENT","since":"0201_completion_ack_canonical_obligation","takes":[]},
  ...
  {"table":"session","trigger":"session_completion_ack_dispatch_insert_guard","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0201_completion_ack_canonical_obligation","takes":[]},
  {"table":"session","trigger":"session_completion_ack_dispatch_revive_guard","event":"BEFORE UPDATE OF task_id, starts_task_work, deleted_at, status","kind":"ROW/STATEMENT","since":"0201_completion_ack_canonical_obligation","takes":[]},
  ...
  {"table":"session","trigger":"session_verification_subject_guard_insert","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0207_verification_subject_dispatch_guard","takes":[]},
  ...
  {"table":"task","trigger":"task_completion_ack_remediation_action","event":"AFTER INSERT","kind":"ROW/STATEMENT","since":"0202_completion_ack_persistent_coordinator","takes":["completion_ack_remediation_action WRITE"]},
  {"table":"task","trigger":"task_completion_ack_remediation_reactivation_guard","event":"BEFORE UPDATE OF status","kind":"ROW/STATEMENT","since":"0202_completion_ack_persistent_coordinator","takes":[]},
  ...
  {"table":"task","trigger":"task_executable_plan_bind","event":"BEFORE INSERT OR UPDATE OF \"completion_criterion\", \"acceptance_command\", \"acceptance_expected_exit_code\", \"acceptance_timeout_seconds\", \"acceptance_owner_timeout_ceiling_seconds\", \"acceptance_policy_timeout_ceiling_seconds\", \"acceptance_schema_revision\", \"acceptance_capability_revision\"","kind":"ROW/STATEMENT","since":"0200_executable_acceptance_runtime_contract","takes":[]},
  ...
  {"table":"task","trigger":"task_verification_subject_live_session_guard","event":"BEFORE UPDATE OF \"completion_criterion\", \"verifies_task_id\"","kind":"ROW/STATEMENT","since":"0207_verification_subject_dispatch_guard","takes":[]},
  ...
  {"table":"task","trigger":"zz_task_completion_ack_remediation_criterion_insert_guard","event":"AFTER INSERT","kind":"ROW/STATEMENT","since":"0202_completion_ack_persistent_coordinator","takes":[]},
  {"table":"task","trigger":"zz_task_completion_ack_remediation_criterion_update_guard","event":"AFTER UPDATE OF completion_criterion, status, project_id, creator_session_id","kind":"ROW/STATEMENT","since":"0202_completion_ack_persistent_coordinator","takes":[]},
  ...
  {"table":"task_executable_admission","trigger":"task_executable_admission_immutable_guard","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0200_executable_acceptance_runtime_contract","takes":[]},
  {"table":"task_executable_attempt","trigger":"task_executable_attempt_failure_continuation_receipt","event":"AFTER UPDATE OF terminated_at, termination_kind, actual_exit_code, signal, raw_output, output_truncated, failure_fingerprint","kind":"ROW/STATEMENT","since":"0210_failure_continuation_trigger","takes":["failure_continuation_attempt_receipt WRITE"]},
  {"table":"task_executable_attempt","trigger":"task_executable_attempt_no_delete","event":"BEFORE DELETE","kind":"ROW/STATEMENT","since":"0200_executable_acceptance_runtime_contract","takes":[]},
  {"table":"task_executable_attempt","trigger":"task_executable_attempt_start_guard","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0200_executable_acceptance_runtime_contract","takes":["task LOCK","task WRITE","task_executable_admission LOCK","task_executable_admission WRITE"]},
  {"table":"task_executable_attempt","trigger":"task_executable_attempt_termination_guard","event":"BEFORE UPDATE","kind":"ROW/STATEMENT","since":"0200_executable_acceptance_runtime_contract","takes":[]},
  {"table":"task_executable_continuation","trigger":"task_executable_continuation_failure_wakeup","event":"AFTER INSERT","kind":"ROW/STATEMENT","since":"0210_failure_continuation_trigger","takes":["failure_continuation_attempt_receipt WRITE","failure_continuation_obligation WRITE","failure_continuation_wakeup_outbox WRITE"]},
  {"table":"task_executable_diagnosis","trigger":"task_executable_diagnosis_append_only","event":"BEFORE UPDATE OR DELETE","kind":"ROW/STATEMENT","since":"0200_executable_acceptance_runtime_contract","takes":[]},
  ...
  {"table":"task_judgment_request","trigger":"task_judgment_request_verifier_role_guard","event":"BEFORE INSERT","kind":"ROW/STATEMENT","since":"0192_verifier_role_completion","takes":[]},
  {"table":"task_judgment_request","trigger":"task_open_verification_request_guard","event":"AFTER INSERT OR UPDATE OF \"status\"","kind":"CONSTRAINT","since":"0192_verifier_role_completion","takes":[]},
...
export const EXCLUDED_SOURCES: readonly ExcludedSource[] = [
  ...
  },
  {
    ...
    why: 'A PostgreSQL-spec fixture helper. It deliberately writes the production HUMAN_SIGNOFF evidence, request, decision and terminal task facts so integration tests exercise canonical completion instead of bypassing its trigger; no application provider imports it.',
  },
  {
    path: 'projects/project-ratification-test-helper.ts',
    why: 'A PostgreSQL-spec fixture helper. It invokes the production owner-ratification SQL boundary to prepare an authorised fixture and is not reachable from an application module or HTTP route.',
  },
  {
    path: 'outcome-reconciler/outcome-closed-test-helper.ts',
    why: 'A PostgreSQL-spec fixture helper. It records a complete production-style canonical outcome cut for gate integration tests and is never registered by the application module or invoked by an HTTP route.',
  },
];
