...
export const TRANSACTION_UNITS: readonly TransactionUnit[] = [
  {
    ...
    shape: 'TX_RETRIED',
    locks: 'project_integration_job (rank 60) by primary key, then whatever the writes it implies take: session_merge_receipt (rank 60) for a landing, task_comment (rank 30\'s child, locked through its task foreign key FOR KEY SHARE) for the signal a no-commit answer leaves, project_open_item (rank 60) for a failure, the project_promotion row (rank 60) and its project_open_item card (rank 60) for a promotion\'s transition, and — for a landing that answered ALREADY_LANDED about a branch the task work did not end on — one more project_integration_job row (rank 60) for the generation that branch is owed (`queueLandingBehindTheWork`), as there are one project_promotion row (rank 60), one project_open_item card (rank 60) and one CHECK_PROMOTION row (rank 60) for the candidate a task\'s work is owed when a check found it was looking at a branch the work did not end on (`refileCandidateBehindTheWork`). Nothing above rank 60 is locked — the foreign keys of those children take `session`, `task` and `project` FOR KEY SHARE, which no status write conflicts with, and the job row itself is the only thing two runners could both want.',
