...
export class RunnersController {
  ...
  @Patch(':id')
  update(@CurrentUser() user: AuthUser, @Param('id', Base62UuidPipe) id: string, @Body() dto: UpdateRunnerDto) {
  ...
  @Get(':id/login')
  loginState(@CurrentUser() user: AuthUser, @Param('id', Base62UuidPipe) id: string) {
    return this.runners.getLoginState(user.userId, id);
  }

  @Post(':id/login')
  startLogin(@CurrentUser() user: AuthUser, @Param('id', Base62UuidPipe) id: string, @Body() dto: StartLoginDto) {
  ...
  @Delete(':id/login')
  cancelLogin(@CurrentUser() user: AuthUser, @Param('id', Base62UuidPipe) id: string) {
  ...
  @Get(':id/install')
  installState(@CurrentUser() user: AuthUser, @Param('id', Base62UuidPipe) id: string) {
  ...
  @Delete(':id/install')
  cancelInstall(@CurrentUser() user: AuthUser, @Param('id', Base62UuidPipe) id: string) {
  ...
  startEngineUpdate(@CurrentUser() user: AuthUser, @Param('id', Base62UuidPipe) id: string) {
    return this.runners.startEngineUpdate(user.userId, id);
  }

  @Post(':id/rotate-token')
  rotateToken(@CurrentUser() user: AuthUser, @Param('id', Base62UuidPipe) id: string) {
