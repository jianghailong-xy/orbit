...
function canonicalPlanJson(value: Record<string, string | number>): string {
...
export function executableFailureFingerprint(input: {
  ...
  return sha256(canonicalPlanJson({
    evaluationPlanDigest: input.evaluationPlanDigest,
    terminationKind: input.terminationKind,
    actualExitCode: input.actualExitCode == null ? 'NULL' : input.actualExitCode,
