...
async function fixture(
  ...
  await db.conversationTurn.create({
    data: {
      ...
      status: 'IN_FLIGHT',
      // The claim and the delivery are one write in `dequeueTurn`, and `turnComplete`'s
      // idempotency ack matches on `delivered_at IS NOT NULL`. A turn that claimed IN_FLIGHT
      // without ever being delivered is not a state the runner door produces, and the
      // completion would be discarded before the acceptance round below is ever minted.
