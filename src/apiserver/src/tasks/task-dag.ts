 ...
 *
 * Batch rather than one edge at a time, because the intermediate states of a restructure are not
 * states anyone wants the dispatcher to see. Re-pointing a task from one prerequisite to another
 * as two calls leaves it, for a moment, either depending on both (blocked when it should run) or
 * on neither (running when it should wait) — and the auto-run sweep fires every 60 seconds, so
 * that moment is long enough to start the wrong work.
 */
export type DagOp =
  | { op: 'add'; taskId: string; dependsOnTaskId: string }
  | { op: 'remove'; taskId: string; dependsOnTaskId: string };
...
const key = (e: DependencyEdge) => `${e.taskId} ${e.dependsOnTaskId}`;
 ...
 */
export function resultingEdges(current: DependencyEdge[], ops: DagOp[]): DependencyEdge[] {
  const edges = new Map(current.map((e) => [key(e), e]));
  for (const op of ops) if (op.op === 'remove') edges.delete(key(op));
  for (const op of ops) if (op.op === 'add') edges.set(key(op), op);
  return [...edges.values()];
}
...
export function findCycle(edges: DependencyEdge[]): string[] | null {
  ...
  const walk = (start: string): string[] | null => {
    ...
    while (stack.length) {
      ...
      if (seen === ON_STACK) return [...path.slice(path.indexOf(next)), next];
  ...
  for (const node of adjacency.keys()) {
    if ((state.get(node) ?? UNVISITED) !== UNVISITED) continue;
...
function prerequisitesByTask(edges: DependencyEdge[]): Map<string, string[]> {
  const out = new Map<string, string[]>();
  for (const e of edges) {
    const list = out.get(e.taskId);
    if (list) list.push(e.dependsOnTaskId);
    else out.set(e.taskId, [e.dependsOnTaskId]);
  }
  return out;
}

export interface DagStateChange {
  taskId: string;
...
export function stateChanges(
  ...
  statusOf: ReadonlyMap<string, TaskStatus>,
): DagStateChange[] {
  ...
  for (const taskId of new Set([...prereqBefore.keys(), ...prereqAfter.keys()])) {
    ...
    if (from !== to) out.push({ taskId, from, to });
  }
  return out;
}
 ...
 * approve.
 */
export function effectiveOps(current: DependencyEdge[], ops: DagOp[]): { effective: DagOp[]; noop: DagOp[] } {
  const present = new Set(current.map(key));
  const effective: DagOp[] = [];
  const noop: DagOp[] = [];
  for (const op of ops) {
    const exists = present.has(key(op));
    if (op.op === 'add' ? exists : !exists) noop.push(op);
    else effective.push(op);
  }
