/** What an agent's own notification says on a lock screen, or null for "there is nothing to say". */
export interface AgentAlert {
  /** Alert body. The session's title is the alert *title*, so this carries only the agent's line. */
  body: string;
  /** Whether the agent's message was longer than a notification can carry and was cut. */
  truncated: boolean;
}
 ...
 */
export function agentAlert(message: string): AgentAlert | null {
  const collapsed = message.replace(/\s+/g, ' ').trim();
  if (!collapsed) return null;
  if (collapsed.length <= MAX_BODY_CHARS) return { body: collapsed, truncated: false };
