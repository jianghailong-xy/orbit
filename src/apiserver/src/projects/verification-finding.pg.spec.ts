...
const FINDING = migration('0135_task_verification_finding');
...
async function reset(client: Client): Promise<void> {
  ...
  // coordinator pg specs do: the point is to exercise 0135's own SQL, not to replay 134 migrations.
  await client.query(`
...
test('0135 applies to a fresh database and changes no existing task', { skip }, async () => {
