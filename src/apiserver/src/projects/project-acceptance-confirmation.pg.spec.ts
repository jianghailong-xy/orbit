 ...
 * tasks nothing will start but that conversation — and `(8)` that the Automatic switch, the other
 * press that starts a project, tells it too.
 *
...
test('the owner confirms one version of a project’s acceptance standard set, and only the owner', {
  ...
  // ═══ the conversation holding the work, for (7) and (8) ════════════════════════════════════════

  // The press lets Orbit start the tasks opted into auto-run. The ones a coordinator filed to start
  // by hand wait on that coordinator, so the start is said to its conversation — through the one
  // collaborator every case above goes without, which is why they tell nobody.
  ...
  /** A project not yet started, coordinated from a conversation parked where a turn appends — on a
   *  workspace whose runner is heartbeating — with one task of each kind filed under it. */
  ...
  await t.test('(7) starting the project tells its coordinator which tasks wait on it, once', async () => {
    ...
    const { rows: [confirmation] } = await sql.query<{ id: string; confirmed_at: Date }>(
      `SELECT "id", "confirmed_at" FROM "project_standard_set_confirmation"
    ...
    const [, message] = told;
    assert.equal(message.client_turn_id, projectStartedTurnId(waiting.id, {
      by: 'CONFIRMATION', confirmationId: confirmation.id, criteriaCount: 1, at: confirmation.confirmed_at,
    }));
  ...
  await t.test('(8) switching the project on by hand tells its coordinator too, and only then', async () => {
    ...
    await switchTo(true);
    assert.deepEqual(await authorization(project.id), { enabled: true, revision: '1' });
    let told = await turns(project.sessionId);
    assert.equal(told.length, 2, 'switching the project on put exactly one message on its conversation');
    assert.equal(told[1].client_turn_id, projectStartedTurnId(project.id, {
      by: 'SWITCH', configRevision: '1', at: new Date(),
    }));
    assert.match(told[1].content, /^From Orbit · project switched on/);
    ...
    // switching it off.
    await switchTo(true);
    assert.deepEqual(await authorization(project.id), { enabled: true, revision: '2' });
    assert.equal((await turns(project.sessionId)).length, 2,
      'switching on a project that was already on told the coordinator again');
    await switchTo(false);
    assert.deepEqual(await authorization(project.id), { enabled: false, revision: '3' });
    assert.equal((await turns(project.sessionId)).length, 2, 'switching the project off told the coordinator');

    // Off → on again is a start again, and the owner who paused it is exactly who wants it heard.
    await switchTo(true);
    told = await turns(project.sessionId);
    assert.equal(told.length, 3, 'switching the project back on told the coordinator nothing');
