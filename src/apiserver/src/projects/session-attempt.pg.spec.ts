...
async function reset(client: Client): Promise<void> {
  ...
  // coordinator pg specs do — the point is to exercise 0133's own SQL, not to replay 132 migrations.
  await client.query(`
...
test('the budget column must carry all six dimensions, each finite or explicitly unbounded', { skip }, async () => {
  ...
  try {
    ...
    // CHECK from 0132 is what stops one being written without a USER's signature.
