...
/** The seven-key counters every task carried before 0140. */
...
/** The world as it stood BEFORE 0140: 0138 and 0139 applied, rows written, nothing else. */
async function resetToPre0140(client: Client): Promise<void> {
...
test('AC5: 0140 applies over a live 0138/0139 database and backfills both counters', { skip }, async () => {
  const client = await connect();
  try {
    await resetToPre0140(client);
    // A task and a committed decision written under the OLD seven-key shape, exactly as every
    // deployed row is on the day this migration runs. Written with pre-0140 SQL rather than
    ...
    assert.equal(row.action_identity, null, 'a pre-0140 row cannot claim an identity it never had');
...
test('AC5: a task created after 0140 is born with nine counters, not seven', { skip }, async () => {
  const client = await connect();
  try {
    await reset(client);
    await client.query(`
...
test('AC5: the counter CHECK and the reason CHECK both know the new names', { skip }, async () => {
  ...
  try {
    ...
    // And the two reasons a trip may now name are storable, which they were not before 0140.
