...
describe('context seed decisions', () => {
  const untouched: ContextSeedState = { contextKey: 'draft:agent-a', dirty: false };

  it('allows an async seed refinement while the current context is untouched', () => {
    expect(decideContextSeed(untouched, 'draft:agent-a', true).apply).toBe(true);
  });

  it('does not overwrite an explicit choice in the same context', () => {
    const dirty = dirtyContextSeed('draft:agent-a');
    expect(decideContextSeed(dirty, 'draft:agent-a', true).apply).toBe(false);
  });

  it('resets dirty when the Agent or Session context changes', () => {
