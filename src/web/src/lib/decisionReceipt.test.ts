...
describe('the criteria receipts an open conversation draws', () => {
  it('draws one per answered proposal the read publishes, at the moment it was decided', () => {
    expect(criteriaDecisionReceiptRows(queue([settled()]), EVENTS)).toEqual([
      { settled: settled(), reply: null, afterSeq: 2 },
    ]);
    ...
    expect(criteriaDecisionReceiptRows(queue([settled(), older]), EVENTS).map((row) => row.afterSeq))
      .toEqual([2, 1]);
  });
  ...
  it('draws nothing for an answer older than the events loaded so far', () => {
    const before = settled({ decidedAt: '2026-09-13T12:00:00.000Z' });
    expect(criteriaDecisionReceiptRows(queue([before]), EVENTS)).toEqual([]);
    // The paired negative on the same row: one event before it, and it is drawn.
    expect(criteriaDecisionReceiptRows(queue([before]), [
      { seq: 0, ts: '2026-09-13T11:00:00.000Z' },
      ...EVENTS,
    ]).map((row) => row.afterSeq)).toEqual([0]);
  });
