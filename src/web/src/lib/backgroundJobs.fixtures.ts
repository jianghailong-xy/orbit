/**
 * `<background-jobs>` blocks copied verbatim out of this deployment's `run_event` rows — both the
 * wording written today and the one that shipped until 2026-09-15, which 85 of the 106 blocks in the
 * record still carry. Each is stamped with the row it came from; none of it is typed out by hand.
 */

/** One job, ended: the shape 85 of the 106 blocks in this deployment have. (run_event 01a08842-5caa-7717-a77c-de1ebd7f701c) */
export const ZH_ENDED = `<background-jobs>
  你不在的时候结束了：
...
export const ZH_KILLED = `<background-jobs>
  你不在的时候结束了：
...
export const ZH_NO_END = `<background-jobs>
  你不在的时候结束了：
...
export const ZH_RUNNING_AND_ENDED = `<background-jobs>
...
export const ZH_MONITOR = `<background-jobs>
  你不在的时候结束了：
...
export const ZH_MULTILINE_COMMAND = `<background-jobs>
  仍在运行（runner 托管，不随 engine 重启而死）：
...
export const EN_ENDED = `<background-jobs>
...
export const EN_RUNNING_AND_ENDED = `<background-jobs>
