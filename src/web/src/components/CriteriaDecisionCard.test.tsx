...
describe('the receipt a decision leaves in the conversation', () => {
  ...
  const answered = (over: Partial<SettledCriteriaDecision> = {}): SettledCriteriaDecision => ({
    intentId: row().intentId,
    decision: 'APPROVE',
  ...
  const receipt = (
    settled: SettledCriteriaDecision,
    reply?: CriteriaDecisionResult['reply'],
  ): string => renderToStaticMarkup(
    <CriteriaDecisionReceipt settled={settled} reply={reply ?? null} />,
  );
  ...
  it('says the same of a refusal', () => {
    const html = receipt(answered({ decision: 'REJECT', resultingSeal: SEAL_DRAFTED }), toSession);
  ...
  it('claims no reply for a change nobody proposed through a session, or from an older server', () => {
    // The two shapes of "nothing to say": the door answered `reply: null`, and the read — which
    // never carries a destination at all — drew the receipt without one.
