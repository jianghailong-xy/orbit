...
// The rest of this suite renders with `react-dom/server`, which cannot press a button — and what
// this toggle is FOR is what happens when one is pressed, so this file mounts into a real DOM.
// jsdom is a dev dependency of @orbit/web for exactly this, selected per file by the docblock
// above; every other test file keeps running in node.
 ...
 * FACTORY, which vitest runs only when something actually imports the module — so a toggle that
 * pulled React Flow in eagerly would set it during the first mount, before any click. Stubbing it
 * also keeps React Flow itself out of jsdom, which has no ResizeObserver for it to measure with.
 */
...
beforeEach(() => {
  ...
  // would only hold for whichever test happened to run first.
  vi.resetModules();
...
async function loadToggle() {
  return (await import('./ProjectTasksViewToggle')).ProjectTasksViewToggle;
}
...
const LIST = <div data-testid="project-task-list">the task list</div>;

/** The two controls, found the way a reader finds them — by their label. */
function option(label: 'List' | 'Graph'): HTMLButtonElement {
  const button = Array.from(container.querySelectorAll('button')).find(
    (b) => b.textContent?.trim() === label,
  );
  if (!button) throw new Error(`the ${label} option is not on the toggle`);
  return button;
}
...
describe('ProjectTasksViewToggle', () => {
  it('opens on List, and has not loaded the graph module to do it', async () => {
    const Toggle = await loadToggle();
    await mount(
      <Toggle projectId="p1" taskCount={12}>
        {LIST}
      </Toggle>,
    );

    expect(option('List').getAttribute('aria-pressed')).toBe('true');
    expect(option('Graph').getAttribute('aria-pressed')).toBe('false');
    expect(testid('project-task-list')).not.toBeNull();
    // Both halves of "lazy": nothing from the graph module is on screen, and the module was never
    // even imported. The first check alone would also pass on an eager import that renders nothing.
    expect(testid('project-dependency-graph')).toBeNull();
    expect(graphModule.imported).toBe(false);
  });

  it('loads and shows the graph on Graph, and comes back to the list on List', async () => {
    const Toggle = await loadToggle();
    await mount(
      <Toggle projectId="p1" taskCount={12}>
        {LIST}
    ...
    await click(option('Graph'));
    expect(graphModule.imported).toBe(true);
    expect(testid('project-dependency-graph')).not.toBeNull();
    expect(testid('project-task-list')).toBeNull();
    expect(option('Graph').getAttribute('aria-pressed')).toBe('true');
    expect(option('List').getAttribute('aria-pressed')).toBe('false');

    await click(option('List'));
    expect(testid('project-task-list')).not.toBeNull();
    expect(testid('project-dependency-graph')).toBeNull();
    expect(option('List').getAttribute('aria-pressed')).toBe('true');
  });

  it('restores a remembered Graph pick, and falls back to List once it is cleared', async () => {
    const { PROJECT_TASKS_VIEW_STORAGE_KEY } = await import('./ProjectTasksViewToggle');
    localStorage.setItem(PROJECT_TASKS_VIEW_STORAGE_KEY, 'graph');
    const Toggle = await loadToggle();
    await mount(
      <Toggle projectId="p1" taskCount={12}>
        {LIST}
    ...
    // The reverse direction, which is the one that matters: List is the default because it is the
    // default, not because nothing has been stored yet. Clearing the key must come back to it.
    await act(async () => root.unmount());
    container.remove();
    localStorage.clear();
    await mount(
  ...
  it('remembers the pick for the next mount', async () => {
    const { PROJECT_TASKS_VIEW_STORAGE_KEY } = await import('./ProjectTasksViewToggle');
    const Toggle = await loadToggle();
    await mount(
      <Toggle projectId="p1" taskCount={12}>
