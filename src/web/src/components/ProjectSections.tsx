...
export interface ProjectSection<T extends SectionProject> {
  ...
  note: string;
  projects: T[];
  /** Folds on first render. The completed work does; the sections that need attention never should. */
 ...
 *
 * Collapsing is uniform — every section takes the same control, and a collapsed one folds into a row
 * of pills rather than disappearing, so a folded project stays visible, countable and one click from
 * being opened.
 */
...
function Section<T extends SectionProject>({
  ...
  return (
    <section aria-labelledby={headingId} data-section={section.key} style={{ marginTop: 20 }}>
      <div
        style={{
          display: 'flex',
          alignItems: 'center',
