...
// (which derives over its loaded event window for the live overlay) and the apiserver
// (which derives over ALL persisted events to serve the authoritative, complete list at
// GET /sessions/:id/background). Keeping one implementation guarantees the two never drift.
//
...
//
// NB: for AGENT-launched shells the live background_output tail is broadcast-only (never
// persisted), so a cold read of the stream has no output events at all — the agent's
// persisted `Read` polls are the only durable output source. That is exactly why the server
// derives this list rather than leaving each client to reconstruct output from live-only tails.
...
/** The minimal event shape this derivation reads — satisfied by both the web `RunEvent` and
 *  a persisted `run_event` row mapped to `{ seq, type, payload, ts }`. */
export interface BgDeriveEvent {
  seq: number;
  type: string;
  payload?: unknown;
  ts?: string | null;
...
export function deriveBackgroundShells(
  ...
  // tool_result → its originating tool_use, keyed by toolUseId (same linkage the transcript uses).
  const resultByToolUseId = new Map<string, Record<string, unknown>>();
  for (const ev of events) {
    if (ev.type === RunEventType.TOOL_RESULT) {
      const p = obj(ev.payload);
      const id = p.toolUseId;
      if (id != null) resultByToolUseId.set(String(id), p);
    }
  }
  ...
  for (const ev of events) {
    if (ev.type !== RunEventType.TOOL_USE) continue;
    const p = obj(ev.payload);
    ...
    if (p.name === 'Bash' && input.run_in_background === true) {
      ...
      const content = toolResultText(res.content);
    ...
    if (p.name === 'Read') {
      ...
      const res = resultByToolUseId.get(String(p.id));
      if (!res) continue;
      applyOutput(shell, toolResultText(res.content), ev.seq, ev.ts);
    }
  }
  ...
  for (const ev of events) {
    if (ev.type !== RunEventType.BACKGROUND_OUTPUT && ev.type !== RunEventType.BACKGROUND_TASK)
      continue;
    const p = obj(ev.payload);
    const shell = matchShell(p, byToolUseId, byId);
    if (!shell) continue;
    if (ev.type === RunEventType.BACKGROUND_OUTPUT) {
