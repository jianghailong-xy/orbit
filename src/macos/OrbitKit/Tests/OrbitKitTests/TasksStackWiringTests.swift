...
final class TasksStackWiringTests: XCTestCase {
    ...
    func testOneRowInTwoContainersAndOnlyTheCompactOnePushes() throws {
        ...
        // ...and its actions wrap the row, never sit inside what the row is made of. `.swipeActions`
        ...
        // compact row carried them — leaves a task row that cannot be swiped or long-pressed. The
        // actions are applied to the `Button` itself, at the row's own level.
        let push = code(String(row[pushBranch.lowerBound...]))
            .split(separator: "\n")
            .map { $0.trimmingCharacters(in: .whitespaces) }
            .filter { !$0.isEmpty }
        let actions = try XCTUnwrap(push.firstIndex { $0.contains("rowActions(") },
                                    "the compact row attaches its own actions")
        XCTAssertTrue(push[actions].contains("Button"),
                      "wrapped around the `Button` the row is made of, not nested inside its label")
