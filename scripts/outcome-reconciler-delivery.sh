#!/usr/bin/env bash
# Acceptance harness for first-class merge/delivery obligations. PostgreSQL is mandatory and
# disposable; no scenario is skipped because a shared database or provider fixture is missing.
set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "$REPO/scripts/lib/outcome-reconciler-release-dag.sh"
API="$REPO/src/apiserver"
CONTAINER="${OUTCOME_DELIVERY_PG_CONTAINER:-pcdelivery-pg16-$$}"
ADMIN="${OUTCOME_DELIVERY_PG_USER:-pcdelivery_admin}"
PASSWORD="${OUTCOME_DELIVERY_PG_PASSWORD:-pcdelivery_pw}"
DATABASE="${OUTCOME_DELIVERY_PG_DATABASE:-pcdelivery_outcome}"
IMAGE="${OUTCOME_DELIVERY_PG_IMAGE:-postgres:16-alpine}"
TIMEOUT_SECONDS="${OUTCOME_DELIVERY_TIMEOUT:-900}"
BUILD="$REPO/build"
COMPILED="$BUILD/outcome-delivery-ts"
TAP="$BUILD/outcome-reconciler-v2-delivery.tap"
EVIDENCE="$BUILD/outcome-reconciler-v2-delivery-evidence.json"
MANIFEST="$BUILD/outcome-reconciler-v2-delivery-manifest.json"
...
# This disposable worktree intentionally has no install. Link the already-provisioned cache only
# while production delivery code is compiled, and remove both links during cleanup.
...
if [ "${OUTCOME_RELEASE_DAG_PREPARED_BUILD:-0}" = 1 ]; then
  ...
  DELIVERY_MODULE="$API/dist/outcome-reconciler/delivery-attestation.js"
  DELIVERY_EVALUATOR_MODULE="$API/dist/outcome-reconciler/outcome-evaluator.js"
  echo '==> delivery-obligation: use exact bound production build'
else
  echo "==> delivery-obligation: compiling production reducer, Action Executor adapter and service"
  "$TSC" "$API/src/outcome-reconciler/delivery-attestation.ts" \
    "$API/src/outcome-reconciler/delivery-attestation.service.ts" \
  ...
  DELIVERY_MODULE="$COMPILED/outcome-reconciler/delivery-attestation.js"
  DELIVERY_EVALUATOR_MODULE="$COMPILED/outcome-reconciler/outcome-evaluator.js"
fi
...
if outcome_release_dag_db_enabled; then
  ...
  echo "==> delivery-obligation: provisioning disposable PostgreSQL 16"
  ...
  echo "==> delivery-obligation: applying every Prisma migration"
...
echo "==> delivery-obligation: migrations=$MIGRATIONS system_identifier=$SYSTEM_ID port=$PORT"

echo "==> delivery-obligation: running repository, target, replay, policy and recovery matrix"
set +e
NODE_PATH="$NODE_MODULES" \
OUTCOME_DELIVERY_MODULE="$DELIVERY_MODULE" \
OUTCOME_DELIVERY_EVALUATOR_MODULE="$DELIVERY_EVALUATOR_MODULE" \
OUTCOME_DELIVERY_PG_URL="$URL" \
OUTCOME_DELIVERY_PG_EXPECTED_DATABASE="$DATABASE" \
OUTCOME_DELIVERY_PG_EXPECTED_USER="$ADMIN" \
OUTCOME_DELIVERY_PG_EXPECTED_SYSTEM_IDENTIFIER="$SYSTEM_ID" \
OUTCOME_DELIVERY_EVIDENCE_PATH="$EVIDENCE" \
timeout -k 20 "$TIMEOUT_SECONDS" node --test --test-concurrency=1 \
  "$REPO/test/outcome-reconciler-v2.delivery.test.mjs" 2>&1 | tee "$TAP"
TEST_RC=${PIPESTATUS[0]}
set -e
if [ "$TEST_RC" -ne 0 ]; then
  echo "!! delivery-obligation tests failed rc=$TEST_RC" >&2
  exit "$TEST_RC"
fi

echo "==> delivery-obligation: validating zero-skip evidence and writing manifest"
OUTCOME_DELIVERY_STARTED_AT="$STARTED_AT" node \
  "$REPO/scripts/outcome-reconciler-delivery-manifest.mjs" "$TAP" "$EVIDENCE" "$MANIFEST"
