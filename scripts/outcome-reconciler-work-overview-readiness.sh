#!/usr/bin/env bash
# Unique executable acceptance for canonical Work overview readiness. All fixture writes target a
# disposable PostgreSQL container; the deployment checks are read-only and never start a task.
set -euo pipefail
...
cleanup() {
  if [ "$PG_STARTED" = "1" ]; then docker rm -fv "$PG_CONTAINER" >/dev/null 2>&1 || true; fi
...
echo "✓ Work overview readiness accepted: tests=22 skip=0 target=$TARGET_SHA"
