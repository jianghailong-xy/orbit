...
assert.ok(tapArg && evidenceArg && outputArg,
  'usage: outcome-reconciler-delivery-manifest.mjs TAP EVIDENCE OUTPUT');
...
assert.ok(summary.tests >= 12, 'the delivery suite is unexpectedly empty or truncated');
assert.equal(summary.passed, summary.tests, 'not every delivery test passed');
assert.equal(summary.failed, 0, 'delivery suite contains failures');
assert.equal(summary.skipped, 0, 'skipped delivery tests are forbidden');
assert.equal(summary.cancelled, 0, 'cancelled delivery tests are forbidden');
assert.equal(summary.todo, 0, 'todo delivery tests are forbidden');
assert.equal(evidence.schemaVersion, 1);
assert.equal(evidence.suite, 'outcome-reconciler-v2-delivery');
...
for (const label of ['scenarios', 'policies', 'actionExecutor', 'recovery', 'invariants']) {
  assertAllTrue(evidence[label], label);
}
assert.match(evidence.samples.satisfiedProofDigest, /^[0-9a-f]{64}$/);
assert.match(evidence.samples.reopenedObligationRevision, /^[0-9a-f]{64}$/);
assert.match(evidence.samples.deliveryBindingDigest, /^[0-9a-f]{64}$/);

const sourceFiles = [
  'package.json',
  'scripts/outcome-reconciler-delivery.sh',
  'scripts/outcome-reconciler-delivery-manifest.mjs',
  'src/apiserver/prisma/migrations/0198_outcome_delivery_attestation/migration.sql',
  ...
  'src/apiserver/src/outcome-reconciler/action-executor.ts',
  'src/apiserver/src/outcome-reconciler/delivery-attestation.ts',
  'src/apiserver/src/outcome-reconciler/delivery-attestation.service.ts',
  ...
  'test/outcome-reconciler-v2.delivery.test.mjs',
];
const sources = Object.fromEntries(sourceFiles.map((relative) => [relative, fileDigest(relative)]));
const startedAt = process.env.OUTCOME_DELIVERY_STARTED_AT;
assert.match(startedAt ?? '', /^\d{4}-\d\d-\d\dT/, 'OUTCOME_DELIVERY_STARTED_AT is required');
const body = {
  schemaVersion: 1,
  ...
  proofs: {
    ...
    actionExecutor: evidence.actionExecutor,
...
console.log(`outcome-delivery manifest: ${outputPath}`);
