...
assert.ok(tapArg && evidenceArg && outputArg,
  'usage: outcome-reconciler-evaluator-manifest.mjs TAP EVIDENCE OUTPUT');
...
assert.ok(summary.tests >= 16, 'the evaluator suite is unexpectedly empty or truncated');
assert.equal(summary.passed, summary.tests, 'not every evaluator test passed');
assert.equal(summary.failed, 0, 'evaluator suite contains failures');
assert.equal(summary.skipped, 0, 'skipped evaluator tests are forbidden');
assert.equal(summary.cancelled, 0, 'cancelled evaluator tests are forbidden');
assert.equal(summary.todo, 0, 'todo evaluator tests are forbidden');
assert.equal(evidence.schemaVersion, 1);
assert.equal(evidence.suite, 'outcome-reconciler-v2-evaluator');
...
assertAllTrue(evidence.pure, 'pure');
assertAllTrue(evidence.races, 'races');
assertAllTrue(evidence.invariants, 'invariants');
assert.match(evidence.samples.closedEvaluationDigest, /^[0-9a-f]{64}$/);
assert.match(evidence.samples.switchedEvaluationDigest, /^[0-9a-f]{64}$/);
assert.notEqual(evidence.samples.closedEvaluationDigest, evidence.samples.switchedEvaluationDigest);
assert.ok(BigInt(evidence.samples.newWatermark) > BigInt(evidence.samples.oldWatermark));

const sourceFiles = [
  'package.json',
  'scripts/outcome-reconciler-evaluator.sh',
  'scripts/outcome-reconciler-evaluator-manifest.mjs',
  ...
  'src/apiserver/src/outcome-reconciler/outcome-evaluator.service.ts',
  'src/apiserver/src/outcome-reconciler/outcome-reconciler.module.ts',
  'test/outcome-reconciler-v2.evaluator.test.mjs',
];
const sources = Object.fromEntries(sourceFiles.map((relative) => [relative, fileDigest(relative)]));
const startedAt = process.env.OUTCOME_EVALUATOR_STARTED_AT;
assert.match(startedAt ?? '', /^\d{4}-\d\d-\d\dT/, 'OUTCOME_EVALUATOR_STARTED_AT is required');
const body = {
  schemaVersion: 1,
  ...
  proofs: { pure: evidence.pure, races: evidence.races, invariants: evidence.invariants },
...
console.log(`outcome-evaluator manifest: ${outputPath}`);
