...
PASSWORD="${OUTCOME_DONE_GATE_PG_PASSWORD:-done_gate_pw}"
DATABASE="${OUTCOME_DONE_GATE_PG_DATABASE:-done_gate_outcome}"
IMAGE="${OUTCOME_DONE_GATE_PG_IMAGE:-postgres:16-alpine}"
TIMEOUT_SECONDS="${OUTCOME_DONE_GATE_TIMEOUT:-900}"
BUILD="$REPO/build"
COMPILED="$BUILD/outcome-done-gate-ts"
TAP="$BUILD/outcome-reconciler-v2-done-gate.tap"
EVIDENCE="$BUILD/outcome-reconciler-v2-done-gate-evidence.json"
MANIFEST="$BUILD/outcome-reconciler-v2-done-gate-manifest.json"
...
OUTCOME_DONE_GATE_PG_EXPECTED_SYSTEM_IDENTIFIER="$SYSTEM_ID" \
OUTCOME_DONE_GATE_EVIDENCE_PATH="$EVIDENCE" \
timeout -k 20 "$TIMEOUT_SECONDS" node --test --test-concurrency=1 \
  "$REPO/test/outcome-reconciler-v2.done-gate.test.mjs" 2>&1 | tee "$TAP"
TEST_RC=${PIPESTATUS[0]}
set -e
if [ "$TEST_RC" -ne 0 ]; then
  echo "!! done-gate tests failed rc=$TEST_RC" >&2
  exit "$TEST_RC"
fi

echo "==> done-gate: validating zero-skip evidence and writing manifest"
OUTCOME_DONE_GATE_STARTED_AT="$STARTED_AT" node \
  "$REPO/scripts/outcome-reconciler-done-gate-manifest.mjs" "$TAP" "$EVIDENCE" "$MANIFEST"
