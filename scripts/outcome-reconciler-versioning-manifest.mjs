...
assert.ok(tapArg && evidenceArg && outputArg,
  'usage: outcome-reconciler-versioning-manifest.mjs TAP EVIDENCE OUTPUT');
...
assert.ok(summary.tests >= 9, 'the versioning suite is unexpectedly empty or truncated');
assert.equal(summary.passed, summary.tests, 'not every versioning test passed');
assert.equal(summary.failed, 0, 'versioning suite contains failures');
assert.equal(summary.skipped, 0, 'skipped versioning tests are forbidden');
assert.equal(summary.cancelled, 0, 'cancelled versioning tests are forbidden');
assert.equal(summary.todo, 0, 'todo versioning tests are forbidden');
assert.equal(evidence.schemaVersion, 1);
assert.equal(evidence.suite, 'outcome-reconciler-v2-binding-versioning');
...
assert.equal(Object.keys(evidence.dimensions).length, 17);
assertAllTrue(evidence.dimensions, 'dimensions');
assertAllTrue(evidence.invariants, 'invariants');
assertAllTrue(evidence.races, 'races');
assert.equal(evidence.samples.bindingDimensions, 17);
assert.ok(evidence.samples.multipleSuccessorCount > 1);
assert.ok(evidence.samples.obsoleteProofs > 0);
assert.ok(evidence.samples.successorEdges > 0);

const sourceFiles = [
  'package.json',
  'scripts/outcome-reconciler-versioning.sh',
  'scripts/outcome-reconciler-versioning-manifest.mjs',
  'src/apiserver/prisma/migrations/0196_outcome_binding_version_invalidation/migration.sql',
  'src/apiserver/prisma/schema.prisma',
  'src/apiserver/src/outcome-reconciler/outcome-evaluator.ts',
  'src/apiserver/src/outcome-reconciler/outcome-versioning.service.ts',
  'src/apiserver/src/outcome-reconciler/outcome-reconciler.module.ts',
  'test/outcome-reconciler-v2.versioning.test.mjs',
];
const sources = Object.fromEntries(sourceFiles.map((relative) => [relative, fileDigest(relative)]));
const startedAt = process.env.OUTCOME_VERSIONING_STARTED_AT;
assert.match(startedAt ?? '', /^\d{4}-\d\d-\d\dT/, 'OUTCOME_VERSIONING_STARTED_AT is required');
const body = {
  schemaVersion: 1,
...
console.log(`outcome-versioning manifest: ${outputPath}`);
