#!/usr/bin/env bash
# Acceptance harness for the V2 canonical fact ledger. PostgreSQL is mandatory: no URL-less skip,
# no shared Orbit database, and no reuse of a server that another test can mutate underneath us.
set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "$REPO/scripts/lib/outcome-reconciler-release-dag.sh"
API="$REPO/src/apiserver"
CONTAINER="${OUTCOME_FACT_PG_CONTAINER:-pccfact-ingress-pg16-$$}"
ADMIN="${OUTCOME_FACT_PG_USER:-pccfact_admin}"
PASSWORD="${OUTCOME_FACT_PG_PASSWORD:-pccfact_pw}"
DATABASE="${OUTCOME_FACT_PG_DATABASE:-pccfact_ingress}"
IMAGE="${OUTCOME_FACT_PG_IMAGE:-postgres:16-alpine}"
TIMEOUT_SECONDS="${OUTCOME_FACT_TIMEOUT:-600}"
BUILD="$REPO/build"
TAP="$BUILD/outcome-reconciler-v2-fact-ingress.tap"
EVIDENCE="$BUILD/outcome-reconciler-v2-fact-ingress-evidence.json"
MANIFEST="$BUILD/outcome-reconciler-v2-fact-ingress-manifest.json"
...
if outcome_release_dag_db_enabled; then
  ...
  echo "==> fact-ingress: provisioning disposable PostgreSQL 16"
  ...
  echo "==> fact-ingress: applying every Prisma migration"
...
echo "==> fact-ingress: migrations=$MIGRATIONS system_identifier=$SYSTEM_ID port=$PORT"

echo "==> fact-ingress: running PostgreSQL invariants and races"
set +e
NODE_PATH="$NODE_MODULES" \
OUTCOME_FACT_PG_URL="$URL" \
OUTCOME_FACT_PG_EXPECTED_DATABASE="$DATABASE" \
OUTCOME_FACT_PG_EXPECTED_USER="$ADMIN" \
OUTCOME_FACT_PG_EXPECTED_SYSTEM_IDENTIFIER="$SYSTEM_ID" \
OUTCOME_FACT_EVIDENCE_PATH="$EVIDENCE" \
timeout -k 20 "$TIMEOUT_SECONDS" node --test --test-concurrency=1 \
  "$REPO/test/outcome-reconciler-v2.fact-ingress.test.mjs" 2>&1 | tee "$TAP"
TEST_RC=${PIPESTATUS[0]}
set -e
if [ "$TEST_RC" -ne 0 ]; then
  echo "!! fact-ingress tests failed rc=$TEST_RC" >&2
  exit "$TEST_RC"
fi

echo "==> fact-ingress: validating zero-skip evidence and writing manifest"
OUTCOME_FACT_STARTED_AT="$STARTED_AT" node \
  "$REPO/scripts/outcome-reconciler-fact-ingress-manifest.mjs" "$TAP" "$EVIDENCE" "$MANIFEST"
