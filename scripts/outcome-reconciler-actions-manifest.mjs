...
assert.ok(tapArg && evidenceArg && outputArg,
  'usage: outcome-reconciler-actions-manifest.mjs TAP EVIDENCE OUTPUT');
...
assert.ok(summary.tests >= 16, 'the action suite is unexpectedly empty or truncated');
assert.equal(summary.passed, summary.tests, 'not every action test passed');
assert.equal(summary.failed, 0, 'action suite contains failures');
assert.equal(summary.skipped, 0, 'skipped action tests are forbidden');
assert.equal(summary.cancelled, 0, 'cancelled action tests are forbidden');
assert.equal(summary.todo, 0, 'todo action tests are forbidden');
assert.equal(evidence.schemaVersion, 1);
assert.equal(evidence.suite, 'outcome-reconciler-v2-actions');
...
for (const label of ['validation', 'idempotency', 'scheduling', 'races', 'recovery', 'modelGap']) {
  assertAllTrue(evidence[label], label);
}
assert.match(evidence.samples.replayedActionIntentId, /^[0-9a-f-]{36}$/);
assert.match(evidence.samples.diagnosticId, /^[0-9a-f-]{36}$/);
assert.equal(evidence.samples.fairDispatch.length, 2);
assert.notEqual(evidence.samples.fairDispatch[0], evidence.samples.fairDispatch[1]);

const sourceFiles = [
  'package.json',
  'scripts/outcome-reconciler-actions.sh',
  'scripts/outcome-reconciler-actions-manifest.mjs',
  ...
  'test/outcome-reconciler-v2.actions.test.mjs',
];
const sources = Object.fromEntries(sourceFiles.map((relative) => [relative, fileDigest(relative)]));
const startedAt = process.env.OUTCOME_ACTION_STARTED_AT;
assert.match(startedAt ?? '', /^\d{4}-\d\d-\d\dT/, 'OUTCOME_ACTION_STARTED_AT is required');
const body = {
  schemaVersion: 1,
...
console.log(`outcome-actions manifest: ${outputPath}`);
